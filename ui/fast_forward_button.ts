...
export class FastForwardButton extends Element {
  ...
  private updateAriaLabel_() {
    this.button_.ariaLabel = this.localization.resolve(shaka.ui.Locales.Ids.FAST_FORWARD);
  }
  ...
  private fastForward_() {
    ...
    const newRate = newRateIndex != this.fastForwardRates_.length ? this.fastForwardRates_[newRateIndex] : this.fastForwardRates_[0];
