...
export class AirPlayButton extends Element {
  ...
  constructor(parent: HTMLElement, controls: Controls) {
    ...
    this.airplayCurrentSelectionSpan_.classList.add('shaka-current-selection-span');
    ...
    this.eventManager.listen(video, 'webkitplaybacktargetavailabilitychanged',  
    (e) => {
      const event = (e as AirPlayEvent);
      this.onAirPlayAvailabilityChange_(event);
    });
    this.eventManager.listen(video, 'webkitcurrentplaybacktargetiswirelesschanged',  
    () => {
      this.onAirPlayStatusChange_();
    });
  }
  ...
  private updateLocalizedStrings_() {
    ...
    this.airplayNameSpan_.textContent = this.localization.resolve(LocIds.AIRPLAY);
  }
}
