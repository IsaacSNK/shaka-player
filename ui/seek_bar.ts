 ...
 * @export
 */ 
export class SeekBar extends RangeElement implements shaka.extern.IUISeekBar {
  ...
  constructor(parent: HTMLElement, controls: Controls) {
    super(parent, controls, ['shaka-seek-bar-container'], ['shaka-seek-bar', 'shaka-no-propagation', 'shaka-show-controls-on-mouse-over']);
    ...
    // positioning. 
    this.container.insertBefore(this.adMarkerContainer_, this.container.childNodes[0]);
    ...
    this.eventManager.listen(this.adManager, AdManagerExports.CUEPOINTS_CHANGED,  
    (e) => {
  ...
  update() {
    ...
    const bufferedEnd = bufferedLength ? this.video.buffered.end(bufferedLength - 1) : 0;
    ...
    if (!this.shouldBeDisplayed_()) {
      ...
      if (bufferedLength == 0) {
        ...
        const unbufferedColor = this.config_.showUnbufferedStart ? colors.base : colors.played;
        const gradient = ['to right', this.makeColor_(unbufferedColor, bufferStartFraction), this.makeColor_(colors.played, bufferStartFraction), this.makeColor_(colors.played, playheadFraction), this.makeColor_(colors.buffered, playheadFraction), this.makeColor_(colors.buffered, bufferEndFraction), this.makeColor_(colors.base, bufferEndFraction)];
  ...
  private markAdBreaks_() {
    ...
    this.adMarkerContainer_.style.background = 'linear-gradient(' + gradient.join(',') + ')';
  }
  ...
  private shouldBeDisplayed_(): boolean {
    ...
    const seekRange = this.player.seekRange();
    const seekRangeSize = seekRange.end - seekRange.start;
    if (this.player.isLive() && seekRangeSize < Constants.MIN_SEEK_WINDOW_TO_SHOW_SEEKBAR) {
      return false;
    }
 ...
 * @export
 */ 
export class Factory implements shaka.extern.IUISeekBar.Factory {
