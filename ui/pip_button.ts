...
export class PipButton extends Element {
  ...
  constructor(parent: HTMLElement, controls: Controls) {
    ...
    this.pipNameSpan_.textContent = this.localization.resolve(LocIds.PICTURE_IN_PICTURE);
  ...
  private async onPipClick_(): Promise {
    try {
      ...
      this.controls.dispatchEvent(new FakeEvent('error', (new Map()).set('detail', error)));
    }
  }
  ...
  private onEnterPictureInPicture_() {
    ...
    this.pipButton_.ariaLabel = this.localization.resolve(LocIds.EXIT_PICTURE_IN_PICTURE);
  ...
  private onLeavePictureInPicture_() {
    const LocIds = shaka.ui.Locales.Ids;
    this.pipIcon_.textContent = Enums.MaterialDesignIcons.PIP;
    this.pipButton_.ariaLabel = this.localization.resolve(LocIds.ENTER_PICTURE_IN_PICTURE);
  ...
  private updateLocalizedStrings_() {
    const LocIds = shaka.ui.Locales.Ids;
    this.pipNameSpan_.textContent = this.localization.resolve(LocIds.PICTURE_IN_PICTURE);
    const ariaLabel = document.pictureInPictureElement ? LocIds.EXIT_PICTURE_IN_PICTURE : LocIds.ENTER_PICTURE_IN_PICTURE;
    this.pipButton_.ariaLabel = this.localization.resolve(ariaLabel);
    const currentPipState = document.pictureInPictureElement ? LocIds.ON : LocIds.OFF;
    this.currentPipState_.textContent = this.localization.resolve(currentPipState);
  }
