...
namespace shaka.ui {
   ...
   * @export
   */
  export class Localization extends shaka.util.FakeEventTarget {
    ...
    changeLocale(locales: Iterable<string>) {
      ...
      const data = new Map().set(
        "locales",
        found.length ? found : [this.fallbackLocale_]
      );
      this.dispatchEvent(new shaka.util.FakeEvent(Class.LOCALE_CHANGED, data));
    }
    ...
    private updateCurrentMap_() {
      ...
      const localizations: Map<string, Map<string, string>> = this
        .localizations_;
...
namespace shaka.ui.Localization {
  /**
