...
export class Localization extends FakeEventTarget {
  ...
  changeLocale(locales: Iterable<string>) {
    ...
    const missing = Iterables.filter(
        this.currentLocales_, (locale) => !this.localizations_.has(locale));
    if (missing.length) {
      this.dispatchEvent(new FakeEvent(
          Class.UNKNOWN_LOCALES, (new Map()).set('locales', missing)));
    }
    const found = Iterables.filter(
        this.currentLocales_, (locale) => this.localizations_.has(locale));
   ...
   */
  insert(
      locale: string, localizations: Map<string, string>,
      conflictResolution?: ConflictResolution): Localization {
    const Class = Localization;
  ...
  private updateCurrentMap_() {
    ...
    for (const locale of preferredLocales) {
      ...
      const siblings = Iterables.filter(
          localizations.keys(),
      ...
      const children = Iterables.filter(
          localizations.keys(),
   ...
   *
   */
  private static findMissingKeys_(
      map: Map<string, string>, keys: Iterable<string>, missing: Set<string>) {
