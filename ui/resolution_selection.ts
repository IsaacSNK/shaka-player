...
export class ResolutionSelection extends SettingsMenu {
  ...
  private updateResolutionSelection_() {
    ...
    (track, idx) => {
      ...
      const otherIdx = this.player.isAudioOnly() ? tracks.findIndex( 
      (t) => t.bandwidth == track.bandwidth) : tracks.findIndex( 
    ...
    const backButton = Utils.getFirstDescendantWithClassName(this.menu, 'shaka-back-to-overflow-button');
    ...
    for (const track of tracks) {
      ...
      span.textContent = this.player.isAudioOnly() ? Math.round(track.bandwidth / 1000) + ' kbits/s' : track.height + 'p';
    ...
    this.abrOnSpan_.textContent = this.localization.resolve(shaka.ui.Locales.Ids.AUTO_QUALITY);
    ...
    if (abrEnabled) {
      ...
      this.currentSelection.textContent = this.localization.resolve(shaka.ui.Locales.Ids.AUTO_QUALITY);
    }
  ...
  private updateLocalizedStrings_() {
    ...
    this.abrOnSpan_.textContent = this.localization.resolve(LocIds.AUTO_QUALITY);
