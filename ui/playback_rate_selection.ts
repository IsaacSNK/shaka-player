...
export class PlaybackRateSelection extends SettingsMenu {
  ...
  private updatePlaybackRateSelection_(rate: number) {
    ...
    const checkmarkIcon = Utils.getDescendantIfExists(this.menu, 'material-icons-round shaka-chosen-item');
    if (checkmarkIcon) {
      const previouslySelectedButton = checkmarkIcon.parentElement;
      previouslySelectedButton.removeAttribute('aria-selected');
      const previouslySelectedSpan = previouslySelectedButton.getElementsByTagName('span')[0];
    ...
    // button. 
    const span = Array.from(this.menu.querySelectorAll('span')).find( 
    (el) => {
