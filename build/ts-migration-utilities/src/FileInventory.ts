...
export const generateFileInventory = (filePath: string, includeFolders: string[]) => {
    ...
    if (!(fs.statSync(filePath).isDirectory())) {
        ...
        includeFolders.forEach(inclusion => {
            ...
            if (effectiveBasePath.endsWith("/")) {
                effectiveBasePath = effectiveBasePath.substring(0, effectiveBasePath.length - 1);
            }
            if (effectInclusionPath.startsWith("/")) {
                effectInclusionPath = effectInclusionPath.substring(1);
            }
            filesToAdd = [...filesToAdd, ...getAllFilesForDir(effectiveBasePath + "/" + effectInclusionPath)];
        });
    }
    return filesToAdd;
};
