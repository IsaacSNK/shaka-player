...
namespace shaka.dash {
  ...
  export class SegmentTemplate {
    ...
    static createStreamInfo(
      ...
      isUpdate: boolean,
      segmentLimit: number,
      periodDurationMap: { [key: string]: number }
    ): StreamInfo {
      goog.asserts.assert(
     ...
     *
     */
    private static parseSegmentTemplateInfo_(
      context: Context
    ): SegmentTemplateInfo {
      const SegmentTemplate = shaka.dash.SegmentTemplate;
    ...
    private static generateSegmentIndexFromDuration_(
      ...
      const segmentIndex: SegmentIndex = new shaka.media.SegmentIndex(
        references
      );
     ...
     *
     */
    private static createInitSegment_(context: Context): InitSegmentReference {
      const MpdUtils = shaka.dash.MpdUtils;
      const ManifestParserUtils = shaka.util.ManifestParserUtils;
      const SegmentTemplate = shaka.dash.SegmentTemplate;
      ...
      const getUris = () => {
        ...
        const resolvedUris = ManifestParserUtils.resolveUris(baseUris, [
          filledTemplate,
        ]);
        return resolvedUris;
      };
