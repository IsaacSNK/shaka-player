...
export class SegmentTemplate {
  ...
  /**
     ...
     */ 
  static createStreamInfo(context: DashParserExports.Context, requestInitSegment: DashParserExports.RequestInitSegmentCallback, streamMap: {[key:string]:shaka.extern.Stream}, isUpdate: boolean, segmentLimit: number, periodDurationMap: {[key:string]:number}): DashParserExports.StreamInfo {
    asserts.assert(context.representation.segmentTemplate, 'Should only be called with SegmentTemplate');
    ...
    if (info.indexTemplate) {
      ...
      return {generateSegmentIndex: 
      () => {
        return SegmentTemplate.generateSegmentIndexFromIndexTemplate_(shallowCopyOfContext, requestInitSegment, initSegmentReference, info);
      }};
    } else {
      if (info.segmentDuration) {
        ...
        return {generateSegmentIndex: 
        () => {
          return SegmentTemplate.generateSegmentIndexFromDuration_(shallowCopyOfContext, info, segmentLimit, initSegmentReference, periodDurationMap);
        }};
      } else {
  ...
  private static fromInheritance_(frame: DashParserExports.InheritanceFrame | null): Element {
  ...
  /**
     ...
     *
     */ 
  private static parseSegmentTemplateInfo_(context: DashParserExports.Context): SegmentTemplateInfo {
    const SegmentTemplate = SegmentTemplate;
    ...
    return {segmentDuration:segmentInfo.segmentDuration, timescale:segmentInfo.timescale, startNumber:segmentInfo.startNumber, scaledPresentationTimeOffset:segmentInfo.scaledPresentationTimeOffset, unscaledPresentationTimeOffset:segmentInfo.unscaledPresentationTimeOffset, timeline:segmentInfo.timeline, mediaTemplate:media, indexTemplate:index};
  }
  ...
  /**
     ...
     *
     */ 
  private static checkSegmentTemplateInfo_(context: DashParserExports.Context, info: SegmentTemplateInfo) {
    let n = 0;
    n += info.indexTemplate ? 1 : 0;
    n += info.timeline ? 1 : 0;
    n += info.segmentDuration ? 1 : 0;
    if (n == 0) {
      log.error('SegmentTemplate does not contain any segment information:', 'the SegmentTemplate must contain either an index URL template', 'a SegmentTimeline, or a segment duration.', context.representation);
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.DASH_NO_SEGMENT_INFO);
    } else {
      if (n != 1) {
        log.warning('SegmentTemplate containes multiple segment information sources:', 'the SegmentTemplate should only contain an index URL template,', 'a SegmentTimeline or a segment duration.', context.representation);
        if (info.indexTemplate) {
    ...
    if (!info.indexTemplate && !info.mediaTemplate) {
      log.error('SegmentTemplate does not contain sufficient segment information:', "the SegmentTemplate's media URL template is missing.", context.representation);
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.DASH_NO_SEGMENT_INFO);
    }
  }
  ...
  /**
     ...
     *     requestInitSegment
     */ 
  private static generateSegmentIndexFromIndexTemplate_(context: DashParserExports.Context, requestInitSegment: DashParserExports.RequestInitSegmentCallback, init: InitSegmentReference, info: SegmentTemplateInfo): Promise<SegmentIndex> {
    const MpdUtils = MpdUtils;
  ...
  /**
     ...
     */ 
  private static generateSegmentIndexFromDuration_(context: DashParserExports.Context, info: SegmentTemplateInfo, segmentLimit: number, initSegmentReference: InitSegmentReference, periodDurationMap: {[key:string]:number}): Promise<SegmentIndex> {
    ...
    () => {
      return [Math.max(presentationTimeline.getSegmentAvailabilityStart(), periodStart), Math.min(presentationTimeline.getSegmentAvailabilityEnd(), getPeriodEnd())];
    };
    ...
    () => {
      ...
      asserts.assert(availablePresentationTimes.every( 
      (x) => x >= 0), 'Available presentation times must be positive!');
      asserts.assert(segmentDuration != null, 'Segment duration must not be null!');
       
      // In period-relative timestamps. 
      const availablePeriodTimes = availablePresentationTimes.map( 
      (x) => x - periodStart);
      ...
      const availablePeriodPositions = [Math.ceil(availablePeriodTimes[0] / segmentDuration), Math.ceil(availablePeriodTimes[1] / segmentDuration) - 1];
       
      // In absolute positions. 
      const availablePresentationPositions = availablePeriodPositions.map( 
      (x) => x + startNumber);
    ...
    (position) => {
      ...
      const ref = new SegmentReference(segmentStart, segmentEnd, getUris,  
      /* startByte= */ 
      0,  
      /* endByte= */ 
  ...
  /**
     ...
     *
     */ 
  private static createFromTimeline_(context: DashParserExports.Context, info: SegmentTemplateInfo, initSegmentReference: InitSegmentReference): SegmentReference[] {
    const MpdUtils = MpdUtils;
    const ManifestParserUtils = ManifestParserUtils;
    const periodStart = context.periodInfo.start;
    ...
    for (let i = 0; i < info.timeline.length; i++) {
      ...
      const createUris =  
      () => {
        asserts.assert(mediaTemplate, 'There should be a media template with a timeline');
        const mediaUri = MpdUtils.fillUriTemplate(mediaTemplate, repId, segmentReplacement, bandwidth || null, timeReplacement);
        return ManifestParserUtils.resolveUris(baseUris, [mediaUri]).map( 
        (g) => {
          return g.toString();
        });
      };
      references.push(new SegmentReference(periodStart + start, periodStart + end, createUris,  
      /* startByte= */ 
      0,  
      /* endByte= */ 
  ...
  /**
     ...
     *
     */ 
  private static createInitSegment_(context: DashParserExports.Context): InitSegmentReference {
    const MpdUtils = MpdUtils;
    const ManifestParserUtils = ManifestParserUtils;
    const SegmentTemplate = SegmentTemplate;
...
type SegmentTemplateInfo = {timescale:number, segmentDuration:number | null, startNumber:number, scaledPresentationTimeOffset:number, unscaledPresentationTimeOffset:number, timeline:MpdUtilsExports.TimeRange[], mediaTemplate:string | null, indexTemplate:string | null};
