...
export class SegmentList {
  ...
  /**
     ...
     *
     */ 
  static createStreamInfo(context: DashParserExports.Context, streamMap: {[key:string]:shaka.extern.Stream}): DashParserExports.StreamInfo {
  ...
  /**
     ...
     *
     */ 
  private static parseSegmentListInfo_(context: DashParserExports.Context): SegmentListInfo {
    const SegmentList = SegmentList;
    ...
    return {segmentDuration:segmentInfo.segmentDuration, startTime:startTime, startNumber:startNumber, scaledPresentationTimeOffset:segmentInfo.scaledPresentationTimeOffset, timeline:segmentInfo.timeline, mediaSegments:mediaSegments};
  }
  ...
  /**
     ...
     *
     */ 
  private static checkSegmentListInfo_(context: DashParserExports.Context, info: SegmentListInfo) {
    if (!info.segmentDuration && !info.timeline && info.mediaSegments.length > 1) {
      log.warning('SegmentList does not contain sufficient segment information:', 'the SegmentList specifies multiple segments,', 'but does not specify a segment duration or timeline.', context.representation);
    ...
    if (!info.segmentDuration && !context.periodInfo.duration && !info.timeline && info.mediaSegments.length == 1) {
      log.warning('SegmentList does not contain sufficient segment information:', 'the SegmentList specifies one segment,', 'but does not specify a segment duration, period duration,', 'or timeline.', context.representation);
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.DASH_NO_SEGMENT_INFO);
    }
    if (info.timeline && info.timeline.length == 0) {
      log.warning('SegmentList does not contain sufficient segment information:', 'the SegmentList has an empty timeline.', context.representation);
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.DASH_NO_SEGMENT_INFO);
    }
  }
  ...
  /**
     ...
     */ 
  private static createSegmentReferences_(periodStart: number, periodDuration: number | null, startNumber: number, baseUris: string[], info: SegmentListInfo, initSegmentReference: InitSegmentReference): SegmentReference[] {
    ...
    if (info.timeline && info.timeline.length != info.mediaSegments.length) {
      ...
      log.warning('The number of items in the segment timeline and the number of ', 'segment URLs do not match, truncating', info.mediaSegments.length, 'to', max);
    }
    ...
    for (let i = 0; i < max; i++) {
      ...
      if (info.segmentDuration != null) {
        ...
        if (info.timeline) {
          ...
          asserts.assert(info.mediaSegments.length == 1 && periodDuration, 'There should be exactly one segment with a Period duration.');
          endTime = startTime + periodDuration;
        }
      }
      const getUris =  
      () => mediaUri;
      references.push(new SegmentReference(periodStart + startTime, periodStart + endTime, getUris, segment.start, segment.end, initSegmentReference, timestampOffset, appendWindowStart, appendWindowEnd));
      prevEndTime = endTime;
    }
    return references;
  }
  ...
  /**
     ...
     *
     */ 
  private static parseMediaSegments_(context: DashParserExports.Context): MediaSegment[] {
    const Functional = Functional;
    const segmentLists: Element[] = [context.representation.segmentList, context.adaptationSet.segmentList, context.period.segmentList].filter(Functional.isNotNull);
    ...
    (urlNode) => {
      if (urlNode.getAttribute('indexRange') && !context.indexRangeWarningGiven) {
        ...
        log.warning('We do not support the SegmentURL@indexRange attribute on ' + 'SegmentList.  We only use the SegmentList@duration ' + 'attribute or SegmentTimeline, which must be accurate.');
      }
      const uri = urlNode.getAttribute('media');
      const range = XmlUtils.parseAttr(urlNode, 'mediaRange', XmlUtils.parseRange, {start:0, end:null});
      return {mediaUri:uri, start:range.start, end:range.end};
    });
  }
}
type MediaSegment = {mediaUri:string, start:number, end:number | null};
 
export{MediaSegment};
type SegmentListInfo = {segmentDuration:number | null, startTime:number, startNumber:number, scaledPresentationTimeOffset:number, timeline:MpdUtilsExports.TimeRange[], mediaSegments:MediaSegment[]};
