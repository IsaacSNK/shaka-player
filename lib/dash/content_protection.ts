...
export class ContentProtection {
  ...
  /**
     ...
     *
     */ 
  static parseFromAdaptationSet(elems: Element[], ignoreDrmInfo: boolean, keySystemsByURI: {[key:string]:string}): Context {
    ...
    const keyIds = new Set(parsed.map( 
    (element) => element.keyId));
     
    // Remove any possible null value (elements may have no key ids). 
    keyIds.delete(null);
    if (keyIds.size > 1) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.DASH_CONFLICTING_KEY_IDS);
    }
    if (!ignoreDrmInfo) {
       
      // Find the default key ID and init data.  Create a new array of all the
      // non-CENC elements. 
      parsedNonCenc = parsed.filter( 
      (elem) => {
        if (elem.schemeUri == ContentProtection.MP4Protection_) {
          asserts.assert(!elem.init || elem.init.length, 'Init data must be null or non-empty.');
      ...
      if (parsedNonCenc.length) {
        drmInfos = ContentProtection.convertElements_(defaultInit, parsedNonCenc, keySystemsByURI, keyIds);
    ...
    return {defaultKeyId:defaultKeyId, defaultInit:defaultInit, drmInfos:drmInfos, firstRepresentation:true};
  }
  ...
  /**
     ...
     */ 
  static parseFromRepresentation(elems: Element[], context: Context, ignoreDrmInfo: boolean, keySystemsByURI: {[key:string]:string}): string | null {
    const ContentProtection = ContentProtection;
    ...
    if (context.firstRepresentation) {
      ...
      if (repContext.drmInfos.length > 0) {
        ...
        context.drmInfos = context.drmInfos.filter( 
        (asInfo) => {
          return repContext.drmInfos.some( 
          (repInfo) => {
        ...
        if (context.drmInfos.length == 0) {
          throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.DASH_NO_COMMON_KEY_SYSTEM);
        }
      }
    }
  ...
  static getClearKeyLicenseUrl(element: Element): string {
    const clearKeyLaurlNode = XmlUtils.findChildNS(element.node, ClearKeyNamespaceUri_, 'Laurl');
  ...
  /**
     ...
     *
     */ 
  private static parseMsProRecords_(view: DataView, byteOffset: number): PlayReadyRecord[] {
    const records = [];
  ...
  /**
     ...
     *
     */ 
  private static getInitDataFromPro_(element: Element): shaka.extern.InitDataOverride[] | null {
    const proNode = XmlUtils.findChildNS(element.node, 'urn:microsoft:playready', 'pro');
    if (!proNode) {
      return null;
    }
  ...
  /**
     ...
     */ 
  private static getInitDataClearKey_(element: Element, keyIds: Set<string>): shaka.extern.InitDataOverride[] | null {
    if (keyIds.size == 0) {
      return null;
    }
    const systemId = new Uint8Array([16, 119, 239, 236, 192, 178, 77, 2, 172, 227, 60, 30, 82, 226, 251, 75]);
  ...
  /**
     ...
     *
     */ 
  private static convertElements_(defaultInit: shaka.extern.InitDataOverride[], elements: Element[], keySystemsByURI: {[key:string]:string}, keyIds: Set<string>): shaka.extern.DrmInfo[] {
    const ContentProtection = ContentProtection;
    const ManifestParserUtils = ManifestParserUtils;
  ...
  private static parseElement_(elem: Element): Element | null {
    ...
    const psshs: string[] = XmlUtils.findChildrenNS(elem, NS, 'pssh').map(XmlUtils.getContents);
    if (!schemeUri) {
    ...
    if (keyId) {
      ...
      if (keyId.includes(' ')) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.DASH_MULTIPLE_KEY_IDS_NOT_SUPPORTED);
      }
    }
    ...
    try {
      ...
      init = psshs.map( 
      (pssh) => {
        return {initDataType:'cenc', initData:Uint8ArrayUtils.fromBase64(pssh), keyId:null};
      });
    } catch (e) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.DASH_PSSH_BAD_ENCODING);
    }
    return {node:elem, schemeUri:schemeUri, keyId:keyId, init:init.length > 0 ? init : null};
  }
}
type PlayReadyRecord = {type:number, value:Uint8Array};
...
type Context = {defaultKeyId:string | null, defaultInit:shaka.extern.InitDataOverride[], drmInfos:shaka.extern.DrmInfo[], firstRepresentation:boolean};
 
export{Context};
type Element = {node:Element, schemeUri:string, keyId:string | null, init:shaka.extern.InitDataOverride[]};
...
export const ClearKeyNamespaceUri_: string = 'http://dashif.org/guidelines/clearKey';
 
export const ClearKeySchemeUri_: string = 'urn:uuid:e2719d58-a985-b3c9-781a-b030af78d30e';
