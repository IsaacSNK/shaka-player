...
namespace shaka.dash {
  ...
  export class DashParser implements shaka.extern.ManifestParser {
    ...
    private async processManifest_(
      ...
      this.updatePeriod_ = XmlUtils.parseAttr(
        mpd,
        "minimumUpdatePeriod",
    ...
    private parsePeriods_(
      ...
      for (let i = 0; i < periodNodes.length; i++) {
        ...
        const start = XmlUtils.parseAttr(
          elem,
          "start",
          XmlUtils.parseDuration,
          prevEnd
        ...
        const givenDuration = XmlUtils.parseAttr(
          elem,
          "duration",
          XmlUtils.parseDuration
        );
        ...
        if (next) {
          ...
          const nextStart = XmlUtils.parseAttr(
            next,
            "start",
            XmlUtils.parseDuration
          );
    ...
    private parsePeriod_(
      ...
      if (!context.period.id) {
        shaka.log.info(
          "No Period ID given for Period with start time " +
            periodInfo.start +
            ",  Assigning a default"
        );
        context.period.id = "__shaka_period_" + periodInfo.start;
      }
      const eventStreamNodes = XmlUtils.findChildren(
        periodInfo.node,
        "EventStream"
      );
      ...
      const adaptationSetNodes = XmlUtils.findChildren(
        periodInfo.node,
        "AdaptationSet"
      );
    ...
    private parseAdaptationSet_(
      ...
      const essentialProperties = XmlUtils.findChildren(
        elem,
        "EssentialProperty"
      );
      ...
      const supplementalProperties = XmlUtils.findChildren(
        elem,
      ...
      for (const prop of accessibilities) {
        ...
        if (schemeId == "urn:scte:dash:cc:cea-608:2015") {
          ...
          if (schemeId == "urn:scte:dash:cc:cea-708:2015") {
            let serviceNumber = 1;
            if (value != null) {
      ...
      const contentProtectionElems = XmlUtils.findChildren(
        elem,
        "ContentProtection"
      );
    ...
    private parseRepresentation_(
      ...
      node: Element
    ): shaka.extern.Stream | null {
      const XmlUtils = shaka.util.XmlUtils;
      const ContentType = shaka.util.ManifestParserUtils.ContentType;
      context.representation = this.createFrame_(
        node,
        context.adaptationSet,
        null
      );
      ...
      try {
        ...
        if (context.representation.segmentBase) {
          ...
          if (context.representation.segmentList) {
              ...
              streamInfo = {
                generateSegmentIndex: () => {
                  return Promise.resolve(
                    shaka.media.SegmentIndex.forSingleSegment(
                      periodStart,
                      duration,
                      baseUris
                    )
      ...
      const supplementalPropertyElems = XmlUtils.findChildren(
        node,
        "SupplementalProperty"
      );
      const hasJoc = supplementalPropertyElems.some((element) => {
        ...
        return (
          element.getAttribute("schemeIdUri") == expectedUri &&
      ...
      if (isText) {
        ...
        forced =
          roles.includes("forced_subtitle") ||
          roles.includes("forced-subtitle");
      }
      let tilesLayout;
      if (isImage) {
        const essentialPropertyElems = XmlUtils.findChildren(
          node,
          "EssentialProperty"
        );
      ...
      const stream: shaka.extern.Stream = {
        ...
        createSegmentIndex: async () => {
          ...
          const currentStream = storedInMap
    ...
    private createFrame_(
      ...
      const audioChannelConfigs = XmlUtils.findChildren(
        elem,
      ...
      return {
        ...
        height:
          XmlUtils.parseAttr(elem, "height", parseNumber) || parent.height,
    ...
    private parseAudioChannels_(audioChannelConfigs: Element[]): number | null {
      for (const elem of audioChannelConfigs) {
        ...
        switch (scheme) {
          ...
          default:
            shaka.log.warning(
              "Unrecognized audio channel scheme:",
              scheme,
              value
            );
            continue;
    ...
    private verifyRepresentation_(frame: InheritanceFrame): boolean {
      ...
      if (n == 0) {
        ...
        if (
          frame.contentType == ContentType.TEXT ||
    ...
    private async requestForTiming_(
      ...
      const requestUris = shaka.util.ManifestParserUtils.resolveUris(baseUris, [
        uri,
      ]);
      const request = shaka.net.NetworkingEngine.makeRequest(
        requestUris,
      ...
      if (method == "HEAD") {
        if (!response.headers || !response.headers["date"]) {
          shaka.log.warning(
            "UTC timing response is missing",
    ...
    private async parseUtcTiming_(
      ...
      for (const sv of schemesAndValues) {
        try {
          ...
          shaka.log.warning(
            "Error fetching time from UTCTiming elem",
            e.message
          );
        }
      }
...
namespace shaka.dash.DashParser {
  /**
