...
export class MpdUtils {
  ...
  /**
     ...
     */ 
  static fillUriTemplate(uriTemplate: string, representationId: string | null, number: number | null, bandwidth: number | null, time: number | null): string {
    const valueTable: {[key:string]:number | null | string | null} = {'RepresentationID':representationId, 'Number':number, 'Bandwidth':bandwidth, 'Time':time};
    const re = /\$(RepresentationID|Number|Bandwidth|Time)?(?:%0([0-9]+)([diouxX]))?\$/g;
     
    // eslint-disable-line max-len 
    const uri = uriTemplate.replace(re,  
    (match, name, widthStr, format) => {
      if (match == '$$') {
        return '$';
      }
      ...
      if (value == null) {
        log.warning('URL template does not have an available substitution for ', 'identifier "' + name + '":', uriTemplate);
        return match;
      }
      if (name == 'RepresentationID' && widthStr) {
        log.warning('URL template should not contain a width specifier for identifier', '"RepresentationID":', uriTemplate);
        widthStr = undefined;
      }
      if (name == 'Time') {
        asserts.assert(typeof value == 'number', 'Time value should be a number!');
        asserts.assert(Math.abs(value - Math.round(value)) < 0.2, 'Calculated $Time$ values must be close to integers');
  ...
  /**
     ...
     */ 
  static createTimeline(segmentTimeline: Element, timescale: number, unscaledPresentationTimeOffset: number, periodDuration: number): TimeRange[] {
    ...
    for (let i = 0; i < timePoints.length; ++i) {
      ...
      if (repeat < 0) {
        if (next) {
          ...
          if (nextStartTime == null) {
            log.warning('An "S" element cannot have a negative repeat', 'if the next "S" element does not have a valid start time:', 'ignoring the remaining "S" elements.', timePoint);
            return timeline;
          } else {
            if (startTime >= nextStartTime) {
              log.warning('An "S" element cannot have a negative repeatif its start ', 'time exceeds the next "S" element\'s start time:', 'ignoring the remaining "S" elements.', timePoint);
              return timeline;
            }
          }
          ...
          if (periodDuration == Infinity) {
            ...
            // ever. 
            log.warning('The last "S" element cannot have a negative repeat', 'if the Period has an infinite duration:', 'ignoring the last "S" element.', timePoint);
            return timeline;
          } else {
            if (startTime / timescale >= periodDuration) {
              log.warning('The last "S" element cannot have a negative repeat', "if its start time exceeds the Period's duration:", 'igoring the last "S" element.', timePoint);
              return timeline;
            }
          }
      ...
      if (timeline.length > 0 && startTime != lastEndTime) {
        ...
        if (Math.abs(delta / timescale) >= ManifestParserUtilsExports.GAP_OVERLAP_TOLERANCE_SECONDS) {
      ...
      for (let j = 0; j <= repeat; ++j) {
        ...
        const item = {start:startTime / timescale, end:endTime / timescale, unscaledStart:startTime};
        timeline.push(item);
        startTime = endTime;
        lastEndTime = endTime;
      }
  ...
  /**
     ...
     */ 
  static parseSegmentInfo(context: DashParserExports.Context, callback: (p1: DashParserExports.InheritanceFrame | null) => Element): SegmentInfo {
    ...
    return {timescale:timescale, segmentDuration:segmentDuration, startNumber:startNumber, scaledPresentationTimeOffset:scaledPresentationTimeOffset, unscaledPresentationTimeOffset:unscaledPresentationTimeOffset, timeline:timeline};
  }
  ...
  /**
     ...
     */ 
  static inheritAttribute(context: DashParserExports.Context, callback: (p1: DashParserExports.InheritanceFrame | null) => Element, attribute: string): string | null {
    const Functional = Functional;
    asserts.assert(callback(context.representation), 'There must be at least one element of the given type');
    const nodes: Element[] = [callback(context.representation), callback(context.adaptationSet), callback(context.period)].filter(Functional.isNotNull);
    return nodes.map( 
    (s) => {
  ...
  /**
     ...
     */ 
  static inheritChild(context: DashParserExports.Context, callback: (p1: DashParserExports.InheritanceFrame | null) => Element, child: string): Element {
    const Functional = Functional;
    asserts.assert(callback(context.representation), 'There must be at least one element of the given type');
    const nodes: Element[] = [callback(context.representation), callback(context.adaptationSet), callback(context.period)].filter(Functional.isNotNull);
    const XmlUtils = XmlUtils;
  ...
  /**
     ...
     *
     */ 
  private static handleXlinkInElement_(element: Element, retryParameters: shaka.extern.RetryParameters, failGracefully: boolean, baseUri: string, networkingEngine: NetworkingEngine, linkDepth: number): AbortableOperation<Element> {
    const MpdUtils = MpdUtils;
    const XmlUtils = XmlUtils;
    const Error = Error;
    ...
    const xlinkActuate = XmlUtils.getAttributeNS(element, NS, 'actuate') || 'onRequest';
    ...
    if (linkDepth >= 5) {
      return AbortableOperation.failed(new Error(Error.Severity.CRITICAL, Error.Category.MANIFEST, Error.Code.DASH_XLINK_DEPTH_LIMIT));
    }
    if (xlinkActuate != 'onLoad') {
       
      // Only xlink:actuate="onLoad" is supported.
      // When no value is specified, the assumed value is "onRequest". 
      return AbortableOperation.failed(new Error(Error.Severity.CRITICAL, Error.Category.MANIFEST, Error.Code.DASH_UNSUPPORTED_XLINK_ACTUATE));
    }
    ...
    const networkOperation = (requestOperation as AbortableOperation<shaka.extern.Response>);
    ...
    (response) => {
      ...
      if (!rootElem) {
        ...
        return AbortableOperation.failed(new Error(Error.Severity.CRITICAL, Error.Category.MANIFEST, Error.Code.DASH_INVALID_XML, xlinkHref));
      }
  ...
  /**
     ...
     */ 
  static processXlinks(element: Element, retryParameters: shaka.extern.RetryParameters, failGracefully: boolean, baseUri: string, networkingEngine: NetworkingEngine, linkDepth: number = 0): AbortableOperation<Element> {
    const MpdUtils = MpdUtils;
    const XmlUtils = XmlUtils;
    const NS = MpdUtils.XlinkNamespaceUri_;
    if (XmlUtils.getAttributeNS(element, NS, 'href')) {
      ...
      if (failGracefully) {
        ...
        handled = handled.chain(undefined,  
        (error) => {
      ...
      if (child instanceof Element) {
        ...
        if (XmlUtils.getAttributeNS(child, NS, 'href') == resolveToZeroString) {
          ...
          if (child.tagName != 'SegmentTimeline') {
            ...
            childOperations.push(MpdUtils.processXlinks((child as Element), retryParameters, failGracefully, baseUri, networkingEngine, linkDepth));
          }
        }
      }
    }
...
type TimeRange = {start:number, unscaledStart:number, end:number};
 
export{TimeRange};
type SegmentInfo = {timescale:number, segmentDuration:number | null, startNumber:number, scaledPresentationTimeOffset:number, unscaledPresentationTimeOffset:number, timeline:TimeRange[]};
