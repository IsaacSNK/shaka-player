  ...
  export class MpdUtils {
    ...
    static createTimeline(
      ...
      for (let i = 0; i < timePoints.length; ++i) {
        ...
        let t = XmlUtils.parseAttr(
          timePoint,
          "t",
          XmlUtils.parseNonNegativeInt
        );
        const d = XmlUtils.parseAttr(
          timePoint,
          "d",
          XmlUtils.parseNonNegativeInt
        );
    ...
    static parseSegmentInfo(
      ...
      const durationStr = MpdUtils.inheritAttribute(
        context,
        callback,
        "duration"
      );
      ...
      const startNumberStr = MpdUtils.inheritAttribute(
        context,
        callback,
        "startNumber"
      );
    ...
    private static handleXlinkInElement_(
      ...
      return networkOperation.chain((response) => {
        ...
        // rejected.
        const rootElem = shaka.util.XmlUtils.parseXml(
          response.data,
          element.tagName
        );
        if (!rootElem) {
    ...
    static processXlinks(
      ...
      for (const child of Array.from(element.childNodes)) {
        if (child instanceof Element) {
          ...
          if (
            XmlUtils.getAttributeNS(child, NS, "href") == resolveToZeroString
          ) {
