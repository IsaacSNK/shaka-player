...
namespace shaka.cea {
  ...
  export class CeaDecoder implements ICaptionDecoder {
     ...
     */
    private cea608DataArray_: Cea608Packet[] = [];

    /**
     * An array of CEA-708 closed caption data extracted for decoding.
     */
    private cea708DataArray_: Cea708Byte[] = [];
    ...
    constructor() {
      ...
      this.cea608ModeToStream_ = new Map([
        ["CC1", new shaka.cea.Cea608DataChannel(0, 0)],
        // F1 + C1 -> CC1
        ["CC2", new shaka.cea.Cea608DataChannel(0, 1)],
        // F1 + C2 -> CC2
        ["CC3", new shaka.cea.Cea608DataChannel(1, 0)],
        // F2 + C1 -> CC3
        ["CC4", new shaka.cea.Cea608DataChannel(1, 1)],
      ]);
    ...
    extract(userDataSeiMessage, pts) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (ccValid) {
          ...
          if (
            ccType === shaka.cea.CeaDecoder.NTSC_CC_FIELD_1 ||
     ...
     */
    decode() {
      const parsedClosedCaptions: ClosedCaption[] = [];
      ...
      const stableComparator = (p1, p2) =>
        p1.pts - p2.pts || p1.order - p2.order;
     ...
     */
    private decodeCea608_(ccPacket: Cea608Packet): ClosedCaption | null {
      ...
      const selectedChannel = fieldNum
        ? this.currentField2Channel_
        : this.currentField1Channel_;
      ...
      if (
        (ccPacket.ccData1 === 255 && ccPacket.ccData2 === 255) ||
     ...
     */
    private decodeCea708_(dtvccPacket: DtvccPacket): ClosedCaption[] {
        ...
        while (dtvccPacket.hasMoreData()) {
          ...
          if (serviceNumber != 0) {
            ...
            while (dtvccPacket.getPosition() - startPos < blockSize) {
              const closedCaption =
                service.handleCea708ControlCode(dtvccPacket);
              if (closedCaption) {
...
namespace shaka.cea.CeaDecoder {
  /**
