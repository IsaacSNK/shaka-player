...
import{DtvccPacketBuilder}from './dtvcc_packet_builder';
...
export class CeaDecoder implements ICaptionDecoder {
  ...
  constructor() {
    ...
    this.cea608ModeToStream_ = new Map([['CC1', new Cea608DataChannel(0, 0)],  
    // F1 + C1 -> CC1 
  ...
  extract(userDataSeiMessage, pts) {
    const reader = new DataViewReader(userDataSeiMessage, DataViewReaderExports.Endianness.BIG_ENDIAN);
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (ccValid) {
        ...
        if (ccType === NTSC_CC_FIELD_1 || ccType === NTSC_CC_FIELD_2) {
          ...
          this.cea608DataArray_.push({pts, type:ccType, ccData1, ccData2, order:this.cea608DataArray_.length});
        } else {
           
          // CEA-708 DTVCC Data. 
          this.cea708DataArray_.push({pts, type:ccType, value:ccData1, order:this.cea708DataArray_.length});
          ...
          this.cea708DataArray_.push({pts, type:DtvccPacketBuilderExports.DTVCC_PACKET_DATA, value:ccData2, order:this.cea708DataArray_.length});
        }
      }
    }
  }
  ...
  /**
     ...
     */ 
  private decodeCea608_(ccPacket: Cea608DataChannelExports.Cea608Packet): ICaptionDecoderExports.ClosedCaption | null {
    ...
    const selectedChannel = fieldNum ? this.currentField2Channel_ : this.currentField1Channel_;
    ...
    if (ccPacket.ccData1 === 255 && ccPacket.ccData2 === 255 || !ccPacket.ccData1 && !ccPacket.ccData2 || !this.isOddParity_(ccPacket.ccData1) || !this.isOddParity_(ccPacket.ccData2)) {
  ...
  /**
     ...
     */ 
  private decodeCea708_(dtvccPacket: DtvccPacket): ICaptionDecoderExports.ClosedCaption[] {
    ...
    try {
      ...
      if (error instanceof Error && error.code === ErrorExports.Code.BUFFER_READ_OUT_OF_BOUNDS) {
        log.warnOnce('CEA708_INVALID_DATA', 'Buffer read out of bounds / invalid CEA-708 Data.');
      } else {
