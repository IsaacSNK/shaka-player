...
import{StyledChar}from './cea_utils';
...
export class Cea708Window {
  ...
  constructor(windowNum: number) {
    ...
    this.justification_ = TextJustification.CENTER;
    ...
    Functional.ignored(this.verticalAnchor_, this.relativeToggle_, this.horizontalAnchor_, this.anchorId_, this.windowNum_);
  }
   
  defineWindow(visible: boolean, verticalAnchor: number, horizontalAnchor: number, anchorId: number, relativeToggle: boolean, rowCount: number, colCount: number) {
  ...
  setCharacter(char: string) {
    ...
    const cea708Char = new StyledChar(char, this.underline_, this.italics_, this.backgroundColor_, this.textColor_);
  ...
  private moveUpRows_(count: number) {
    ...
    for (let i = count; i < MAX_ROWS; i++, dst++) {
  ...
  /**
     ...
     */ 
  forceEmit(endTime: number, serviceNumber: number): ICaptionDecoderExports.ClosedCaption | null {
    const stream = `svc${serviceNumber}`;
    const TextJustification = TextJustification;
    const topLevelCue = new Cue(this.startTime_, endTime,  
    /* payload= */ 
    '');
    if (this.justification_ === TextJustification.LEFT) {
       
      // LEFT justified. 
      topLevelCue.textAlign = CueExports.textAlign.LEFT;
    } else {
      if (this.justification_ === TextJustification.RIGHT) {
         
        // RIGHT justified. 
        topLevelCue.textAlign = CueExports.textAlign.RIGHT;
      } else {
         
        // CENTER justified. Both FULL and CENTER are handled as CENTER justified. 
        topLevelCue.textAlign = CueExports.textAlign.CENTER;
      }
    }
    const caption = CeaUtils.getParsedCaption(topLevelCue, stream, this.memory_, this.startTime_, endTime);
    if (caption) {
