...
import {CeaUtils, StyledChar} from './cea___cea_utils';
...
export class Cea708Window {
  ...
  constructor(windowNum: number) {
    ...
    Functional.ignored(
        this.verticalAnchor_, this.relativeToggle_, this.horizontalAnchor_,
        this.anchorId_, this.windowNum_);
  }

  defineWindow(
      visible: boolean, verticalAnchor: number, horizontalAnchor: number,
      anchorId: number, relativeToggle: boolean, rowCount: number,
      colCount: number) {
  ...
  setCharacter(char: string) {
    ...
    const cea708Char = new StyledChar(
  ...
  forceEmit(endTime: number, serviceNumber: number):
    ...
    if (this.justification_ === TextJustification.LEFT) {
      ...
      if (this.justification_ === TextJustification.RIGHT) {
        ...
        // CENTER justified. Both FULL and CENTER are handled as CENTER
        // justified.
