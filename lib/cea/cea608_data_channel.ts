...
export class Cea608DataChannel {
  ...
  private controlMidrow_(b2: number) {
    ...
    this.curbuf_.addChar(
        Cea608MemoryExports.CharSet.BASIC_NORTH_AMERICAN, ' '.charCodeAt(0));
    let textColor = CeaUtilsExports.DEFAULT_TXT_COLOR;
    let italics = false;
   ...
   */
  private controlMiscellaneous_(ccPacket: Cea608Packet):
   ...
   */
  private controlRu_(scrollSize: number, pts: number):
      ICaptionDecoderExports.ClosedCaption|null {
    this.curbuf_ = this.displayedMemory_;
  ...
  private controlRtd_() {
    log.warnOnce(
        'Cea608DataChannel', 'CEA-608 text mode entered, but is unsupported');
  ...
  private handleExtendedWesternEuropeanChar_(b1: number, b2: number) {
    ...
    const charSet = b1 & 1 ? Cea608MemoryExports.CharSet.PORTUGUESE_GERMAN :
                             Cea608MemoryExports.CharSet.SPANISH_FRENCH;
   ...
   */
  handleControlCode(ccPacket: Cea608Packet):
      ICaptionDecoderExports.ClosedCaption|null {
      ...
      if (this.isMidrowStyleChange_(b1, b2)) {
        ...
        if (this.isBackgroundAttribute_(b1, b2)) {
          ...
          if (this.isSpecialNorthAmericanChar_(b1, b2)) {
            this.curbuf_.addChar(
                Cea608MemoryExports.CharSet.SPECIAL_NORTH_AMERICAN, b2);
          } else {
...
export const BG_COLORS: string[] =
    ['black', 'green', 'blue', 'cyan', 'red', 'yellow', 'magenta', 'black'];

export const TEXT_COLORS: string[] = [
  'white', 'green', 'blue', 'cyan', 'red', 'yellow', 'magenta', 'white_italics'
];
type Style = {
