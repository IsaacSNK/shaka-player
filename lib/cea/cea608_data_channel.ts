...
export class Cea608DataChannel {
  ...
  constructor(fieldNum: number, channelNum: number) {
    this.type_ = CaptionType.NONE;
    this.text_ = new Cea608Memory(fieldNum, channelNum);
    this.displayedMemory_ = new Cea608Memory(fieldNum, channelNum);
    this.nonDisplayedMemory_ = new Cea608Memory(fieldNum, channelNum);
  ...
  /**
     ...
     */ 
  reset() {
    this.type_ = CaptionType.PAINTON;
    this.curbuf_ = this.displayedMemory_;
    this.lastcp_ = null;
  ...
  private controlPac_(b1: number, b2: number) {
    ...
    // Set up the defaults. 
    let textColor = CeaUtilsExports.DEFAULT_TXT_COLOR;
    let italics = false;
     
    // Attributes < 7 are colors, = 7 is white w/ italics, and >7 are indents 
    if (attr < 7) {
      textColor = TEXT_COLORS[attr];
    } else {
      if (attr === 7) {
        italics = true;
      }
    ...
    if (this.type_ === CaptionType.ROLLUP && row !== buf.getRow()) {
      ...
      buf.resetRows(row + 1, Cea608MemoryExports.CC_ROWS - row);
    }
    buf.setRow(row);
    ...
    this.curbuf_.setBackgroundColor(CeaUtilsExports.DEFAULT_BG_COLOR);
  }
  ...
  private controlMidrow_(b2: number) {
    ...
    this.curbuf_.setTextColor(CeaUtilsExports.DEFAULT_TXT_COLOR);
     
    // Mid-row attrs use a space. 
    this.curbuf_.addChar(Cea608MemoryExports.CharSet.BASIC_NORTH_AMERICAN, ' '.charCodeAt(0));
    let textColor = CeaUtilsExports.DEFAULT_TXT_COLOR;
    let italics = false;
  ...
  private controlBackgroundAttribute_(b1: number, b2: number) {
    let backgroundColor = CeaUtilsExports.DEFAULT_BG_COLOR;
    if ((b1 & 7) === 0) {
       
      // If background provided, last 3 bits of b1 are |0|0|0|. Color is in b2. 
      backgroundColor = BG_COLORS[(b2 & 14) >> 1];
    }
  ...
  /**
     ...
     */ 
  private controlMiscellaneous_(ccPacket: Cea608Packet): ICaptionDecoderExports.ClosedCaption | null {
  ...
  /**
     ...
     */ 
  private controlCr_(pts: number): ICaptionDecoderExports.ClosedCaption | null {
    const buf = this.curbuf_;
     
    // Only rollup and text mode is affected, but we don't emit text mode. 
    if (this.type_ !== CaptionType.ROLLUP) {
      return null;
    }
    ...
    buf.resetRows(buf.getRow(), Cea608MemoryExports.CC_ROWS - buf.getRow());
  ...
  /**
     ...
     */ 
  private controlRu_(scrollSize: number, pts: number): ICaptionDecoderExports.ClosedCaption | null {
    this.curbuf_ = this.displayedMemory_;
    ...
    if (this.type_ !== CaptionType.ROLLUP && this.type_ !== CaptionType.TEXT) {
      ...
      buf.setRow(Cea608MemoryExports.CC_ROWS);
    }
    this.type_ = CaptionType.ROLLUP;
  ...
  private controlEoc_(pts: number): ICaptionDecoderExports.ClosedCaption | null {
    let parsedClosedCaption = null;
    if (this.type_ !== CaptionType.TEXT) {
      parsedClosedCaption = this.displayedMemory_.forceEmit(this.prevEndTime_, pts);
    }
  ...
  /**
     ...
     */ 
  private controlRtd_() {
    log.warnOnce('Cea608DataChannel', 'CEA-608 text mode entered, but is unsupported');
    this.curbuf_ = this.text_;
    this.type_ = CaptionType.TEXT;
  }
  ...
  handleBasicNorthAmericanChar(b1: number, b2: number) {
    this.curbuf_.addChar(Cea608MemoryExports.CharSet.BASIC_NORTH_AMERICAN, b1);
    this.curbuf_.addChar(Cea608MemoryExports.CharSet.BASIC_NORTH_AMERICAN, b2);
  }
  ...
  private handleExtendedWesternEuropeanChar_(b1: number, b2: number) {
    ...
    const charSet = b1 & 1 ? Cea608MemoryExports.CharSet.PORTUGUESE_GERMAN : Cea608MemoryExports.CharSet.SPANISH_FRENCH;
  ...
  /**
     ...
     */ 
  handleControlCode(ccPacket: Cea608Packet): ICaptionDecoderExports.ClosedCaption | null {
      ...
      if (this.isMidrowStyleChange_(b1, b2)) {
        ...
        if (this.isBackgroundAttribute_(b1, b2)) {
          ...
          if (this.isSpecialNorthAmericanChar_(b1, b2)) {
            this.curbuf_.addChar(Cea608MemoryExports.CharSet.SPECIAL_NORTH_AMERICAN, b2);
          } else {
  ...
  private isBackgroundAttribute_(b1: number, b2: number): boolean {
    ...
    return (b1 & 247) === 16 && (b2 & 240) === 32 || (b1 & 247) === 23 && (b2 & 255) === 45;
  }
...
export const BG_COLORS: string[] = ['black', 'green', 'blue', 'cyan', 'red', 'yellow', 'magenta', 'black'];
 
export const TEXT_COLORS: string[] = ['white', 'green', 'blue', 'cyan', 'red', 'yellow', 'magenta', 'white_italics'];
type Style = {textColor:string | null, backgroundColor:string | null, italics:boolean | null, underline:boolean | null};
 
export{Style};
type Cea608Packet = {pts:number, type:number, ccData1:number, ccData2:number, order:number};
