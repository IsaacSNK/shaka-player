...
export class Mp4CeaParser implements ICeaParser {
  ...
  init(initSegment) {
    ...
    const trackIds = [];
    const timescales = [];
    (new Mp4Parser()).box('moov', Mp4Parser.children).box('mvex', Mp4Parser.children).fullBox('trex',  
    (box) => {
    ...
    }).box('trak', Mp4Parser.children).fullBox('tkhd',  
    (box) => {
      asserts.assert(box.version != null, 'TKHD is a full box and should have a valid version.');
      const parsedTKHDBox = Mp4BoxParsers.parseTKHD(box.reader, box.version);
      trackIds.push(parsedTKHDBox.trackId);
    }).box('mdia', Mp4Parser.children).fullBox('mdhd',  
    (box) => {
    ...
    if (!trackIds.length || !timescales.length || trackIds.length != timescales.length) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_MP4_CEA);
    }
  ...
  parse(mediaSegment) {
    ...
    (new Mp4Parser()).box('moof', Mp4Parser.children).box('traf', Mp4Parser.children).fullBox('trun',  
    (box) => {
      asserts.assert(box.version != null && box.flags != null, 'TRUN is a full box and should have a valid version & flags.');
      const parsedTRUN = Mp4BoxParsers.parseTRUN(box.reader, box.version, box.flags);
      sampleData = parsedTRUN.sampleData;
    }).fullBox('tfhd',  
    (box) => {
      asserts.assert(box.flags != null, 'TFHD is a full box and should have valid flags.');
      ...
      defaultSampleDuration = parsedTFHD.defaultSampleDuration || this.defaultSampleDuration_;
      defaultSampleSize = parsedTFHD.defaultSampleSize || this.defaultSampleSize_;
    ...
    (box) => {
      asserts.assert(box.version != null, 'TFDT is a full box and should have a valid version.');
      const parsedTFDT = Mp4BoxParsers.parseTFDT(box.reader, box.version);
    ...
    (box) => {
      ...
      this.parseMdat_(box.reader, baseMediaDecodeTime, timescale, defaultSampleDuration, defaultSampleSize, sampleData, captionPackets);
    }).parse(mediaSegment,  
    /* partialOkay= */ 
    false);
  ...
  /**
     * Parse MDAT box.
     */ 
  private parseMdat_(reader: DataViewReader, time: number, timescale: number, defaultSampleDuration: number, defaultSampleSize: number, sampleData: ParsedTRUNSample[], captionPackets: ICeaParserExports.CaptionPacket[]) {
    let sampleIndex = 0;
    ...
    while (reader.hasMoreData()) {
      ...
      if (naluType == ICeaParserExports.NALU_TYPE_SEI) {
        ...
        for (const packet of this.seiProcessor_.process(reader.readBytes(naluSize - 1))) {
      ...
      if (sampleSize == 0) {
        if (sampleData.length > sampleIndex) {
          time += sampleData[sampleIndex].sampleDuration || defaultSampleDuration;
        } else {
