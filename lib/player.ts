...
namespace shaka {
    ...
    implements IDestroyable
  {
    ...
    constructor(
      ...
      const walkerImplementation: Implementation = {
        ...
        enterNode: (node, has, wants) => {
          this.dispatchEvent(
            this.makeEvent_(
              shaka.util.FakeEvent.EventName.OnStateChange,
              /* name= */
              /* data= */
        ...
        onIdle: (node) => {
          this.dispatchEvent(
            this.makeEvent_(
              shaka.util.FakeEvent.EventName.OnStateIdle,
              /* name= */
              /* data= */
              new Map().set("state", node.name)
            )
      ...
      this.checkVariantsTimer_ = new shaka.util.Timer(() =>
        this.checkVariants_()
      );
    ...
    private makeEvent_(name: EventName, data?: Map<string, Object>): FakeEvent {
    ...
    static isBrowserSupported(): boolean {
      if (!window.Promise) {
        shaka.log.alwaysWarn(
          "A Promise implementation or polyfill is required"
        );
      }
    ...
    attach(
      mediaElement: HTMLMediaElement,
      initializeMediaSource: boolean = true
    ): Promise {
    ...
    unload(initializeMediaSource: boolean = true): Promise {
      ...
      const events = this.walker_.startNewRoute((currentPayload) => {
        ...
        goog.asserts.assert(
          destination,
          "We should have picked a destination."
        );
    ...
    load(
      ...
      this.dispatchEvent(
        this.makeEvent_(shaka.util.FakeEvent.EventName.Loading)
      );
    ...
    private shouldUseSrcEquals_(payload: Payload): boolean {
      ...
      if (mimeType) {
        ...
        const canPlayMediaSource = shaka.media.ManifestParser.isSupported(
          uri,
          mimeType
        );
    ...
    private async onUnload_(has: Payload, wants: Payload): Promise {
      ...
      if (has.mediaElement && has.mediaElement.src) {
        ...
        // otherwise.
        await new Promise((resolve) =>
          new shaka.util.Timer(resolve).tickAfter(0.1)
        );
     ...
     *
     */
    private async onInitializeMediaSourceEngine_(
      has: Payload,
      wants: Payload
    ): Promise {
      goog.asserts.assert(
    ...
    private async onInitializeParser_(has: Payload, wants: Payload): Promise {
      ...
      const manifestConfig = shaka.util.ObjectUtils.cloneObject(
        this.config_.manifest
      );
      ...
      return new shaka.util.AbortableOperation(
        ...
        (async () => {
          ...
          const event = this.makeEvent_(
            shaka.util.FakeEvent.EventName.ManifestParsed
          );
    ...
    private async onInitializeDrm_(has: Payload, wants: Payload): Promise {
      ...
      this.drmEngine_ = this.createDrmEngine({
        ...
        onEvent: (e) => {
          ...
          if (
            e.type == shaka.util.FakeEvent.EventName.DrmSessionUpdate &&
            firstEvent
          ) {
            firstEvent = false;
            const now = Date.now() / 1000;
    ...
    private async onLoad_(has: Payload, wants: Payload) {
      ...
      if (!activeVariantTrack) {
        ...
        if (this.config_.streaming.startAtSegmentBoundary) {
          ...
          const adjustedTime = await this.adjustStartTime_(
            initialVariant,
            startTime
          );
     ...
     *
     */
    private async onInitializeSrcEqualsDrm_(
      has: Payload,
      wants: Payload
    ): Promise {
      ...
      const variant: shaka.extern.Variant = {
        ...
        video: {
          ...
          mimeType: wants.mimeType
            ? shaka.util.MimeUtils.getBasicType(wants.mimeType)
            : mimeType,
     ...
     *
     */
    createDrmEngine(playerInterface: PlayerInterface): DrmEngine {
    ...
    createNetworkingEngine(): NetworkingEngine {
      ...
      const onHeadersReceived_: OnHeadersReceived = (
        headers,
        request,
        requestType
      ) => {
      ...
      const onDownloadFailed_: OnDownloadFailed = (
        request,
        error,
        httpResponseCode,
        aborted
    ...
    private createPlayheadObserversForMSE_(): PlayheadObserverManager {
      ...
      regionObserver.addEventListener("enter", (event) => {
        const region: shaka.extern.TimelineRegionInfo = event["region"];
        this.onRegionEvent_(
          shaka.util.FakeEvent.EventName.TimelineRegionEnter,
          region
        );
      });
      regionObserver.addEventListener("exit", (event) => {
        const region: shaka.extern.TimelineRegionInfo = event["region"];
        this.onRegionEvent_(
          shaka.util.FakeEvent.EventName.TimelineRegionExit,
          region
        );
      });
    ...
    private startBufferManagement_(rebufferingGoal: number) {
      ...
      // the same state.
      this.bufferObserver_.setState(
        shaka.media.BufferingObserver.State.STARVING
      );
    ...
    private updateBufferingSettings_(rebufferingGoal: number) {
      ...
      const satisfiedThreshold = Math.min(
        shaka.Player.TYPICAL_BUFFERING_THRESHOLD_,
        rebufferingGoal / 2
      );
    ...
    private pollBufferState_() {
      ...
      const stateChanged = this.bufferObserver_.update(
        bufferLead,
        bufferedToEnd
      );
      ...
      if (this.streamingEngine_) {
        ...
        if (activeVariant) {
          if (
            ...
            shaka.log.debug(
              "Choosing new variant after changing configuration"
            );
    ...
    getSharedConfiguration(): shaka.extern.PlayerConfiguration {
      goog.asserts.assert(
        this.config_,
        "Cannot call getSharedConfiguration after call destroy!"
      );
      return this.config_;
    }
    ...
    getBufferFullness(): number {
      if (this.video_) {
        ...
        const lengthToBeBuffered = Math.min(
          this.video_.currentTime + bufferingGoal,
          this.seekRange().end
        );
        if (bufferedEnd >= lengthToBeBuffered) {
          ...
          if (bufferedEnd <= this.video_.currentTime) {
            ...
            if (bufferedEnd < lengthToBeBuffered) {
              return (
                (bufferedEnd - this.video_.currentTime) /
    ...
    trickPlay(rate: number) {
      ...
      goog.asserts.assert(
        rate != 0,
        "Should never set a trick play rate of 0!"
      );
      if (rate == 0) {
    ...
    getTextTracks(): shaka.extern.Track[] {
      if (this.manifest_) {
        ...
        if (this.video_ && this.video_.src && this.video_.textTracks) {
          ...
          return textTracks.map((text) =>
            StreamUtils.html5TextTrackToTrack(text)
          );
        } else {
          return [];
        }
    ...
    getImageTracks(): shaka.extern.Track[] {
      if (this.manifest_) {
        ...
        const StreamUtils = shaka.util.StreamUtils;
        return imageStreams.map((image) =>
          StreamUtils.imageStreamToTrack(image)
        );
      } else {
        return [];
      }
    ...
    async getThumbnails(
      ...
      if (this.manifest_) {
        ...
        // positionY = 0
        if (totalImages > 1) {
          const thumbnailPosition = Math.floor(
    ...
    selectVariantTrack(
      ...
      if (this.manifest_ && this.streamingEngine_) {
        ...
        this.currentAdaptationSetCriteria_ =
          new shaka.media.ExampleBasedCriteria(variant);
    ...
    getAudioLanguages(): string[] {
      return Array.from(
        shaka.Player.getLanguagesFrom_(this.getVariantTracks())
      );
    }
    ...
    selectAudioLanguage(language: string, role?: string) {
      ...
      if (this.manifest_ && this.playhead_) {
        ...
        const diff = (a, b) => {
          if (!a.video && !b.video) {
            ...
            if (!a.video || !b.video) {
              ...
              return (
                Math.abs((a.video.height || 0) - (b.video.height || 0)) +
        ...
        const set = this.currentAdaptationSetCriteria_.create(
          this.manifest_.variants
        );
        ...
        if (this.video_ && this.video_.audioTracks) {
          ...
          for (const audioTrack of audioTracks) {
            const track =
              shaka.util.StreamUtils.html5AudioTrackToTrack(audioTrack);
    ...
    selectTextLanguage(
      ...
      if (this.manifest_ && this.playhead_) {
        ...
        const track = this.getTextTracks().find((t) => {
          return (
            LanguageUtils.normalize(t.language) == selectedLanguage &&
    ...
    selectVariantsByLabel(label: string) {
      if (this.manifest_ && this.playhead_) {
        ...
        if (firstVariantWithLabel == null) {
          shaka.log.warning(
            "No variants were found with label: " +
              label +
    ...
    isTextTrackVisible(): boolean {
      ...
      if (this.mediaSourceEngine_) {
        ...
        const actual = this.mediaSourceEngine_
          .getTextDisplayer()
          .isTextVisible();
        goog.asserts.assert(
          actual == expected,
          "text visibility has fallen out of sync"
        );
    ...
    private getFilteredTextTracks_(): TextTrack[] {
      ...
      return Array.from(this.video_.textTracks).filter(
        (t) =>
          t.kind != "metadata" &&
          t.kind != "chapters" &&
    ...
    getPresentationStartTimeAsDate(): Date {
      if (!this.isLive()) {
        shaka.log.warning(
          "getPresentationStartTimeAsDate is for live streams!"
        );
        return null;
      }
      if (this.manifest_) {
    ...
    getStats(): shaka.extern.Stats {
      ...
      goog.asserts.assert(
        this.video_,
        "If we have stats, we should have video_"
      );
      ...
      const licenseSeconds = this.drmEngine_
        ? this.drmEngine_.getLicenseTime()
        : NaN;
      ...
      if (this.loadMode_ == shaka.Player.LoadMode.MEDIA_SOURCE) {
        ...
        if (this.isLive()) {
          const now =
            this.getPresentationStartTimeAsDate().valueOf() +
    ...
    async addTextTrackAsync(
      ...
      if (this.loadMode_ == shaka.Player.LoadMode.SRC_EQUALS) {
        ...
        const srcTrack = textTracks.find((t) => {
          return (
            t.language == language && t.label == (label || "") && t.kind == kind
          );
        });
        if (srcTrack) {
          this.onTracksChanged_();
    ...
    private makeTextStreamsForClosedCaptions_(manifest: shaka.extern.Manifest) {
      ...
      for (const textStream of manifest.textStreams) {
        if (
          textStream.mimeType == CEA608_MIME ||
     ...
     *
     */
    private async filterManifestWithStreamUtils_(
     ...
     *
     */
    private filterManifestWithRestrictions_(
      manifest: shaka.extern.Manifest | null
    ) {
    ...
    private switchHtml5Track_(track: AudioTrack) {
      ...
      const newTrack = shaka.util.StreamUtils.html5AudioTrackToTrack(track);
      this.onVariantChanged_(oldTrack, newTrack);
    }
    ...
    private setInitialTextState_(initialVariant, initialTextStream) {
      ...
      if (initialTextStream) {
        if (
          initialVariant.audio &&
          this.shouldInitiallyShowText_(initialVariant.audio, initialTextStream)
        ) {
    ...
    private shouldInitiallyShowText_(
      ...
      const LanguageUtils = shaka.util.LanguageUtils;
      const preferredTextLocale: string = LanguageUtils.normalize(
        this.config_.preferredTextLanguage
      );
      const textLocale: string = LanguageUtils.normalize(textStream.language);
      if (
        this.config_.autoShowText == AutoShowText.IF_PREFERRED_TEXT_LANGUAGE
      ) {
      ...
      if (
        this.config_.autoShowText == AutoShowText.IF_SUBTITLES_MAY_BE_NEEDED
      ) {
         ...
         *
         */
        const audioLocale: string = LanguageUtils.normalize(
          audioStream.language
        );
        return (
          LanguageUtils.areLanguageCompatible(
            textLocale,
            preferredTextLocale
    ...
    private onSegmentAppended_(start, end, contentType) {
      ...
      this.dispatchEvent(
        this.makeEvent_(shaka.util.FakeEvent.EventName.SegmentAppended, data)
      );
    }
    ...
    private switch_(
      ...
      goog.asserts.assert(
        this.manifest_,
        "We need a manifest to switch " + "variants."
      );
      if (!this.streamingEngine_) {
    ...
    private onAdaptation_(
      ...
      const event = this.makeEvent_(
        shaka.util.FakeEvent.EventName.Adaptation,
        data
      );
    ...
    private onVariantChanged_(
      ...
      const data = new Map().set("oldTrack", from).set("newTrack", to);
      const event = this.makeEvent_(
        shaka.util.FakeEvent.EventName.VariantChanged,
        data
      );
      this.delayDispatchEvent_(event);
    }
    ...
    private onAbrStatusChanged_() {
      ...
      this.delayDispatchEvent_(
        this.makeEvent_(shaka.util.FakeEvent.EventName.AbrStatusChanged, data)
      );
    }
    ...
    private shouldStreamText_(): boolean {
      return (
        this.config_.streaming.alwaysStreamText || this.isTextTrackVisible()
      );
    }
    ...
    private checkRestrictedVariants_(manifest: shaka.extern.Manifest) {
      const restrictedStatuses = shaka.Player.restrictedStatuses_;
      const keyStatusMap = this.drmEngine_
        ? this.drmEngine_.getKeyStatuses()
        : {};
      const keyIds = Object.keys(keyStatusMap);
     ...
     *
     */
    private static getLanguagesFrom_(
      tracks: (shaka.extern.Track | null)[]
    ): Set<string> {
     ...
     *
     */
    private static getLanguageAndRolesFrom_(
      tracks: (shaka.extern.Track | null)[]
      ...
      languageToRoles.forEach((roles, language) => {
        for (const role of roles) {
          let label = null;
          if (
            languageRoleToLabel.has(language) &&
    ...
    private isBufferedToEndMS_(): boolean {
      ...
      if (this.manifest_.presentationTimeline.isLive()) {
        ...
        const bufferEnd = shaka.media.TimeRangesUtils.bufferEnd(
          this.video_.buffered
        );
    ...
    private getNextStep_(
      ...
      if (currentlyAt == this.srcEqualsDrmNode_) {
        if (
          wantsToBeAt == this.srcEqualsNode_ &&
    ...
    private getNextAfterMediaSource_(
      goingTo: Node,
      has: Payload,
      wants: Payload
    ): Node | null {
     ...
     *
     */
    private wrapWalkerListenersWithPromise_(listeners: Listeners): Promise {
      return new Promise((resolve, reject) => {
