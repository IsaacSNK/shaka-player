...
export class Mp4Parser {
  ...
  parse(data: BufferSource, partialOkay?: boolean, stopOnPartial?: boolean) {
    const reader =
        new DataViewReader(data, DataViewReaderExports.Endianness.BIG_ENDIAN);
    this.done_ = false;
   ...
   */
  parseNext(
      absStart: number, reader: DataViewReader, partialOkay?: boolean,
    ...
    if (boxDefinition) {
      ...
      const payloadReader = new DataViewReader(
          payload, DataViewReaderExports.Endianness.BIG_ENDIAN);
  ...
  static typeToString(type: number): string {
    const name = String.fromCharCode(
        type >> 24 & 255, type >> 16 & 255, type >> 8 & 255, type & 255);
    return name;
  }
