...
export class Mp4Parser {
  ...
  box(type: string, definition: CallbackType): Mp4Parser {
    ...
    this.headers_[typeCode] = BoxType_.BASIC_BOX;
  ...
  fullBox(type: string, definition: CallbackType): Mp4Parser {
    const typeCode = Mp4Parser.typeFromString_(type);
    this.headers_[typeCode] = BoxType_.FULL_BOX;
    this.boxDefinitions_[typeCode] = definition;
    return this;
  }
  ...
  parse(data: BufferSource, partialOkay?: boolean, stopOnPartial?: boolean) {
    const reader = new DataViewReader(data, DataViewReaderExports.Endianness.BIG_ENDIAN);
    this.done_ = false;
  ...
  /**
     ...
     * @export
     */ 
  parseNext(absStart: number, reader: DataViewReader, partialOkay?: boolean, stopOnPartial?: boolean) {
    ...
    if (boxDefinition) {
      let version = null;
      let flags = null;
      if (this.headers_[type] == BoxType_.FULL_BOX) {
      ...
      const payloadReader = new DataViewReader(payload, DataViewReaderExports.Endianness.BIG_ENDIAN);
      const box: shaka.extern.ParsedBox = {parser:this, partialOkay:partialOkay || false, version, flags, reader:payloadReader, size, start:start + absStart, has64BitSize};
      boxDefinition(box);
    } else {
