...
export class FakeEventTarget implements EventTarget, IReleasable {
  ...
  /**
     ...
     * @exportInterface
     */ 
  addEventListener(type: string, listener: ListenerType, options?: AddEventListenerOptions | boolean) {
    if (!this.listeners_) {
      return;
    }
  ...
  /**
     ...
     * @exportInterface
     */ 
  removeEventListener(type: string, listener: ListenerType, options?: EventListenerOptions | boolean) {
    if (!this.listeners_) {
      return;
    }
  ...
  dispatchEvent(event: Event): boolean {
    ...
    for (const listener of listeners) {
      ...
      try {
        ...
        log.error('Uncaught exception in event handler', exception, exception ? exception.message : null, exception ? exception.stack : null);
      }
      if (event.stopped) {
        break;
      }
