...
export class Destroyer {
  ...
  ensureNotDestroyed(error?: any) {
    if (this.destroyed_) {
      if (error instanceof Error && error.code == ErrorExports.Code.OBJECT_DESTROYED) {
        throw error;
      }
  ...
  static destroyedError(error?: any): Error {
    return new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.PLAYER, ErrorExports.Code.OBJECT_DESTROYED, error);
  }
}
