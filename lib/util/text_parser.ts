...
export class TextParser {
  ...
  /**
     ...
     *
     */ 
  private readRegexReturnCapture_(regex: RegExp, index: number): string | null {
    if (this.atEnd()) {
      return null;
    }
  ...
  /**
     ...
     *
     */ 
  private indexOf_(regex: RegExp): {position:number, length:number, results:string[]} | null {
     
    // The global flag is required to use lastIndex. 
    asserts.assert(regex.global, 'global flag should be set');
    regex.lastIndex = this.position_;
    const results = regex.exec(this.data_);
    if (results == null) {
      return null;
    } else {
      return {position:results.index, length:results[0].length, results:results};
    }
  }
}
