...
export class Mp4BoxParsers {
  ...
  static parseTFDT(reader: DataViewReader, version: number): ParsedTFDTBox {
    const baseMediaDecodeTime =
        version == 1 ? reader.readUint64() : reader.readUint32();
   ...
   * Parses a TRUN Box.
   */
  static parseTRUN(reader: DataViewReader, version: number, flags: number):
      ParsedTRUNBox {
    ...
    for (let i = 0; i < sampleCount; i++) {
      ...
      if (flags & 2048) {
        sample.sampleCompositionTimeOffset =
            version == 0 ? reader.readUint32() : reader.readInt32();
      }
