...
export class SwitchHistory {
  ...
  /**
     ...
     *
     */ 
  updateCurrentVariant(newVariant: shaka.extern.Variant, fromAdaptation: boolean) {
    if (this.currentVariant_ == newVariant) {
      return;
    }
    this.currentVariant_ = newVariant;
    this.history_.push({timestamp:this.getNowInSeconds_(), id:newVariant.id, type:'variant', fromAdaptation:fromAdaptation, bandwidth:newVariant.bandwidth});
  }
  ...
  updateCurrentText(newText: shaka.extern.Stream, fromAdaptation: boolean) {
    ...
    this.history_.push({timestamp:this.getNowInSeconds_(), id:newText.id, type:'text', fromAdaptation:fromAdaptation, bandwidth:null});
  }
  ...
  private clone_(entry: shaka.extern.TrackChoice): shaka.extern.TrackChoice {
    return {timestamp:entry.timestamp, id:entry.id, type:entry.type, fromAdaptation:entry.fromAdaptation, bandwidth:entry.bandwidth};
  }
}
