...
export class StringUtils {
  ...
  static fromUTF8(data: BufferSource | null): string {
    ...
    if (window.TextDecoder && !Platform.isPS4()) {
      ...
      if (decoded.includes('\ufffd')) {
        log.alwaysError('Decoded string contains an "unknown character' + '" codepoint.  That probably means the UTF8 ' + 'encoding was incorrect!');
      }
      return decoded;
    } else {
      ...
      for (let i = 0; i < uint8.length; ++i) {
        ...
        if ((uint8[i] & 128) == 0) {
          ...
          // 3-byte encoding. 
          if (uint8.length >= i + 2 && (uint8[i] & 224) == 192 && (uint8[i + 1] & 192) == 128) {
            ...
            // 4-byte encoding. 
            if (uint8.length >= i + 3 && (uint8[i] & 240) == 224 && (uint8[i + 1] & 192) == 128 && (uint8[i + 2] & 192) == 128) {
              codePoint = (uint8[i] & 15) << 12 | (uint8[i + 1] & 63) << 6 | uint8[i + 2] & 63;
              i += 2;
            } else {
              if (uint8.length >= i + 4 && (uint8[i] & 241) == 240 && (uint8[i + 1] & 192) == 128 && (uint8[i + 2] & 192) == 128 && (uint8[i + 3] & 192) == 128) {
                codePoint = (uint8[i] & 7) << 18 | (uint8[i + 1] & 63) << 12 | (uint8[i + 2] & 63) << 6 | uint8[i + 3] & 63;
                i += 3;
              }
            }
          }
  ...
  /**
     ...
     * @export
     */ 
  static fromUTF16(data: BufferSource | null, littleEndian: boolean, noThrow?: boolean): string {
    if (!data) {
      return '';
    }
    if (!noThrow && data.byteLength % 2 != 0) {
      log.error('Data has an incorrect length, must be even.');
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.BAD_ENCODING);
    }
  ...
  static fromBytesAutoDetect(data: BufferSource | null): string {
    ...
    if (uint8[0] == 239 && uint8[1] == 187 && uint8[2] == 191) {
      ...
      if (uint8[0] == 254 && uint8[1] == 255) {
        return StringUtils.fromUTF16(uint8.subarray(2),  
        /* littleEndian= */ 
        false);
      } else {
    ...
    throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.UNABLE_TO_DETECT_ENCODING);
  }
...
export const fromCharCodeImpl_: Lazy<(p1: TypedArray) => string> = new Lazy( 
() => {
   
  /** @param size */ 
  const supportsChunkSize =  
  (size: number) => {
    ...
    if (supportsChunkSize(size)) {
      ...
      (buffer) => {
        ...
        for (let i = 0; i < buffer.length; i += size) {
          ...
          // See issue #2186 for more details.
          // eslint-disable-next-line no-restricted-syntax 
          ret += String.fromCharCode.apply(null,  
          // Issue #2186 
          subArray);
        }
        return ret;
