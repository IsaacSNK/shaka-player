...
namespace shaka.util {
   ...
   * @export
   */
  export class Error implements shaka.extern.Error {
    static createStack: boolean = false;
    ...
    constructor(
      public severity: shaka.util.Error.Severity,
      public category: shaka.util.Error.Category,
      public code: shaka.util.Error.Code,
      ...varArgs
    ) {
      this.data = varArgs;
      ...
      if (goog.DEBUG) {
        ...
        if (shaka.util.Error.createStack) {
          try {
            throw new OriginalError(this.message);
          } catch (e) {
            this.stack = e.stack;
          }
        }
