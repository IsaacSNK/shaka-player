...
namespace shaka.util {
   ...
   * @export
   */
  export class EventManager implements shaka.util.IReleasable {
    /**
    ...
    listen(
      ...
      const binding = new shaka.util.EventManager.Binding_(
        target,
        type,
        listener,
        options
...
namespace shaka.util.EventManager {
  export type ListenerType = (p1: Event) => any;
  ...
  export class Binding_ {
    ...
    constructor(
      ...
      this.options = shaka.util.EventManager.Binding_.convertOptions_(
        target,
        options
      );
