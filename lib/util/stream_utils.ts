...
export class StreamUtils {
  ...
  /**
     ...
     */ 
  static chooseCodecsAndFilterManifest(manifest: shaka.extern.Manifest, preferredVideoCodecs: string[], preferredAudioCodecs: string[], preferredAudioChannelCount: number, preferredDecodingAttributes: string[]) {
    ...
    variants = StreamUtils.filterVariantsByAudioChannelCount(variants, preferredAudioChannelCount);
     
    // Now organize variants into buckets by codecs. 
    let variantsByCodecs: MultiMap<shaka.extern.Variant> = StreamUtils.getVariantsByCodecs_(variants);
    variantsByCodecs = StreamUtils.filterVariantsByDensity_(variantsByCodecs);
    const bestCodecs = StreamUtils.chooseCodecsByDecodingAttributes_(variantsByCodecs, preferredDecodingAttributes);
  ...
  /**
    ...
    *
    */ 
  private static getVariantsByCodecs_(variants: shaka.extern.Variant[]): MultiMap<shaka.extern.Variant> {
    const variantsByCodecs = new MultiMap();
    for (const variant of variants) {
      const variantCodecs = StreamUtils.getVariantCodecs_(variant);
  ...
  /**
    ...
    *
    */ 
  private static filterVariantsByDensity_(variantsByCodecs: MultiMap<shaka.extern.Variant>): MultiMap<shaka.extern.Variant> {
    let maxDensity = 0;
    const codecGroupsByDensity = new Map();
    const countCodecs = variantsByCodecs.size();
    variantsByCodecs.forEach( 
    (codecs, variants) => {
      for (const variant of variants) {
        ...
        if (!codecGroupsByDensity.has(density)) {
          codecGroupsByDensity.set(density, new MultiMap());
        }
        const group: MultiMap<shaka.extern.Variant> = codecGroupsByDensity.get(density);
  ...
  /**
     ...
     *
     */ 
  static choosePreferredCodecs(variants: shaka.extern.Variant[], preferredVideoCodecs: string[], preferredAudioCodecs: string[]): shaka.extern.Variant[] {
    let subset = variants;
  ...
  /**
     ...
     *
     */ 
  private static chooseCodecsByDecodingAttributes_(variantsByCodecs: MultiMap<shaka.extern.Variant>, attributes: string[]): string {
    const StreamUtils = StreamUtils;
    for (const attribute of attributes) {
      if (attribute == StreamUtils.DecodingAttributes.SMOOTH || attribute == StreamUtils.DecodingAttributes.POWER) {
        variantsByCodecs = StreamUtils.chooseCodecsByMediaCapabilitiesInfo_(variantsByCodecs, attribute);
  ...
  /**
     ...
     *
     */ 
  private static chooseCodecsByMediaCapabilitiesInfo_(variantsByCodecs: MultiMap<shaka.extern.Variant>, attribute: string): MultiMap<shaka.extern.Variant> {
    let highestScore = 0;
    const bestVariantsByCodecs = new MultiMap();
    variantsByCodecs.forEach( 
    (codecs, variants) => {
      let sum = 0;
      let num = 0;
      ...
      log.debug('codecs', codecs, 'avg', attribute, averageScore);
  ...
  /**
     ...
     *
     */ 
  private static findCodecsByLowestBandwidth_(variantsByCodecs: MultiMap<shaka.extern.Variant>): string {
    let bestCodecs = '';
    ...
    (codecs, variants) => {
      ...
      log.debug('codecs', codecs, 'avg bandwidth', averageBandwidth);
      if (averageBandwidth < lowestAverageBandwidth) {
        bestCodecs = codecs;
        lowestAverageBandwidth = averageBandwidth;
      }
    });
    asserts.assert(bestCodecs !== '', 'Should have chosen codecs!');
    asserts.assert(!isNaN(lowestAverageBandwidth), 'Bandwidth should be a number!');
    return bestCodecs;
  }
  ...
  private static getVariantCodecs_(variant: shaka.extern.Variant): string {
    ...
    let baseVideoCodec = '';
    if (variant.video) {
      baseVideoCodec = MimeUtils.getNormalizedCodec(variant.video.codecs);
    }
    let baseAudioCodec = '';
    if (variant.audio) {
      baseAudioCodec = MimeUtils.getNormalizedCodec(variant.audio.codecs);
    }
  ...
  /**
     ...
     *
     */ 
  static filterByRestrictions(manifest: shaka.extern.Manifest, restrictions: shaka.extern.Restrictions, maxHwResolution: {width:number, height:number}) {
    manifest.variants = manifest.variants.filter( 
    (variant) => {
  ...
  /**
     ...
     */ 
  static meetsRestrictions(variant: shaka.extern.Variant, restrictions: shaka.extern.Restrictions, maxHwRes: {width:number, height:number}): boolean {
    ...
    if (video && video.width && video.height) {
      if (!inRange(video.width, restrictions.minWidth, Math.min(restrictions.maxWidth, maxHwRes.width))) {
        return false;
      }
      if (!inRange(video.height, restrictions.minHeight, Math.min(restrictions.maxHeight, maxHwRes.height))) {
        return false;
      }
  ...
  /**
     ...
     */ 
  static applyRestrictions(variants: shaka.extern.Variant[], restrictions: shaka.extern.Restrictions, maxHwRes: {width:number, height:number}): boolean {
  ...
  /**
     ...
     *
     */ 
  static async filterManifest(drmEngine: DrmEngine, currentVariant: shaka.extern.Variant | null, manifest: shaka.extern.Manifest) {
    await StreamUtils.filterManifestByMediaCapabilities(manifest, manifest.offlineSessionIds.length > 0);
    StreamUtils.filterManifestByCurrentVariant(currentVariant, manifest);
  ...
  /**
     ...
     *
     */ 
  static async filterManifestByMediaCapabilities(manifest: shaka.extern.Manifest, usePersistentLicenses: boolean) {
    asserts.assert(navigator.mediaCapabilities, 'MediaCapabilities should be valid.');
    await StreamUtils.getDecodingInfosForVariants(manifest.variants, usePersistentLicenses,  
    /* srcEquals= */ 
    false);
    manifest.variants = manifest.variants.filter( 
    (variant) => {
      ...
      if (video) {
        let videoCodecs = StreamUtils.patchVp9(video.codecs);
        ...
        if (video.codecs.includes(',')) {
          ...
          const audioCodecs = ManifestParserUtils.guessCodecs(ContentType.AUDIO, allCodecs);
      ...
      if (Platform.isXboxOne() && video && (video.width && video.width > 1920 || video.height && video.height > 1080) && video.codecs.includes('avc1.')) {
        log.debug('Dropping variant - not compatible with platform', StreamUtils.getVariantSummaryString_(variant));
        return false;
      }
      const supported = variant.decodingInfos.some( 
      (decodingInfo) => {
  ...
  /**
     ...
     * @exportDoc
     */ 
  static async getDecodingInfosForVariants(variants: shaka.extern.Variant[], usePersistentLicenses: boolean, srcEquals: boolean) {
    ...
    const operations = [];
    const getVariantDecodingInfos =  
    async(variant, decodingConfig) => {
      try {
    ...
    for (const variant of variants) {
      const decodingConfigs: MediaDecodingConfiguration[] = StreamUtils.getDecodingConfigs_(variant, usePersistentLicenses, srcEquals);
  ...
  /**
     ...
     */ 
  private static getDecodingConfigs_(variant: shaka.extern.Variant, usePersistentLicenses: boolean, srcEquals: boolean): MediaDecodingConfiguration[] {
    const audio = variant.audio;
    const video = variant.video;
    const ContentType = ManifestParserUtilsExports.ContentType;
    const mediaDecodingConfig: MediaDecodingConfiguration = {type:srcEquals ? 'file' : 'media-source'};
    if (video) {
      ...
      if (video.codecs.includes(',')) {
        const allCodecs = video.codecs.split(',');
        videoCodecs = ManifestParserUtils.guessCodecs(ContentType.VIDEO, allCodecs);
        videoCodecs = StreamUtils.patchVp9(videoCodecs);
        const audioCodecs = ManifestParserUtils.guessCodecs(ContentType.AUDIO, allCodecs);
        const audioFullType = MimeUtils.getFullOrConvertedType(video.mimeType, audioCodecs, ContentType.AUDIO);
        mediaDecodingConfig.audio = {contentType:audioFullType, channels:2, bitrate:variant.bandwidth || 1, samplerate:1, spatialRendering:false};
      }
      videoCodecs = StreamUtils.patchVp9(videoCodecs);
      const fullType = MimeUtils.getFullOrConvertedType(video.mimeType, videoCodecs, ContentType.VIDEO);
       
      // VideoConfiguration 
      mediaDecodingConfig.video = {contentType:fullType,  
      ...
      width:video.width || 64, height:video.height || 64, bitrate:video.bandwidth || variant.bandwidth || 1,  
    ...
    if (audio) {
      ...
      // AudioConfiguration 
      mediaDecodingConfig.audio = {contentType:fullType, channels:audio.channelsCount || 2, bitrate:audio.bandwidth || variant.bandwidth || 1, samplerate:audio.audioSamplingRate || 1, spatialRendering:audio.spatialAudio};
    }
    ...
    for (const keySystem of drmInfoByKeySystems.keys()) {
      ...
      const config = (Object.assign({}, mediaDecodingConfig) as MediaDecodingConfiguration);
      const drmInfos = drmInfoByKeySystems.get(keySystem);
      const keySystemConfig: MediaCapabilitiesKeySystemConfiguration = {keySystem:keySystem, initDataType:'cenc', persistentState:persistentState, distinctiveIdentifier:'optional', sessionTypes:sessionTypes};
      for (const info of drmInfos) {
        if (info.initData && info.initData.length) {
          ...
          if (initDataTypes.size > 1) {
            log.v2('DrmInfo contains more than one initDataType,', 'and we use the initDataType of the first initData.', info);
          }
  ...
  /**
     ...
     *
     */ 
  static filterManifestByCurrentVariant(currentVariant: shaka.extern.Variant | null, manifest: shaka.extern.Manifest) {
    const StreamUtils = StreamUtils;
    manifest.variants = manifest.variants.filter( 
    (variant) => {
      const audio = variant.audio;
      ...
      if (audio && currentVariant && currentVariant.audio) {
        if (!StreamUtils.areStreamsCompatible_(audio, currentVariant.audio)) {
          log.debug('Droping variant - not compatible with active audio', 'active audio', StreamUtils.getStreamSummaryString_(currentVariant.audio), 'variant.audio', StreamUtils.getStreamSummaryString_(audio));
          return false;
        }
      }
      if (video && currentVariant && currentVariant.video) {
        if (!StreamUtils.areStreamsCompatible_(video, currentVariant.video)) {
          log.debug('Droping variant - not compatible with active video', 'active video', StreamUtils.getStreamSummaryString_(currentVariant.video), 'variant.video', StreamUtils.getStreamSummaryString_(video));
          return false;
        }
      }
      return true;
  ...
  private static async filterImageStreams_(manifest: shaka.extern.Manifest) {
    ...
    for (const stream of manifest.imageStreams) {
      ...
      if (!supportedImageMimeTypes_.has(mimeType)) {
        const minImage = minImage_.get(mimeType);
        if (minImage) {
           
          // eslint-disable-next-line no-await-in-loop 
          const res = await StreamUtils.isImageSupported_(minImage);
          supportedImageMimeTypes_.set(mimeType, res);
        } else {
          supportedImageMimeTypes_.set(mimeType, false);
        }
      }
      const keep = supportedImageMimeTypes_.get(mimeType);
      if (!keep) {
  ...
  private static areStreamsCompatible_(s0: shaka.extern.Stream, s1: shaka.extern.Stream): boolean {
  ...
  static variantToTrack(variant: shaka.extern.Variant): shaka.extern.Track {
    ...
    const track: shaka.extern.Track = {id:variant.id, active:false, type:'variant', bandwidth:variant.bandwidth, language:variant.language, label:null, kind:kind, width:null, height:null, frameRate:null, pixelAspectRatio:null, hdr:null, mimeType:mimeType, audioMimeType:audioMimeType, videoMimeType:videoMimeType, codecs:codecs.join(', '), audioCodec:audioCodec, videoCodec:videoCodec, primary:variant.primary, roles:Array.from(roles), audioRoles:null, forced:false, videoId:null, audioId:null, channelsCount:null, 
    audioSamplingRate:null, spatialAudio:false, tilesLayout:null, audioBandwidth:null, videoBandwidth:null, originalVideoId:null, originalAudioId:null, originalTextId:null, originalImageId:null};
    if (video) {
  ...
  static textStreamToTrack(stream: shaka.extern.Stream): shaka.extern.Track {
    const ContentType = ManifestParserUtilsExports.ContentType;
    const track: shaka.extern.Track = {id:stream.id, active:false, type:ContentType.TEXT, bandwidth:0, language:stream.language, label:stream.label, kind:stream.kind || null, width:null, height:null, frameRate:null, pixelAspectRatio:null, hdr:null, mimeType:stream.mimeType, audioMimeType:null, videoMimeType:null, codecs:stream.codecs || null, audioCodec:null, videoCodec:null, primary:stream.primary, roles:stream.roles, audioRoles:null, forced:stream.forced, videoId:null, audioId:null, channelsCount:null, 
    audioSamplingRate:null, spatialAudio:false, tilesLayout:null, audioBandwidth:null, videoBandwidth:null, originalVideoId:null, originalAudioId:null, originalTextId:stream.originalId, originalImageId:null};
    return track;
  }
  ...
  static imageStreamToTrack(stream: shaka.extern.Stream): shaka.extern.Track {
    ...
    const track: shaka.extern.Track = {id:stream.id, active:false, type:ContentType.IMAGE, bandwidth:stream.bandwidth || 0, language:'', label:null, kind:null, width, height, frameRate:null, pixelAspectRatio:null, hdr:null, mimeType:stream.mimeType, audioMimeType:null, videoMimeType:null, codecs:null, audioCodec:null, videoCodec:null, primary:false, roles:[], audioRoles:null, forced:false, videoId:null, audioId:null, channelsCount:null, audioSamplingRate:null, spatialAudio:false, tilesLayout:layout || 
    null, audioBandwidth:null, videoBandwidth:null, originalVideoId:null, originalAudioId:null, originalTextId:null, originalImageId:stream.originalId};
    return track;
  }
  ...
  static html5TrackId(html5Track: TextTrack | AudioTrack): number {
    if (!html5Track['__shaka_id']) {
      html5Track['__shaka_id'] = nextTrackId_++;
    }
    return html5Track['__shaka_id'];
  }
   
  static html5TextTrackToTrack(textTrack: TextTrack): shaka.extern.Track {
    const CLOSED_CAPTION_MIMETYPE = MimeUtilsExports.CEA608_CLOSED_CAPTION_MIMETYPE;
    const StreamUtils = StreamUtils;
    const track: shaka.extern.Track = StreamUtils.html5TrackToGenericShakaTrack_(textTrack);
  ...
  static html5AudioTrackToTrack(audioTrack: AudioTrack): shaka.extern.Track {
    const StreamUtils = StreamUtils;
    const track: shaka.extern.Track = StreamUtils.html5TrackToGenericShakaTrack_(audioTrack);
  ...
  /**
     ...
     *
     */ 
  private static html5TrackToGenericShakaTrack_(html5Track: TextTrack | AudioTrack): shaka.extern.Track {
    const track: shaka.extern.Track = {id:StreamUtils.html5TrackId(html5Track), active:false, type:'', bandwidth:0, language:LanguageUtils.normalize(html5Track.language), label:html5Track.label, kind:html5Track.kind, width:null, height:null, frameRate:null, pixelAspectRatio:null, hdr:null, mimeType:null, audioMimeType:null, videoMimeType:null, codecs:null, audioCodec:null, videoCodec:null, primary:false, roles:[], forced:false, audioRoles:null, videoId:null, audioId:null, channelsCount:null, audioSamplingRate:null, 
    spatialAudio:false, tilesLayout:null, audioBandwidth:null, videoBandwidth:null, originalVideoId:null, originalAudioId:null, originalTextId:null, originalImageId:null};
    return track;
  }
  ...
  /**
     ...
     */ 
  static getPlayableVariants(variants: shaka.extern.Variant[]): shaka.extern.Variant[] {
  ...
  /**
     ...
     *
     */ 
  static filterVariantsByAudioChannelCount(variants: shaka.extern.Variant[], preferredAudioChannelCount: number): shaka.extern.Variant[] {
    ...
    const variantsByChannelCount: Map<number, shaka.extern.Variant[]> = new Map();
    for (const variant of variantsWithChannelCounts) {
      const count = variant.audio.channelsCount;
      asserts.assert(count != null, 'Must have count after filtering!');
    ...
    if (countLessThanOrEqualtoConfig.length) {
      return variantsByChannelCount.get(Math.max(...countLessThanOrEqualtoConfig));
    }
  ...
  /**
     ...
     *
     */ 
  static filterStreamsByLanguageAndRole(streams: shaka.extern.Stream[], preferredLanguage: string, preferredRole: string, preferredForced: boolean): shaka.extern.Stream[] {
    const LanguageUtils = LanguageUtils;
    let chosen: shaka.extern.Stream[] = streams;
     
    // Start with the set of primary streams. 
    const primary: shaka.extern.Stream[] = streams.filter( 
    (stream) => {
    ...
    if (preferredLanguage) {
      const closestLocale = LanguageUtils.findClosestLocale(LanguageUtils.normalize(preferredLanguage), streams.map( 
      (stream) => stream.language));
       
      // Only replace |chosen| if we found a locale that is close to our
      // preference. 
      if (closestLocale) {
        chosen = streams.filter( 
        (stream) => {
    ...
    if (preferredRole) {
      const roleMatches = StreamUtils.filterTextStreamsByRole_(chosen, preferredRole);
      ...
      const noRoleMatches = chosen.filter( 
      (stream) => {
  ...
  /**
     ...
     *
     */ 
  private static filterTextStreamsByRole_(textStreams: shaka.extern.Stream[], preferredRole: string): shaka.extern.Stream[] {
  ...
  /**
     ...
     *
     */ 
  static getVariantStreams(variant: shaka.extern.Variant): shaka.extern.Stream[] {
    const streams = [];
    if (variant.audio) {
  ...
  /**
     ...
     */ 
  private static getVariantSummaryString_(variant: shaka.extern.Variant): string {
    const summaries = [];
    if (variant.audio) {
  ...
  /**
     ...
     */ 
  private static getStreamSummaryString_(stream: shaka.extern.Stream): string {
     
    // Accepted parameters for Chromecast can be found (internally) at
    // go/cast-mime-params 
    if (StreamUtils.isAudio(stream)) {
      return 'type=audio' + ' codecs=' + stream.codecs + ' bandwidth=' + stream.bandwidth + ' channelsCount=' + stream.channelsCount + ' audioSamplingRate=' + stream.audioSamplingRate;
    }
    if (StreamUtils.isVideo(stream)) {
      return 'type=video' + ' codecs=' + stream.codecs + ' bandwidth=' + stream.bandwidth + ' frameRate=' + stream.frameRate + ' width=' + stream.width + ' height=' + stream.height;
    }
...
export const supportedImageMimeTypes_: Map<string, boolean> = (new Map()).set('image/svg+xml', true).set('image/png', true).set('image/jpeg', true).set('image/jpg', true);
 
export const minWebPImage_: string = 'data:image/webp;base64,UklGRjoAAABXRU' + 'JQVlA4IC4AAACyAgCdASoCAAIALmk0mk0iIiIiIgBoSygABc6WWgAA/veff/0PP8bA//LwY' + 'AAA';
 
export const minAvifImage_: string = 'data:image/avif;base64,AAAAIGZ0eXBhdm' + 'lmAAAAAGF2aWZtaWYxbWlhZk1BMUIAAADybWV0YQAAAAAAAAAoaGRscgAAAAAAAAAAcGljd' + 'AAAAAAAAAAAAAAAAGxpYmF2aWYAAAAADnBpdG0AAAAAAAEAAAAeaWxvYwAAAABEAAABAAEA' + 'AAABAAABGgAAAB0AAAAoaWluZgAAAAAAAQAAABppbmZlAgAAAAABAABhdjAxQ29sb3IAAAA' + 'AamlwcnAAAABLaXBjbwAAABRpc3BlAAAAAAAAAAIAAAACAAAAEHBpeGkAAAAAAwgICAAAAA' + 'xhdjFDgQ0MAAAAABNjb2xybmNseAACAAIAAYAAAAAXaXBtYQAAAAAAAAABAAEEAQKDBAAAA' + 'CVtZGF0EgAKCBgANogQEAwgMg8f8D///8WfhwB8+ErK42A=';
 
export const minImage_: Map<string, string> = (new Map()).set('image/webp', minWebPImage_).set('image/avif', minAvifImage_);
