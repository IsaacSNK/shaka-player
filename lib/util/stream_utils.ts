...
export class StreamUtils {
  ...
  static chooseCodecsAndFilterManifest(
      manifest: shaka.extern.Manifest, preferredVideoCodecs: string[],
      preferredAudioCodecs: string[], preferredAudioChannelCount: number,
    ...
    if (preferredVideoCodecs.length || preferredAudioCodecs.length) {
      variants = StreamUtils.choosePreferredCodecs(
          variants, preferredVideoCodecs, preferredAudioCodecs);
    }
   ...
   *
   */
  private static getVariantsByCodecs_(variants: shaka.extern.Variant[]):
   ...
   *
   */
  private static filterVariantsByDensity_(variantsByCodecs:
                                              MultiMap<shaka.extern.Variant>):
      MultiMap<shaka.extern.Variant> {
    let maxDensity = 0;
  ...
  static choosePreferredCodecs(
      variants: shaka.extern.Variant[], preferredVideoCodecs: string[],
      preferredAudioCodecs: string[]): shaka.extern.Variant[] {
    let subset = variants;
  ...
  private static chooseCodecsByMediaCapabilitiesInfo_(
      variantsByCodecs: MultiMap<shaka.extern.Variant>,
      attribute: string): MultiMap<shaka.extern.Variant> {
    let highestScore = 0;
  ...
  private static findCodecsByLowestBandwidth_(
      variantsByCodecs: MultiMap<shaka.extern.Variant>): string {
    let bestCodecs = '';
    ...
    asserts.assert(
        !isNaN(lowestAverageBandwidth), 'Bandwidth should be a number!');
    return bestCodecs;
  }
  ...
  static filterByRestrictions(
      manifest: shaka.extern.Manifest, restrictions: shaka.extern.Restrictions,
  ...
  static meetsRestrictions(
      variant: shaka.extern.Variant, restrictions: shaka.extern.Restrictions,
      maxHwRes: {width: number, height: number}): boolean {
    ...
    if (video && video.width && video.height) {
      if (!inRange(
              video.width, restrictions.minWidth,
    ...
    if (variant && variant.video && variant.video.frameRate) {
      if (!inRange(
              variant.video.frameRate, restrictions.minFrameRate,
              restrictions.maxFrameRate)) {
        return false;
      }
    }
    if (!inRange(
            variant.bandwidth, restrictions.minBandwidth,
  ...
  static applyRestrictions(
      variants: shaka.extern.Variant[], restrictions: shaka.extern.Restrictions,
      maxHwRes: {width: number, height: number}): boolean {
    let tracksChanged = false;
    for (const variant of variants) {
      const originalAllowed = variant.allowedByApplication;
      variant.allowedByApplication =
          StreamUtils.meetsRestrictions(variant, restrictions, maxHwRes);
  ...
  static async filterManifest(
      drmEngine: DrmEngine, currentVariant: shaka.extern.Variant|null,
      manifest: shaka.extern.Manifest) {
    await StreamUtils.filterManifestByMediaCapabilities(
        manifest, manifest.offlineSessionIds.length > 0);
  ...
  static async filterManifestByMediaCapabilities(
      manifest: shaka.extern.Manifest, usePersistentLicenses: boolean) {
    asserts.assert(
        navigator.mediaCapabilities, 'MediaCapabilities should be valid.');
  ...
  static async getDecodingInfosForVariants(
      variants: shaka.extern.Variant[], usePersistentLicenses: boolean,
      srcEquals: boolean) {
    const gotDecodingInfo =
        variants.some((variant) => variant.decodingInfos.length);
    if (gotDecodingInfo) {
    ...
    for (const variant of variants) {
      const decodingConfigs: MediaDecodingConfiguration[] =
          StreamUtils.getDecodingConfigs_(
  ...
  private static getDecodingConfigs_(
      variant: shaka.extern.Variant, usePersistentLicenses: boolean,
      srcEquals: boolean): MediaDecodingConfiguration[] {
    const audio = variant.audio;
    const video = variant.video;
    const ContentType = ManifestParserUtilsExports.ContentType;
    ...
    for (const keySystem of drmInfoByKeySystems.keys()) {
      ...
      const config =
          (Object.assign({}, mediaDecodingConfig) as
        ...
        if (info.initData && info.initData.length) {
          ...
          if (initDataTypes.size > 1) {
            log.v2(
                ...
                'and we use the initDataType of the first initData.', info);
          }
      ...
      if (audio && currentVariant && currentVariant.audio) {
        if (!StreamUtils.areStreamsCompatible_(audio, currentVariant.audio)) {
          log.debug(
              ...
              'variant.audio', StreamUtils.getStreamSummaryString_(audio));
          return false;
        }
      }
      if (video && currentVariant && currentVariant.video) {
        if (!StreamUtils.areStreamsCompatible_(video, currentVariant.video)) {
          log.debug(
              ...
              'variant.video', StreamUtils.getStreamSummaryString_(video));
          return false;
        }
      }
      return true;
  ...
  private static filterTextStreams_(manifest: shaka.extern.Manifest) {
    ...
    manifest.textStreams = manifest.textStreams.filter((stream) => {
      ...
      if (!keep) {
        log.debug(
            'Dropping text stream. Is not supported by the ' +
                'platform.',
            stream);
      }
      return keep;
  ...
  private static async filterImageStreams_(manifest: shaka.extern.Manifest) {
    ...
    for (const stream of manifest.imageStreams) {
      ...
      if (!keep) {
        log.debug(
            'Dropping image stream. Is not supported by the ' +
                'platform.',
            stream);
      } else {
  ...
  private static areStreamsCompatible_(
      s0: shaka.extern.Stream, s1: shaka.extern.Stream): boolean {
  ...
  static html5TextTrackToTrack(textTrack: TextTrack): shaka.extern.Track {
    const CLOSED_CAPTION_MIMETYPE =
        MimeUtilsExports.CEA608_CLOSED_CAPTION_MIMETYPE;
    const StreamUtils = StreamUtils;
    const track: shaka.extern.Track =
   ...
   *
   */
  private static html5TrackToGenericShakaTrack_(html5Track: TextTrack|
                                                AudioTrack):
      shaka.extern.Track {
    const track: shaka.extern.Track = {
   ...
   */
  static getPlayableVariants(variants: shaka.extern.Variant[]):
  ...
  static filterVariantsByAudioChannelCount(
      ...
      preferredAudioChannelCount: number): shaka.extern.Variant[] {
  ...
  static filterStreamsByLanguageAndRole(
      streams: shaka.extern.Stream[], preferredLanguage: string,
      preferredRole: string, preferredForced: boolean): shaka.extern.Stream[] {
    ...
    if (preferredRole) {
      const roleMatches =
          StreamUtils.filterTextStreamsByRole_(chosen, preferredRole);
  ...
  private static filterTextStreamsByRole_(
      ...
      preferredRole: string): shaka.extern.Stream[] {
   ...
   *
   */
  static getVariantStreams(variant: shaka.extern.Variant):
      shaka.extern.Stream[] {
    const streams = [];
    if (variant.audio) {
   ...
   */
  private static getVariantSummaryString_(variant: shaka.extern.Variant):
      string {
    const summaries = [];
    if (variant.audio) {
  ...
  private static getStreamSummaryString_(stream: shaka.extern.Stream): string {
    ...
    if (StreamUtils.isVideo(stream)) {
      return 'type=video' +
          ' codecs=' + stream.codecs + ' bandwidth=' + stream.bandwidth +
          ' frameRate=' + stream.frameRate + ' width=' + stream.width +
...
export const supportedImageMimeTypes_: Map<string, boolean> =
    (new Map())
...
export const minWebPImage_: string = 'data:image/webp;base64,UklGRjoAAABXRU' +
    'JQVlA4IC4AAACyAgCdASoCAAIALmk0mk0iIiIiIgBoSygABc6WWgAA/veff/0PP8bA//LwY' +
    'AAA';

export const minAvifImage_: string = 'data:image/avif;base64,AAAAIGZ0eXBhdm' +
...
export const minImage_: Map<string, string> =
    (new Map())
        .set('image/webp', minWebPImage_)
        .set('image/avif', minAvifImage_);
