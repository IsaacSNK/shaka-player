...
export class LanguageUtils {
  ...
  static areLanguageCompatible(locale1: string, locale2: string): boolean {
    ...
    const locale1Components: string[] =
        LanguageUtils.disassembleLocale_(locale1);
    const locale2Components: string[] =
        LanguageUtils.disassembleLocale_(locale2);
  ...
  static isParentOf(possibleParent: string, possibleChild: string): boolean {
    ...
    // Get all components. This should only be language and region
    // since we do not support dialect.
    const possibleParentComponents: string[] =
        LanguageUtils.disassembleLocale_(possibleParent);
    const possibleChildComponents: string[] =
  ...
  static isSiblingOf(localeA: string, localeB: string): boolean {
    ...
    // Get all components. This should only be language and region
    // since we do not support dialect.
    const localeAComponents: string[] =
        LanguageUtils.disassembleLocale_(localeA);
    const localeBComponents: string[] =
        LanguageUtils.disassembleLocale_(localeB);
    return localeAComponents.length == 2 && localeBComponents.length == 2 &&
   ...
   * @export
   */
  static findClosestLocale(target: string, searchSpace: Iterable<string>):
      string|null {
    const LanguageUtils = LanguageUtils;
  ...
  private static disassembleLocale_(locale: string): string[] {
    const components = locale.split('-');
    asserts.assert(components.length <= 2, [
      'Locales should not have more than 2 components. ', locale,
 ...
 *
 */
export const isoMap_: Map<string, string> = new Map([
