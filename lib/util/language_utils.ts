...
export class LanguageUtils {
  ...
  static areLanguageCompatible(locale1: string, locale2: string): boolean {
    ...
    const locale1Components: string[] = LanguageUtils.disassembleLocale_(locale1);
    const locale2Components: string[] = LanguageUtils.disassembleLocale_(locale2);
  ...
  static isParentOf(possibleParent: string, possibleChild: string): boolean {
    ...
    // Get all components. This should only be language and region
    // since we do not support dialect. 
    const possibleParentComponents: string[] = LanguageUtils.disassembleLocale_(possibleParent);
    const possibleChildComponents: string[] = LanguageUtils.disassembleLocale_(possibleChild);
    return possibleParentComponents[0] == possibleChildComponents[0] && possibleParentComponents.length == 1 && possibleChildComponents.length == 2;
  }
  ...
  static isSiblingOf(localeA: string, localeB: string): boolean {
    ...
    // Get all components. This should only be language and region
    // since we do not support dialect. 
    const localeAComponents: string[] = LanguageUtils.disassembleLocale_(localeA);
    const localeBComponents: string[] = LanguageUtils.disassembleLocale_(localeB);
    return localeAComponents.length == 2 && localeBComponents.length == 2 && localeAComponents[0] == localeBComponents[0];
  }
  ...
  static getLocaleForText(stream: shaka.extern.Stream): string {
    ...
    asserts.assert(stream.type == ContentType.TEXT, 'Can only get language from text streams');
  ...
  static findClosestLocale(target: string, searchSpace: Iterable<string>): string | null {
    const LanguageUtils = LanguageUtils;
    const safeTarget: string = LanguageUtils.normalize(target);
    const safeSearchSpace: Set<string> = new Set();
  ...
  /**
     ...
     *
     */ 
  private static disassembleLocale_(locale: string): string[] {
    const components = locale.split('-');
    asserts.assert(components.length <= 2, ['Locales should not have more than 2 components. ', locale, ' has too many components.'].join());
    return components;
  }
}
 ...
 *
 */ 
export const isoMap_: Map<string, string> = new Map([['aar', 'aa'], ['abk', 'ab'], ['afr', 'af'], ['aka', 'ak'], ['alb', 'sq'], ['amh', 'am'], ['ara', 'ar'], ['arg', 'an'], ['arm', 'hy'], ['asm', 'as'], ['ava', 'av'], ['ave', 'ae'], ['aym', 'ay'], ['aze', 'az'], ['bak', 'ba'], ['bam', 'bm'], ['baq', 'eu'], ['bel', 'be'], ['ben', 'bn'], ['bih', 'bh'], ['bis', 'bi'], ['bod', 'bo'], ['bos', 'bs'], ['bre', 'br'], ['bul', 'bg'], ['bur', 'my'], ['cat', 'ca'], ['ces', 'cs'], ['cha', 'ch'], ['che', 'ce'], 
['chi', 'zh'], ['chu', 'cu'], ['chv', 'cv'], ['cor', 'kw'], ['cos', 'co'], ['cre', 'cr'], ['cym', 'cy'], ['cze', 'cs'], ['dan', 'da'], ['deu', 'de'], ['div', 'dv'], ['dut', 'nl'], ['dzo', 'dz'], ['ell', 'el'], ['eng', 'en'], ['epo', 'eo'], ['est', 'et'], ['eus', 'eu'], ['ewe', 'ee'], ['fao', 'fo'], ['fas', 'fa'], ['fij', 'fj'], ['fin', 'fi'], ['fra', 'fr'], ['fre', 'fr'], ['fry', 'fy'], ['ful', 'ff'], ['geo', 'ka'], ['ger', 'de'], ['gla', 'gd'], ['gle', 'ga'], ['glg', 'gl'], ['glv', 'gv'], ['gre', 
'el'], ['grn', 'gn'], ['guj', 'gu'], ['hat', 'ht'], ['hau', 'ha'], ['heb', 'he'], ['her', 'hz'], ['hin', 'hi'], ['hmo', 'ho'], ['hrv', 'hr'], ['hun', 'hu'], ['hye', 'hy'], ['ibo', 'ig'], ['ice', 'is'], ['ido', 'io'], ['iii', 'ii'], ['iku', 'iu'], ['ile', 'ie'], ['ina', 'ia'], ['ind', 'id'], ['ipk', 'ik'], ['isl', 'is'], ['ita', 'it'], ['jav', 'jv'], ['jpn', 'ja'], ['kal', 'kl'], ['kan', 'kn'], ['kas', 'ks'], ['kat', 'ka'], ['kau', 'kr'], ['kaz', 'kk'], ['khm', 'km'], ['kik', 'ki'], ['kin', 'rw'], 
['kir', 'ky'], ['kom', 'kv'], ['kon', 'kg'], ['kor', 'ko'], ['kua', 'kj'], ['kur', 'ku'], ['lao', 'lo'], ['lat', 'la'], ['lav', 'lv'], ['lim', 'li'], ['lin', 'ln'], ['lit', 'lt'], ['ltz', 'lb'], ['lub', 'lu'], ['lug', 'lg'], ['mac', 'mk'], ['mah', 'mh'], ['mal', 'ml'], ['mao', 'mi'], ['mar', 'mr'], ['may', 'ms'], ['mkd', 'mk'], ['mlg', 'mg'], ['mlt', 'mt'], ['mon', 'mn'], ['mri', 'mi'], ['msa', 'ms'], ['mya', 'my'], ['nau', 'na'], ['nav', 'nv'], ['nbl', 'nr'], ['nde', 'nd'], ['ndo', 'ng'], ['nep', 
'ne'], ['nld', 'nl'], ['nno', 'nn'], ['nob', 'nb'], ['nor', 'no'], ['nya', 'ny'], ['oci', 'oc'], ['oji', 'oj'], ['ori', 'or'], ['orm', 'om'], ['oss', 'os'], ['pan', 'pa'], ['per', 'fa'], ['pli', 'pi'], ['pol', 'pl'], ['por', 'pt'], ['pus', 'ps'], ['que', 'qu'], ['roh', 'rm'], ['ron', 'ro'], ['rum', 'ro'], ['run', 'rn'], ['rus', 'ru'], ['sag', 'sg'], ['san', 'sa'], ['sin', 'si'], ['slk', 'sk'], ['slo', 'sk'], ['slv', 'sl'], ['sme', 'se'], ['smo', 'sm'], ['sna', 'sn'], ['snd', 'sd'], ['som', 'so'], 
['sot', 'st'], ['spa', 'es'], ['sqi', 'sq'], ['srd', 'sc'], ['srp', 'sr'], ['ssw', 'ss'], ['sun', 'su'], ['swa', 'sw'], ['swe', 'sv'], ['tah', 'ty'], ['tam', 'ta'], ['tat', 'tt'], ['tel', 'te'], ['tgk', 'tg'], ['tgl', 'tl'], ['tha', 'th'], ['tib', 'bo'], ['tir', 'ti'], ['ton', 'to'], ['tsn', 'tn'], ['tso', 'ts'], ['tuk', 'tk'], ['tur', 'tr'], ['twi', 'tw'], ['uig', 'ug'], ['ukr', 'uk'], ['urd', 'ur'], ['uzb', 'uz'], ['ven', 've'], ['vie', 'vi'], ['vol', 'vo'], ['wel', 'cy'], ['wln', 'wa'], ['wol', 
'wo'], ['xho', 'xh'], ['yid', 'yi'], ['yor', 'yo'], ['zha', 'za'], ['zho', 'zh'], ['zul', 'zu']]);
