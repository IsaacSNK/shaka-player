...
export class Pssh {
  ...
  constructor(psshBox: Uint8Array) {
    (new Mp4Parser()).box('moov', Mp4Parser.children).fullBox('pssh',  
    (box) => this.parsePsshBox_(box)).parse(psshBox);
  ...
  /**
     ...
     *
     */ 
  static createPssh(data: Uint8Array, systemId: Uint8Array, keyIds: Set<string>, version: number): Uint8Array {
    ...
    byteCursor += 4;
    psshData.setUint32(byteCursor,  
    // 'pssh' 
    1886614376);
    byteCursor += 4;
    version < 1 ? psshData.setUint32(byteCursor, 0) : psshData.setUint32(byteCursor,  
    // version + flags 
    16777216);
    byteCursor += 4;
  ...
  static normaliseInitData(initData: Uint8Array): Uint8Array {
    ...
    for (const initData of pssh.data) {
      const found = dedupedInitDatas.some( 
      (x) => {
