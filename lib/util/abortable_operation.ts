 ...
 * @export
 */ 
export class AbortableOperation implements shaka.extern.IAbortableOperation<T> {
  ...
  static aborted(): AbortableOperation {
    ...
    return new AbortableOperation(p,  
    () => Promise.resolve());
  }
   
  static abortError(): Error {
    return new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.PLAYER, ErrorExports.Code.OPERATION_ABORTED);
  }
  ...
  static all(operations: AbortableOperation[]): AbortableOperation {
    return new AbortableOperation(Promise.all(operations.map( 
    (op) => op.promise)),  
    () => Promise.all(operations.map( 
    (op) => op.abort())));
  }
  ...
  /**
     ...
     * @export
     */ 
  chain(onSuccess: undefined | ((p1: T) => U) | ((p1: T) => Promise<U>) | ((p1: T) => AbortableOperation<U>), onError?: (p1: any) => any): AbortableOperation<U> {
    ...
    return new AbortableOperation(newPromise,  
  ...
  /**
     ...
     * @template T, U
     */ 
  private static wrapChainCallback_(callback: ((p1: T) => U) | ((p1: T) => Promise<U>) | ((p1: T) => AbortableOperation<U>) | ((p1: any) => any), value: T, newPromise: PublicPromise): () => Promise {
    try {
