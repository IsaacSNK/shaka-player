...
export class MimeUtils {
  ...
  /**
     ...
     *
     */ 
  static getFullOrConvertedType(mimeType: string, codecs: string, contentType: string): string {
 ...
 *
 */ 
export const EXTENDED_MIME_PARAMETERS_: Map<string, string> = (new Map()).set('codecs', 'codecs').set('frameRate',  
// Ours is camelCase, theirs is lowercase. 
'framerate').set('bandwidth',  
// They are in the same units: bits/sec. 
'bitrate').set('width', 'width').set('height', 'height').set('channelsCount', 'channels');
