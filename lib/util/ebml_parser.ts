...
export class EbmlParser {
  ...
  constructor(data: BufferSource) {
    ...
    this.reader_ = new DataViewReader(this.dataView_, DataViewReaderExports.Endianness.BIG_ENDIAN);
  }
  ...
  parseElement(): EbmlElement {
    ...
    const elementSize = this.reader_.getPosition() + size <= this.dataView_.byteLength ? size : this.dataView_.byteLength - this.reader_.getPosition();
  ...
  private parseId_(): number {
    const vint = this.parseVint_();
    if (vint.length > 7) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.EBML_OVERFLOW);
    }
    let id = 0;
    for (const  
    /* byte */ 
 ...
 */ 
export const DYNAMIC_SIZES: number[][] = [[255], [127, 255], [63, 255, 255], [31, 255, 255, 255], [15, 255, 255, 255, 255], [7, 255, 255, 255, 255, 255], [3, 255, 255, 255, 255, 255, 255], [1, 255, 255, 255, 255, 255, 255, 255]];
  ...
  getFloat(): number {
    if (this.dataView_.byteLength == 4) {
      ...
      if (this.dataView_.byteLength == 8) {
        ...
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.EBML_BAD_FLOATING_POINT_SIZE);
      }
    }
  }
}
