...
export class ManifestParserUtils {
  ...
  static resolveUris(baseUris: string[], relativeUris: string[]): string[] {
    ...
    const relativeAsGoog = relativeUris.map( 
    ...
    return baseUris.map( 
    (uri) => new goog.Uri(uri)).map( 
    (base) => relativeAsGoog.map( 
    (i) => base.resolve(i))).reduce(Functional.collapseArrays, []).map( 
    (uri) => uri.toString());
  }
  ...
  /**
     ...
     *
     */ 
  static createDrmInfo(keySystem: string, initData: shaka.extern.InitDataOverride[]): shaka.extern.DrmInfo {
    return {keySystem:keySystem, licenseServerUri:'', distinctiveIdentifierRequired:false, persistentStateRequired:false, audioRobustness:'', videoRobustness:'', serverCertificate:null, serverCertificateUri:'', sessionType:'', initData:initData || [], keyIds:new Set()};
  }
  ...
  static guessCodecs(contentType: string, codecs: string[]): string {
    ...
    throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_COULD_NOT_GUESS_CODECS, codecs);
  }
 ...
 *
 */ 
export const VIDEO_CODEC_REGEXPS_: RegExp[] = [/^avc/, /^hev/, /^hvc/, /^vp0?[89]/, /^av01/];
 
/**
 * A list of regexps to detect well-known audio codecs.
 *
 */ 
export const AUDIO_CODEC_REGEXPS_: RegExp[] = [/^vorbis$/, /^opus$/, /^flac$/, /^mp4a/, /^[ae]c-3$/];
...
export const CODEC_REGEXPS_BY_CONTENT_TYPE_: {[key:string]:RegExp[]} = {'audio':AUDIO_CODEC_REGEXPS_, 'video':VIDEO_CODEC_REGEXPS_, 'text':TEXT_CODEC_REGEXPS_};
