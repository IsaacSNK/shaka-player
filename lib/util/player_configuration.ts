...
import{SimpleAbrManager}from './simple_abr_manager';
import*as SimpleAbrManagerExports from './simple_abr_manager';
import{log}from './log';
...
export class PlayerConfiguration {
  ...
  static createDefault(): shaka.extern.PlayerConfiguration {
    ...
    const drm = {retryParameters:NetworkingEngine.defaultRetryParameters(),  
    // These will all be verified by special cases in mergeConfigObjects_(): 
    servers:{},  
    // key is arbitrary key system ID, value must be string 
    clearKeys:{},  
    // key is arbitrary key system ID, value must be string 
    advanced:{},  
    // key is arbitrary key system ID, value is a record type 
    delayLicenseRequestUntilPlayed:false, initDataTransform: 
    (initData, initDataType, drmInfo) => {
      return ConfigUtils.referenceParametersAndReturn([initData, initDataType, drmInfo], initData);
    }, logLicenseExchange:false, updateExpirationTime:1, preferredKeySystems:[], keySystemsMapping:{}};
    const manifest = {retryParameters:NetworkingEngine.defaultRetryParameters(), availabilityWindowOverride:NaN, disableAudio:false, disableVideo:false, disableText:false, disableThumbnails:false, defaultPresentationDelay:0, segmentRelativeVttTiming:false, dash:{clockSyncUri:'', ignoreDrmInfo:false, disableXlinkProcessing:false, xlinkFailGracefully:false, ignoreMinBufferTime:false, autoCorrectDrift:true, initialSegmentLimit:1000, ignoreSuggestedPresentationDelay:false, ignoreEmptyAdaptationSet:false, 
    ignoreMaxSegmentDuration:false, keySystemsByURI:{'urn:uuid:1077efec-c0b2-4d02-ace3-3c1e52e2fb4b':'org.w3.clearkey', 'urn:uuid:e2719d58-a985-b3c9-781a-b030af78d30e':'org.w3.clearkey', 'urn:uuid:edef8ba9-79d6-4ace-a3c8-27dcd51d21ed':'com.widevine.alpha', 'urn:uuid:9a04f079-9840-4286-ab92-e65be0885f95':'com.microsoft.playready', 'urn:uuid:79f0049a-4098-8642-ab92-e65be0885f95':'com.microsoft.playready', 'urn:uuid:f239e769-efa3-4850-9c16-a903c6932efb':'com.adobe.primetime'}, manifestPreprocessor: 
    (element) => {
      return ConfigUtils.referenceParametersAndReturn([element], element);
    }}, hls:{ignoreTextStreamFailures:false, ignoreImageStreamFailures:false, defaultAudioCodec:'mp4a.40.2', defaultVideoCodec:'avc1.42E01E', ignoreManifestProgramDateTime:false, mediaPlaylistFullMimeType:'video/mp2t; codecs="avc1.42E01E, mp4a.40.2"'}};
    const streaming = {retryParameters:NetworkingEngine.defaultRetryParameters(),  
    ...
    rebufferingGoal:2, bufferingGoal:10, bufferBehind:30, ignoreTextStreamFailures:false, alwaysStreamText:false, startAtSegmentBoundary:false, gapDetectionThreshold:0.5, durationBackoff:1, forceTransmuxTS:false,  
    // Offset by 5 seconds since Chromecast takes a few seconds to start
    // playing after a seek, even when buffered. 
    safeSeekOffset:5, stallEnabled:true, stallThreshold:1,  
    /* seconds */ 
    stallSkip:0.1,  
    /* seconds */ 
    ...
    inaccurateManifestTolerance:2, lowLatencyMode:false, autoLowLatencyMode:false, forceHTTPS:false, preferNativeHls:false, updateIntervalSeconds:1, dispatchAllEmsgBoxes:false, observeQualityChanges:false, maxDisabledTime:30};
    ...
    if (Platform.isWebOS() || Platform.isTizen() || Platform.isChromecast()) {
    ...
    trackSelectionCallback:async(tracks) => tracks, downloadSizeCallback: 
    async(sizeEstimate) => {
    ...
    const abr = {enabled:true, useNetworkInformation:true, defaultBandwidthEstimate:bandwidthEstimate, switchInterval:8, bandwidthUpgradeTarget:0.85, bandwidthDowngradeTarget:0.95, restrictions:{minWidth:0, maxWidth:Infinity, minHeight:0, maxHeight:abrMaxHeight, minPixels:0, maxPixels:Infinity, minFrameRate:0, maxFrameRate:Infinity, minBandwidth:0, maxBandwidth:Infinity}, advanced:{minTotalBytes:128e3, minBytes:16e3, fastHalfLife:2, slowHalfLife:5}, restrictToElementSize:false, ignoreDevicePixelRatio:false};
    const cmcd = {enabled:false, sessionId:'', contentId:'', useHeaders:false};
    const config: shaka.extern.PlayerConfiguration = {drm:drm, manifest:manifest, streaming:streaming, offline:offline, abrFactory: 
    () => new SimpleAbrManager(), abr:abr, preferredAudioLanguage:'', preferredTextLanguage:'', preferredVariantRole:'', preferredTextRole:'', preferredAudioChannelCount:2, preferredVideoCodecs:[], preferredAudioCodecs:[], preferForcedSubs:false, preferredDecodingAttributes:[], restrictions:{minWidth:0, maxWidth:Infinity, minHeight:0, maxHeight:Infinity, minPixels:0, maxPixels:Infinity, minFrameRate:0, maxFrameRate:Infinity, minBandwidth:0, maxBandwidth:Infinity}, playRangeStart:0, playRangeEnd:Infinity, 
    textDisplayFactory: 
    () => null, cmcd:cmcd};
    ...
    // updated value.
    // eslint-disable-next-line require-await 
    offline.trackSelectionCallback =  
    async(tracks) => {
  ...
  static mergeConfigObjects(destination: shaka.extern.PlayerConfiguration, updates: Object, template?: shaka.extern.PlayerConfiguration): boolean {
    const overrides = {'.drm.keySystemsMapping':'', '.drm.servers':'', '.drm.clearKeys':'', '.drm.advanced':{distinctiveIdentifierRequired:false, persistentStateRequired:false, videoRobustness:'', audioRobustness:'', sessionType:'', serverCertificate:new Uint8Array(0), serverCertificateUri:'', individualizationServer:''}};
    return ConfigUtils.mergeConfigObjects(destination, updates, template || PlayerConfiguration.createDefault(), overrides, '');
  }
   
  static defaultTrackSelect(tracks: shaka.extern.Track[], preferredAudioLanguage: string): shaka.extern.Track[] {
    const ContentType = ManifestParserUtilsExports.ContentType;
    const LanguageUtils = LanguageUtils;
    const allVariants: shaka.extern.Track[] = tracks.filter( 
    (track) => track.type == 'variant');
    let selectedVariants: shaka.extern.Track[] = [];
     
    // Find the locale that best matches our preferred audio locale. 
    const closestLocale = LanguageUtils.findClosestLocale(preferredAudioLanguage, allVariants.map( 
    (variant) => variant.language));
     
    // If we found a locale that was close to our preference, then only use
    // variants that use that locale. 
    if (closestLocale) {
      selectedVariants = allVariants.filter( 
      (variant) => {
    ...
    if (selectedVariants.length == 0) {
      ...
      const languages = new Set(allVariants.map( 
      (track) => {
        return track.language;
      }));
      if (languages.size > 1) {
        log.warning('Could not choose a good audio track based on ' + 'language preferences or primary tracks.  An ' + 'arbitrary language will be stored!');
      }
    ...
    const tracksByHeight = selectedVariants.filter( 
    (track) => {
    ...
    if (tracksByHeight.length) {
      ...
      (a, b) => {
        ...
        asserts.assert(a.height != null, 'Null height');
        asserts.assert(b.height != null, 'Null height');
        return b.height - a.height;
      });
      selectedVariants = tracksByHeight.filter( 
      (track) => {
        return track.height == tracksByHeight[0].height;
      });
    }
    const selectedTracks: shaka.extern.Track[] = [];
