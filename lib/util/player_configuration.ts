...
export class PlayerConfiguration {
    ...
    const drm = {
      ...
      initDataTransform: (initData, initDataType, drmInfo) => {
        return ConfigUtils.referenceParametersAndReturn(
            [initData, initDataType, drmInfo], initData);
      },
  ...
  static mergeConfigObjects(
      destination: shaka.extern.PlayerConfiguration, updates: Object,
      template?: shaka.extern.PlayerConfiguration): boolean {
    const overrides = {
    ...
    return ConfigUtils.mergeConfigObjects(
        destination, updates, template || PlayerConfiguration.createDefault(),
        overrides, '');
  }

  static defaultTrackSelect(
      tracks: shaka.extern.Track[],
      preferredAudioLanguage: string): shaka.extern.Track[] {
    const ContentType = ManifestParserUtilsExports.ContentType;
    const LanguageUtils = LanguageUtils;
    const allVariants: shaka.extern.Track[] =
        tracks.filter((track) => track.type == 'variant');
    ...
    const closestLocale = LanguageUtils.findClosestLocale(
        preferredAudioLanguage, allVariants.map((variant) => variant.language));
    ...
    if (selectedVariants.length == 0) {
      ...
      if (languages.size > 1) {
        log.warning(
            'Could not choose a good audio track based on ' +
