...
export class FairPlayUtils {
  ...
  static async isFairPlaySupported(): Promise<boolean> {
    const config = {initDataTypes:['cenc', 'sinf', 'skd'], videoCapabilities:[{contentType:'video/mp4; codecs="avc1.42E01E"'}]};
    try {
  ...
  /**
     ...
     * @export
     */ 
  static initDataTransform(initData: BufferSource, contentId: BufferSource | string, cert: BufferSource | null): Uint8Array {
    if (!cert || !cert.byteLength) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.SERVER_CERTIFICATE_REQUIRED);
    }
    ...
    if (typeof contentId == 'string') {
      contentIdArray = StringUtils.toUTF16(contentId,  
      /* littleEndian= */ 
      true);
    } else {
    ...
    const utf16 = StringUtils.toUTF16(sdkUri,  
    /* littleEndian= */ 
    true);
    const rebuiltInitData = new Uint8Array(12 + utf16.byteLength + contentIdArray.byteLength + cert.byteLength);
    let offset = 0;
    const append =  
    (array: BufferSource) => {
    ...
    asserts.assert(offset == rebuiltInitData.length, 'Inconsistent init data length');
  ...
  /**
     ...
     * @export
     */ 
  static verimatrixInitDataTransform(initData: Uint8Array, initDataType: string, drmInfo: shaka.extern.DrmInfo | null) {
    if (initDataType !== 'skd') {
      return initData;
    }
  ...
  /**
     ...
     * @export
     */ 
  static ezdrmInitDataTransform(initData: Uint8Array, initDataType: string, drmInfo: shaka.extern.DrmInfo | null) {
    if (initDataType !== 'skd') {
      return initData;
    }
    const StringUtils = StringUtils;
  ...
  /**
     ...
     * @export
     */ 
  static conaxInitDataTransform(initData: Uint8Array, initDataType: string, drmInfo: shaka.extern.DrmInfo | null) {
    if (initDataType !== 'skd') {
      return initData;
    }
    const StringUtils = StringUtils;
  ...
  /**
     ...
     * @export
     */ 
  static verimatrixFairPlayRequest(type: NetworkingEngineExports.RequestType, request: shaka.extern.Request) {
  ...
  /**
     ...
     * @export
     */ 
  static ezdrmFairPlayRequest(type: NetworkingEngineExports.RequestType, request: shaka.extern.Request) {
    if (type !== NetworkingEngineExports.RequestType.LICENSE) {
      return;
    }
  ...
  /**
     ...
     * @export
     */ 
  static conaxFairPlayRequest(type: NetworkingEngineExports.RequestType, request: shaka.extern.Request) {
    if (type !== NetworkingEngineExports.RequestType.LICENSE) {
      return;
    }
    request.headers['Content-Type'] = 'application/octet-stream';
  ...
  /**
     ...
     * @export
     */ 
  static commonFairPlayResponse(type: NetworkingEngineExports.RequestType, response: shaka.extern.Response) {
    if (type !== NetworkingEngineExports.RequestType.LICENSE) {
      return;
    }
    ...
    if (licenseProcessing) {
      ...
      // directly. 
      response.data = BufferUtils.toArrayBuffer(Uint8ArrayUtils.fromBase64(responseText));
    }
  }
}
