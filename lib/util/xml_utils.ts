...
export class XmlUtils {
  ...
  /**
     ...
     * @return The attribute's value, or null if not present.
     */ 
  static getAttributeNSList(elem: Element, nsList: string[], name: string): string | null {
  ...
  /**
     ...
     * @template T
     */ 
  static parseAttr(elem: Element, name: string, parseFunction: (p1: string) => T | null, defaultValue: T | null = null): T | null {
  ...
  /**
     ...
     *
     */ 
  static parseXmlString(xmlString: string, expectedRootElemName: string): Element {
    ...
    if (xml.documentElement.tagName != expectedRootElemName) {
      log.error(`XML tag name does not match expected "${expectedRootElemName}":`, xml.documentElement.tagName);
      return null;
    }
    return rootElem;
  }
