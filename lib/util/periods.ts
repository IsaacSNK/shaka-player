...
import{MetaSegmentIndex}from './segment_index';
...
export class PeriodCombiner implements IReleasable {
  ...
  async combinePeriods(periods: Period[], isDynamic: boolean): Promise {
    ...
    if (!this.videoStreams_.length || !this.audioStreams_.length) {
      ...
      for (const stream of streams) {
        ...
        variants.push({id, language:stream.language, primary:stream.primary, audio:stream.type == ContentType.AUDIO ? stream : null, video:stream.type == ContentType.VIDEO ? stream : null, bandwidth:stream.bandwidth || 0, drmInfos:stream.drmInfos, allowedByApplication:true, allowedByKeySystem:true, decodingInfos:[]});
      }
    } else {
      for (const audio of this.audioStreams_) {
        for (const video of this.videoStreams_) {
          ...
          const id = nextVariantId++;
          variants.push({id, language:audio.language, primary:audio.primary, audio, video, bandwidth:(audio.bandwidth || 0) + (video.bandwidth || 0), drmInfos:commonDrmInfos, allowedByApplication:true, allowedByKeySystem:true, decodingInfos:[]});
        }
      }
    }
  ...
  /**
     ...
     *
     */ 
  static async combineDbStreams(streamDbsPerPeriod: shaka.extern.StreamDB[][]): Promise<shaka.extern.StreamDB[]> {
    const ContentType = ManifestParserUtilsExports.ContentType;
  ...
  /**
     ...
     *
     */ 
  private static async combine_(outputStreams: T[], streamsPerPeriod: T[][], firstNewPeriodIndex: number, clone: (p1: T) => T, concat: (p1: T, p2: T) => any): Promise<T[]> {
    const ContentType = ManifestParserUtilsExports.ContentType;
    ...
    for (const outputStream of outputStreams) {
      ...
      if (!ok) {
        ...
        // error. 
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.PERIOD_FLATTENING_FAILED);
      }
    }
    ...
    for (const unusedStreams of unusedStreamsPerPeriod) {
      for (const stream of unusedStreams) {
        ...
        const hasCodec = outputStreams.some( 
        (s) => {
          return s.mimeType == stream.mimeType && MimeUtils.getNormalizedCodec(s.codecs) == MimeUtils.getNormalizedCodec(stream.codecs);
        });
        if (!hasCodec) {
          continue;
        }
  ...
  /**
     ...
     *
     */ 
  private static async extendExistingOutputStream_(outputStream: T, streamsPerPeriod: T[][], firstNewPeriodIndex: number, concat: (p1: T, p2: T) => any, unusedStreamsPerPeriod: Set<T>[]): Promise<boolean> {
    ...
    asserts.assert(outputStream.createSegmentIndex, 'outputStream should be a Stream type!');
  ...
  /**
     ...
     *   represents the first new period that hasn't been processed yet.
     */ 
  private static async extendOutputSegmentIndex_(outputStream: shaka.extern.Stream, firstNewPeriodIndex: number) {
    const operations = [];
  ...
  /**
     ...
     *
     */ 
  private static createNewOutputStream_(stream: T, streamsPerPeriod: T[][], clone: (p1: T) => T, concat: (p1: T, p2: T) => any, unusedStreamsPerPeriod: Set<T>[]): T | null {
  ...
  /**
     ...
     *
     */ 
  private static extendOutputStream_(outputStream: T, firstNewPeriodIndex: number, concat: (p1: T, p2: T) => any, unusedStreamsPerPeriod: Set<T>[]) {
    const ContentType = ManifestParserUtilsExports.ContentType;
  ...
  /**
     ...
     *
     */ 
  private static cloneStream_(stream: shaka.extern.Stream): shaka.extern.Stream {
  ...
  /**
     ...
     *
     */ 
  private static cloneStreamDB_(streamDb: shaka.extern.StreamDB): shaka.extern.StreamDB {
  ...
  /**
     ...
     *
     */ 
  private static concatenateStreams_(output: shaka.extern.Stream, input: shaka.extern.Stream) {
    ...
    if (input.drmInfos.length && output.drmInfos.length && !commonDrmInfos.length) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.INCONSISTENT_DRM_ACROSS_PERIODS);
    }
  ...
  /**
     ...
     *
     */ 
  private static concatenateStreamDBs_(output: shaka.extern.StreamDB, input: shaka.extern.StreamDB) {
  ...
  /**
     ...
     *
     */ 
  private static findMatchesInAllPeriods_(streamsPerPeriod: T[][], outputStream: T) {
    const matches = [];
  ...
  private static findBestMatchInPeriod_(streams: T[], outputStream: T): T | null {
    const areCompatible = {'audio':PeriodCombiner.areAVStreamsCompatible_, 'video':PeriodCombiner.areAVStreamsCompatible_, 'text':PeriodCombiner.areTextStreamsCompatible_, 'image':PeriodCombiner.areImageStreamsCompatible_}[outputStream.type];
    const isBetterMatch = {'audio':PeriodCombiner.isAudioStreamBetterMatch_, 'video':PeriodCombiner.isVideoStreamBetterMatch_, 'text':PeriodCombiner.isTextStreamBetterMatch_, 'image':PeriodCombiner.isImageStreamBetterMatch_}[outputStream.type];
    let best = null;
    for (const stream of streams) {
  ...
  /**
     ...
     *
     */ 
  private static areAVStreamsCompatible_(outputStream: T, candidate: T): boolean {
    const getCodec =  
    (codecs) => MimeUtils.getNormalizedCodec(codecs);
     
    // Check MIME type and codecs, which should always be the same. 
    if (candidate.mimeType != outputStream.mimeType || getCodec(candidate.codecs) != getCodec(outputStream.codecs)) {
      return false;
    }
  ...
  /**
     ...
     *
     */ 
  private static areTextStreamsCompatible_(outputStream: T, candidate: T): boolean {
    const LanguageUtils = LanguageUtils;
  ...
  /**
     ...
     *
     */ 
  private static areImageStreamsCompatible_(outputStream: T, candidate: T): boolean {
  ...
  /**
     ...
     *
     */ 
  private static isAudioStreamBetterMatch_(outputStream: T, best: T, candidate: T): boolean {
    const LanguageUtils = LanguageUtils;
    ...
    if (outputStream.roles.length) {
      const bestRoleMatches = best.roles.filter( 
      (role) => outputStream.roles.includes(role));
      const candidateRoleMatches = candidate.roles.filter( 
      (role) => outputStream.roles.includes(role));
  ...
  /**
     ...
     *
     */ 
  private static isVideoStreamBetterMatch_(outputStream: T, best: T, candidate: T): boolean {
    const {BETTER, EQUAL, WORSE} = BetterOrWorse;
  ...
  /**
     ...
     *
     */ 
  private static isTextStreamBetterMatch_(outputStream: T, best: T, candidate: T): boolean {
    const LanguageUtils = LanguageUtils;
  ...
  /**
     ...
     *
     */ 
  private static isImageStreamBetterMatch_(outputStream: T, best: T, candidate: T): boolean {
    const {BETTER, EQUAL, WORSE} = BetterOrWorse;
  ...
  /**
     ...
     *
     */ 
  private static dummyStreamDB_(type: ManifestParserUtilsExports.ContentType): shaka.extern.StreamDB {
    return {id:0, originalId:'', primary:false, type, mimeType:'', codecs:'', language:'', label:null, width:null, height:null, encrypted:false, keyIds:new Set(), segments:[], variantIds:[], roles:[], forced:false, channelsCount:null, audioSamplingRate:null, spatialAudio:false, closedCaptions:null};
  }
  ...
  /**
     ...
     *
     */ 
  private static dummyStream_(type: ManifestParserUtilsExports.ContentType): shaka.extern.Stream {
    return {id:0, originalId:'', createSegmentIndex: 
    () => Promise.resolve(), segmentIndex:new SegmentIndex([]), mimeType:'', codecs:'', encrypted:false, drmInfos:[], keyIds:new Set(), language:'', label:null, type, primary:false, trickModeVideo:null, emsgSchemeIdUris:null, roles:[], forced:false, channelsCount:null, audioSamplingRate:null, spatialAudio:false, closedCaptions:null};
  }
  ...
  /**
     ...
     *
     */ 
  static compareClosestPreferLower(outputValue: number, bestValue: number, candidateValue: number): BetterOrWorse {
    const {BETTER, EQUAL, WORSE} = BetterOrWorse;
  ...
  private static compareClosestPreferMinimalAbsDiff_(outputValue: number, bestValue: number, candidateValue: number): BetterOrWorse {
    const {BETTER, EQUAL, WORSE} = BetterOrWorse;
...
type Period = {id:string, audioStreams:shaka.extern.Stream[], videoStreams:shaka.extern.Stream[], textStreams:shaka.extern.Stream[], imageStreams:shaka.extern.Stream[]};
