...
namespace shaka.util {
  ...
  export class PeriodCombiner implements IReleasable {
    ...
    async combinePeriods(periods: Period[], isDynamic: boolean): Promise {
      ...
      if (!isDynamic && periods.length == 1) {
        ...
        for (const textStreams of textStreamsPerPeriod) {
          textStreams.push(
            shaka.util.PeriodCombiner.dummyStream_(ContentType.TEXT)
          );
        }
        for (const imageStreams of imageStreamsPerPeriod) {
          imageStreams.push(
            shaka.util.PeriodCombiner.dummyStream_(ContentType.IMAGE)
          );
        }
      ...
      if (!this.videoStreams_.length || !this.audioStreams_.length) {
        ...
        for (const audio of this.audioStreams_) {
          for (const video of this.videoStreams_) {
            ...
            if (
              audio.drmInfos.length &&
              video.drmInfos.length &&
     ...
     *
     */
    static async combineDbStreams(
      streamDbsPerPeriod: shaka.extern.StreamDB[][]
      ...
      const audioStreamDbsPerPeriod = streamDbsPerPeriod.map((streams) =>
        streams.filter((s) => s.type == ContentType.AUDIO)
      );
      const videoStreamDbsPerPeriod = streamDbsPerPeriod.map((streams) =>
        streams.filter((s) => s.type == ContentType.VIDEO)
      );
      const textStreamDbsPerPeriod = streamDbsPerPeriod.map((streams) =>
        streams.filter((s) => s.type == ContentType.TEXT)
      );
      const imageStreamDbsPerPeriod = streamDbsPerPeriod.map((streams) =>
        streams.filter((s) => s.type == ContentType.IMAGE)
      );
      ...
      for (const textStreams of textStreamDbsPerPeriod) {
        textStreams.push(
          shaka.util.PeriodCombiner.dummyStreamDB_(ContentType.TEXT)
        );
      }
      for (const imageStreams of imageStreamDbsPerPeriod) {
        imageStreams.push(
          shaka.util.PeriodCombiner.dummyStreamDB_(ContentType.IMAGE)
        );
      }
      ...
      if (!combinedVideoStreamDbs.length || !combinedAudioStreamDbs.length) {
        // For audio-only or video-only content, just give each stream its own
        // variant ID.
        const combinedStreamDbs = combinedVideoStreamDbs.concat(
    ...
    private static async combine_(
      ...
      for (const unusedStreams of unusedStreamsPerPeriod) {
        for (const stream of unusedStreams) {
          ...
          const outputStream = shaka.util.PeriodCombiner.createNewOutputStream_(
            stream,
            streamsPerPeriod,
            clone,
            concat,
      ...
      for (const unusedStreams of unusedStreamsPerPeriod) {
        for (const stream of unusedStreams) {
          const isDummyText =
            stream.type == ContentType.TEXT && !stream.language;
          const isDummyImage =
    ...
    private static async extendExistingOutputStream_(
      ...
      shaka.util.PeriodCombiner.findMatchesInAllPeriods_(
        streamsPerPeriod,
        outputStream
      );
      ...
      if (!outputStream.matchedStreams) {
        ...
        shaka.log.error(
          "No matches extending output stream!",
          outputStream,
          streamsPerPeriod
        );
        return false;
    ...
    private static concatenateStreams_(
      ...
      if (
        input.drmInfos.length &&
        output.drmInfos.length &&
        !commonDrmInfos.length
      ) {
        throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
     ...
     *
     */
    private static findBestMatchInPeriod_(
      streams: T[],
      outputStream: T
    ): T | null {
      const areCompatible = {
     ...
     *
     */
    private static areAVStreamsCompatible_(
      outputStream: T,
      candidate: T
    ): boolean {
      ...
      if (outputStream.drmInfos) {
        ...
        if (
          !shaka.media.DrmEngine.areDrmCompatible(
            outputStream.drmInfos,
            candidate.drmInfos
          )
        ) {
          return false;
     ...
     *
     */
    private static areTextStreamsCompatible_(
      outputStream: T,
      candidate: T
    ): boolean {
      const LanguageUtils = shaka.util.LanguageUtils;
     ...
     *
     */
    private static areImageStreamsCompatible_(
      outputStream: T,
      candidate: T
    ): boolean {
    ...
    private static isAudioStreamBetterMatch_(
      ...
      if (outputStream.roles.length) {
        const bestRoleMatches = best.roles.filter((role) =>
          outputStream.roles.includes(role)
        );
        const candidateRoleMatches = candidate.roles.filter((role) =>
          outputStream.roles.includes(role)
        );
     ...
     *
     */
    private static dummyStreamDB_(type: ContentType): shaka.extern.StreamDB {
      return {
        id: 0,
        originalId: "",
        primary: false,
     ...
     *
     */
    private static dummyStream_(type: ContentType): shaka.extern.Stream {
      return {
        id: 0,
        originalId: "",
...
namespace shaka.util.PeriodCombiner {
