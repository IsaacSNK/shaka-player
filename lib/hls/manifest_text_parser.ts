 ...
 */ 
import{Attribute}from './hls_classes';
...
export class ManifestTextParser {
  ...
  parsePlaylist(data: BufferSource, absolutePlaylistUri: string): Playlist {
    ...
    if (!/^#EXTM3U($|[ \t\n])/m.test(lines[0])) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_PLAYLIST_HEADER_MISSING);
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (SEGMENT_TAGS.includes(tag.name)) {
        ...
        const segments = this.parseSegments_(absolutePlaylistUri, segmentsData, tags);
  ...
  /**
     ...
     *
     */ 
  private parseSegments_(absoluteMediaPlaylistUri: string, lines: string[], playlistTags: Tag[]): Segment[] {
    ...
    for (const line of lines) {
      if (/^(#EXT)/.test(line)) {
        ...
        if (Utils.isComment(line)) {
          ...
          const absoluteSegmentUri = Utils.constructAbsoluteUri(absoluteMediaPlaylistUri, verbatimSegmentUri);
          ...
          const segment = new Segment(absoluteSegmentUri, segmentTags, partialSegmentTags);
  ...
  static parseTag(id: number, word: string): Tag {
    ...
    if (!blocks) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.INVALID_HLS_TAG, word);
    }
    const name = blocks[1];
    const data = blocks[2];
    const attributes = [];
 ...
 *
 */ 
export const MEDIA_PLAYLIST_TAGS: string[] = ['EXT-X-TARGETDURATION', 'EXT-X-MEDIA-SEQUENCE', 'EXT-X-DISCONTINUITY-SEQUENCE', 'EXT-X-PLAYLIST-TYPE', 'EXT-X-I-FRAMES-ONLY', 'EXT-X-ENDLIST', 'EXT-X-SERVER-CONTROL', 'EXT-X-SKIP'];
 ...
 *
 */ 
export const SEGMENT_TAGS: string[] = ['EXTINF', 'EXT-X-BYTERANGE', 'EXT-X-DISCONTINUITY', 'EXT-X-PROGRAM-DATE-TIME', 'EXT-X-KEY', 'EXT-X-DATERANGE', 'EXT-X-MAP'];
