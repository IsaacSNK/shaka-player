...
namespace shaka.hls {
  ...
  export class HlsParser implements shaka.extern.ManifestParser {
    ...
    async start(uri, playerInterface) {
      goog.asserts.assert(
        this.config_,
        "Must call configure() before start()!"
      );
    ...
    private async updateStream_(streamInfo: StreamInfo): Promise {
      ...
      const variablesTags: Tag[] = shaka.hls.Utils.filterTagsByName(
        playlist.tags,
        "EXT-X-DEFINE"
      );
      ...
      const endListTag = shaka.hls.Utils.getFirstTagWithName(
        playlist.tags,
        "EXT-X-ENDLIST"
      );
      if (endListTag) {
      ...
      for (const streamInfo of this.uriToStreamInfosMap_.values()) {
        ...
        if (segmentIndex) {
          ...
          if (segment0) {
            ...
            if (goog.DEBUG) {
              const firstSequenceStartTime = streamInfo.mediaSequenceToStartTime
                .values()
                .next().value;
              goog.asserts.assert(
            ...
            for (const [
              sequence,
              start,
            ] of streamInfo.mediaSequenceToStartTime) {
    ...
    private async parseManifest_(data: BufferSource, uri: string): Promise {
      ...
      const variablesTags: Tag[] = Utils.filterTagsByName(
        playlist.tags,
        "EXT-X-DEFINE"
      );
      ...
      if (playlist.type == shaka.hls.PlaylistType.MEDIA) {
        ...
        const mediaTags: Tag[] = Utils.filterTagsByName(
          playlist.tags,
          "EXT-X-MEDIA"
        );
        const variantTags: Tag[] = Utils.filterTagsByName(
          playlist.tags,
          "EXT-X-STREAM-INF"
        );
        const imageTags: Tag[] = Utils.filterTagsByName(
          playlist.tags,
          "EXT-X-IMAGE-STREAM-INF"
        );
        this.parseCodecs_(variantTags);
        const sesionDataTags: Tag[] = Utils.filterTagsByName(
          playlist.tags,
          "EXT-X-SESSION-DATA"
        );
      ...
      await Promise.all(
        streamsToNotify.map(async (stream) => {
     ...
     */
    private async parseTexts_(
      mediaTags: Tag[]
    ): Promise<shaka.extern.Stream[]> {
      // Create text stream for each Subtitle media tag.
      const subtitleTags = shaka.hls.Utils.filterTagsByType(
        mediaTags,
        "SUBTITLES"
      );
     ...
     * @param tags Variant tags from the playlist.
     */
    private async createVariantsForTags_(
      tags: Tag[]
      ...
      const variantsPromises = tags.map(async (tag) => {
        ...
        const bandwidth =
          Number(tag.getAttributeValue("AVERAGE-BANDWIDTH")) ||
      ...
      let variants = allVariants.reduce(
    ...
    private async createStreamInfosForVariantTag_(
      ...
      if (!ignoreStream) {
        const streamInfo = await this.createStreamInfoFromVariantTag_(
          tag,
          allCodecs,
          type
        );
    ...
    private createVariants_(
      audioInfos: StreamInfo[],
      videoInfos: StreamInfo[],
      bandwidth: number,
      width: string | null,
      height: string | null,
      frameRate: string | null,
      ...
      for (const audioInfo of audioInfos) {
        for (const videoInfo of videoInfos) {
          ...
          const videoStreamUri = videoInfo
            ? videoInfo.verbatimMediaPlaylistUri
            : "";
          const audioStreamUri = audioInfo
            ? audioInfo.verbatimMediaPlaylistUri
            : "";
          ...
          const variant = {
            ...
            primary:
              (!!audioStream && audioStream.primary) ||
    ...
    private parseClosedCaptions_(mediaTags: Tag[]) {
      const closedCaptionsTags = shaka.hls.Utils.filterTagsByType(
        mediaTags,
        "CLOSED-CAPTIONS"
      );
    ...
    private async createStreamInfoFromMediaTag_(tag: Tag): Promise<StreamInfo> {
      ...
      const channelsCount =
        type == "audio" ? this.getChannelsCount_(tag) : null;
    ...
    private async createStreamInfoFromImageTag_(tag: Tag): Promise<StreamInfo> {
      ...
      const type: string = shaka.util.ManifestParserUtils.ContentType.IMAGE;
    ...
    private async convertParsedPlaylistIntoStreamInfo_(
      ...
      if (playlist.segments) {
        for (const segment of playlist.segments) {
          const segmentKeyTags = shaka.hls.Utils.filterTagsByName(
            segment.tags,
            "EXT-X-KEY"
          );
      ...
      const segmentIndex: SegmentIndex = new shaka.media.SegmentIndex(segments);
      const kind =
        type == shaka.util.ManifestParserUtils.ContentType.TEXT
          ? shaka.util.ManifestParserUtils.TextStreamKind.SUBTITLE
          : undefined;
      const roles = [];
      if (characteristics) {
    ...
    private determinePresentationType_(playlist: Playlist) {
      const PresentationType = shaka.hls.HlsParser.PresentationType_;
      const presentationTypeTag = shaka.hls.Utils.getFirstTagWithName(
        playlist.tags,
        "EXT-X-PLAYLIST-TYPE"
      );
      const endListTag = shaka.hls.Utils.getFirstTagWithName(
      ...
      const isVod =
        (presentationTypeTag && presentationTypeTag.value == "VOD") ||
        endListTag;
      const isEvent =
      ...
      if (isVod) {
        ...
        const targetDurationTag = this.getRequiredTag_(
          playlist.tags,
          "EXT-X-TARGETDURATION"
        );
        const targetDuration = Number(targetDurationTag.value);
        const partialTargetDurationTag = shaka.hls.Utils.getFirstTagWithName(
          playlist.tags,
          "EXT-X-PART-INF"
        );
    ...
    private getInitSegmentReference_(
      ...
      const mapTag: Tag | null = shaka.hls.Utils.getFirstTagWithName(
        tags,
        "EXT-X-MAP"
      );
      if (!mapTag) {
    ...
    private createSegmentReference_(
      ...
      previousReference: SegmentReference,
      hlsSegment: Segment,
      startTime: number,
      variables: Map<string, string>,
      absoluteMediaPlaylistUri: string,
      type: string,
      hlsAes128Key?: shaka.extern.HlsAes128Key
    ): SegmentReference {
      const tags = hlsSegment.tags;
      const absoluteSegmentUri = this.variableSubstitution_(
        hlsSegment.absoluteUri,
        variables
      );
      const extinfTag = shaka.hls.Utils.getFirstTagWithName(tags, "EXTINF");
      let endTime = 0;
      let startByte = 0;
      let endByte = null;
      ...
      let syncTime = null;
      if (!this.config_.hls.ignoreManifestProgramDateTime) {
        const dateTimeTag = shaka.hls.Utils.getFirstTagWithName(
          tags,
          "EXT-X-PROGRAM-DATE-TIME"
        );
      ...
      if (!extinfTag) {
        if (hlsSegment.partialSegments.length == 0) {
          ...
          throw new shaka.util.Error(
            shaka.util.Error.Severity.CRITICAL,
            shaka.util.Error.Category.MANIFEST,
            shaka.util.Error.Code.HLS_REQUIRED_TAG_MISSING,
            "EXTINF"
          );
        } else {
      ...
      if (this.lowLatencyMode_) {
        for (let i = 0; i < hlsSegment.partialSegments.length; i++) {
          ...
          const pDuration =
            Number(item.getAttributeValue("DURATION")) ||
            this.partialTargetDuration_;
      ...
      if (type == shaka.util.ManifestParserUtils.ContentType.IMAGE) {
        ...
        tilesLayout = "1x1";
        const tilesTag = shaka.hls.Utils.getFirstTagWithName(
          tags,
          "EXT-X-TILES"
        );
        if (tilesTag) {
    ...
    private createSegments_(
      ...
      const skippedSegments = skipTag
        ? Number(skipTag.getAttributeValue("SKIPPED-SEGMENTS"))
        : 0;
      ...
      for (let i = 0; i < hlsSegments.length; i++) {
        ...
        if (!this.lowLatencyMode_) {
          const autoLowLatencyMode =
            this.playerInterface_.isAutoLowLatencyMode();
      ...
      if (uriVariables) {
        for (const variable of uriVariables) {
          ...
          if (replaceValue) {
            ...
            shaka.log.error(
              "A variable has been found that is not declared",
              variableName
            );
            throw new shaka.util.Error(
              shaka.util.Error.Severity.CRITICAL,
    ...
    private async guessMimeType_(
      ...
      const middleSegmentUri = this.variableSubstitution_(
        playlist.segments[middleSegmentIdx].absoluteUri,
        variables
      );
    ...
    private static fairplayDrmParser_(
      ...
      if (shaka.util.Platform.isMediaKeysPolyfilled()) {
        throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.MANIFEST,
          shaka.util.Error.Code.HLS_MSE_ENCRYPTED_LEGACY_APPLE_MEDIA_KEYS_NOT_SUPPORTED
        );
      }
    ...
    private static widevineDrmParser_(
      drmTag: Tag
    ): shaka.extern.DrmInfo | null {
      const method = drmTag.getRequiredAttrValue("METHOD");
     ...
     *
     */
    private static playreadyDrmParser_(
      drmTag: Tag
    ): shaka.extern.DrmInfo | null {
      const method = drmTag.getRequiredAttrValue("METHOD");
      const VALID_METHODS = ["SAMPLE-AES", "SAMPLE-AES-CTR"];
     ...
     *
     */
    private static identityDrmParser_(
      drmTag: Tag
    ): shaka.extern.DrmInfo | null {
      const method = drmTag.getRequiredAttrValue("METHOD");
      const VALID_METHODS = ["SAMPLE-AES", "SAMPLE-AES-CTR"];
...
namespace shaka.hls.HlsParser {
  export const KEYFORMATS_TO_DRM_PARSERS_: { [key: string]: DrmParser_ } = {
    "com.apple.streamingkeydelivery": shaka.hls.HlsParser.fairplayDrmParser_,
    "urn:uuid:edef8ba9-79d6-4ace-a3c8-27dcd51d21ed":
      shaka.hls.HlsParser.widevineDrmParser_,
    "com.microsoft.playready": shaka.hls.HlsParser.playreadyDrmParser_,
    identity: shaka.hls.HlsParser.identityDrmParser_,
  };
}
