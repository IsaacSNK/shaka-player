...
import*as ManifestTextParserExports from './manifest_text_parser';
import{Playlist}from './hls_classes';
...
import{DataUriPlugin}from './data_uri_plugin';
...
import{Segment}from './hls_classes';
 ...
 * @export
 */ 
export class HlsParser implements shaka.extern.ManifestParser {
  ...
  private async updateStream_(streamInfo: StreamInfo): Promise {
    ...
    const playlist: Playlist = this.manifestTextParser_.parsePlaylist(response.data, response.uri);
    if (playlist.type != PlaylistType.MEDIA) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_INVALID_PLAYLIST_HIERARCHY);
    }
    const variablesTags: Tag[] = Utils.filterTagsByName(playlist.tags, 'EXT-X-DEFINE');
    const mediaVariables = this.parseMediaVariables_(variablesTags);
    const stream = streamInfo.stream;
    const segments = this.createSegments_(streamInfo.verbatimMediaPlaylistUri, playlist, stream.type, stream.mimeType, streamInfo.mediaSequenceToStartTime, mediaVariables, stream.codecs);
    stream.segmentIndex.mergeAndEvict(segments, this.presentationTimeline_.getSegmentAvailabilityStart());
    if (segments.length) {
      const mediaSequenceNumber = Utils.getFirstTagWithNameAsNumber(playlist.tags, 'EXT-X-MEDIA-SEQUENCE', 0);
      const playlistStartTime = streamInfo.mediaSequenceToStartTime.get(mediaSequenceNumber);
  ...
  private syncStreamsWithSequenceNumber_() {
    ...
    for (const streamInfo of this.uriToStreamInfosMap_.values()) {
      ...
      if (segmentIndex) {
        ...
        if (segment0) {
          ...
          if (goog.DEBUG) {
            const firstSequenceStartTime = streamInfo.mediaSequenceToStartTime.values().next().value;
            asserts.assert(firstSequenceStartTime == segment0.startTime, 'Sequence number map is not ordered as expected!');
          }
          for (const [sequence, start] of streamInfo.mediaSequenceToStartTime) {
            if (start == segment0.startTime) {
              ...
              highestStartingSequenceNumber = Math.max(highestStartingSequenceNumber, sequence);
              break;
            }
          }
        }
    ...
    log.debug('Syncing HLS streams against base sequence number:', this.minSequenceNumber_);
    for (const streamInfo of this.uriToStreamInfosMap_.values()) {
      const segmentIndex = streamInfo.stream.segmentIndex;
      if (segmentIndex) {
         
        // Drop any earlier references. 
        const numSegmentsToDrop = this.minSequenceNumber_ - firstSequenceNumberMap.get(streamInfo);
    ...
    for (const streamInfo of this.uriToStreamInfosMap_.values()) {
      ...
      if (segmentIndex != null) {
        ...
        if (segment0.syncTime == null) {
          log.alwaysError('Missing EXT-X-PROGRAM-DATE-TIME for stream', streamInfo.verbatimMediaPlaylistUri, 'Expect AV sync issues!');
        } else {
  ...
  private offsetStream_(streamInfo: StreamInfo, offset: number) {
    ...
    asserts.assert(streamInfo.maxTimestamp >= 0, 'Negative maxTimestamp after adjustment!');
    for (const [key, value] of streamInfo.mediaSequenceToStartTime) {
      streamInfo.mediaSequenceToStartTime.set(key, value + offset);
    }
    log.debug('Offset', offset, 'applied to', streamInfo.verbatimMediaPlaylistUri);
  }
  ...
  private async parseManifest_(data: BufferSource, uri: string): Promise {
    const Utils = Utils;
    asserts.assert(this.masterPlaylistUri_, 'Master playlist URI must be set before calling parseManifest_!');
    const playlist = this.manifestTextParser_.parsePlaylist(data, this.masterPlaylistUri_);
    const variablesTags: Tag[] = Utils.filterTagsByName(playlist.tags, 'EXT-X-DEFINE');
    ...
    if (playlist.type == PlaylistType.MEDIA) {
      ...
      const streamInfo = await this.convertParsedPlaylistIntoStreamInfo_(playlist, uri, uri, codecs, type, language, primary, name, channelsCount, closedCaptions, characteristics, forced, spatialAudio, mimeType);
      this.uriToStreamInfosMap_.set(uri, streamInfo);
       
      // Wrap the stream from that stream info with a variant. 
      variants.push({id:0, language:'und', disabledUntilTime:0, primary:true, audio:type == 'audio' ? streamInfo.stream : null, video:type == 'video' ? streamInfo.stream : null, bandwidth:0, allowedByApplication:true, allowedByKeySystem:true, decodingInfos:[]});
    } else {
      const mediaTags: Tag[] = Utils.filterTagsByName(playlist.tags, 'EXT-X-MEDIA');
      const variantTags: Tag[] = Utils.filterTagsByName(playlist.tags, 'EXT-X-STREAM-INF');
      const imageTags: Tag[] = Utils.filterTagsByName(playlist.tags, 'EXT-X-IMAGE-STREAM-INF');
      this.parseCodecs_(variantTags);
      const sesionDataTags: Tag[] = Utils.filterTagsByName(playlist.tags, 'EXT-X-SESSION-DATA');
      for (const tag of sesionDataTags) {
        ...
        if (uri) {
          data.set('uri', Utils.constructAbsoluteUri(this.masterPlaylistUri_, uri));
        }
        if (language) {
    ...
    asserts.assert(this.presentationTimeline_ == null, 'Presentation timeline created early!');
    this.createPresentationTimeline_();
     
    // This assert satisfies the compiler that it is not null for the rest of
    // the method. 
    asserts.assert(this.presentationTimeline_, 'Presentation timeline not created!');
    if (this.isLive_()) {
      ...
      if (this.presentationType_ == PresentationType.LIVE) {
        ...
        this.presentationTimeline_.setSegmentAvailabilityDuration(segmentAvailabilityDuration);
      }
    } else {
    ...
    await Promise.all(streamsToNotify.map( 
    async(stream) => {
    ...
    asserts.assert(!this.presentationTimeline_.usingPresentationStartTime(), 'We should not be using the presentation start time in HLS!');
    this.manifest_ = {presentationTimeline:this.presentationTimeline_, variants, textStreams, imageStreams, offlineSessionIds:[], minBufferTime:0, sequenceMode:true};
  ...
  private parseCodecs_(tags: Tag[]) {
    ...
    for (const variantTag of tags) {
      ...
      if (subGroupId) {
        const textCodecs = ManifestParserUtils.guessCodecsSafe(ContentType.TEXT, allCodecs);
      ...
      if (audioGroupId) {
        const codecs = ManifestParserUtils.guessCodecs(ContentType.AUDIO, allCodecs);
        this.groupIdToCodecsMap_.set(audioGroupId, codecs);
      }
      if (videoGroupId) {
        const codecs = ManifestParserUtils.guessCodecs(ContentType.VIDEO, allCodecs);
  ...
  private async parseTexts_(mediaTags: Tag[]): Promise<shaka.extern.Stream[]> {
    ...
    const textStreamPromises = subtitleTags.map( 
    async(tag) => {
  ...
  private async parseImages_(imageTags: Tag[]): Promise<shaka.extern.Stream[]> {
    ...
    const imageStreamPromises = imageTags.map( 
    async(tag) => {
  ...
  /**
     * @param tags Variant tags from the playlist.
     */ 
  private async createVariantsForTags_(tags: Tag[]): Promise<shaka.extern.Variant[]> {
     
    // Create variants for each variant tag. 
    const variantsPromises = tags.map( 
    async(tag) => {
      const frameRate = tag.getAttributeValue('FRAME-RATE');
      const bandwidth = Number(tag.getAttributeValue('AVERAGE-BANDWIDTH')) || Number(tag.getRequiredAttrValue('BANDWIDTH'));
      ...
      const streamInfos = await this.createStreamInfosForVariantTag_(tag, resolution, frameRate);
      asserts.assert(streamInfos.audio.length || streamInfos.video.length, 'We should have created a stream!');
      return this.createVariants_(streamInfos.audio, streamInfos.video, bandwidth, width, height, frameRate, videoRange);
    });
  ...
  /**
     ...
     *
     */ 
  private async createStreamInfosForVariantTag_(tag: Tag, resolution: string | null, frameRate: string | null): Promise<StreamInfos> {
    const ContentType = ManifestParserUtilsExports.ContentType;
    ...
    asserts.assert(audioGroupId == null || videoGroupId == null, 'Unexpected: both video and audio described by media tags!');
    const groupId = audioGroupId || videoGroupId;
    const streamInfos = groupId && this.groupIdToStreamInfosMap_.has(groupId) ? this.groupIdToStreamInfosMap_.get(groupId) : [];
    const res: StreamInfos = {audio:audioGroupId ? streamInfos : [], video:videoGroupId ? streamInfos : []};
    ...
    const hasSameUri = res.audio.find( 
    (audio) => {
      return audio && audio.verbatimMediaPlaylistUri == streamURI;
    });
    const videoCodecs = ManifestParserUtils.guessCodecsSafe(ContentType.VIDEO, allCodecs);
    const audioCodecs = ManifestParserUtils.guessCodecsSafe(ContentType.AUDIO, allCodecs);
    ...
    if (!ignoreStream) {
      const streamInfo = await this.createStreamInfoFromVariantTag_(tag, allCodecs, type);
  ...
  private getClosedCaptions_(tag: Tag, type: string): Map<string, string> {
    ...
    if (type == ContentType.VIDEO && closedCaptionsAttr && closedCaptionsAttr != 'NONE') {
  ...
  private createVariants_(audioInfos: StreamInfo[], videoInfos: StreamInfo[], bandwidth: number, width: string | null, height: string | null, frameRate: string | null, videoRange: string | null): shaka.extern.Variant[] {
    const ContentType = ManifestParserUtilsExports.ContentType;
    const DrmEngine = DrmEngine;
    for (const info of videoInfos) {
      this.addVideoAttributes_(info.stream, width, height, frameRate, videoRange);
    }
    ...
    for (const audioInfo of audioInfos) {
      for (const videoInfo of videoInfos) {
        ...
        const videoStreamUri = videoInfo ? videoInfo.verbatimMediaPlaylistUri : '';
        const audioStreamUri = audioInfo ? audioInfo.verbatimMediaPlaylistUri : '';
        ...
        asserts.assert(!audioStream || audioStream.type == ContentType.AUDIO, 'Audio parameter mismatch!');
        asserts.assert(!videoStream || videoStream.type == ContentType.VIDEO, 'Video parameter mismatch!');
        const variant = {id:this.globalId_++, language:audioStream ? audioStream.language : 'und', primary:!!audioStream && audioStream.primary || !!videoStream && videoStream.primary, audio:audioStream, video:videoStream, bandwidth, allowedByApplication:true, allowedByKeySystem:true, decodingInfos:[]};
  ...
  private parseClosedCaptions_(mediaTags: Tag[]) {
    const closedCaptionsTags = Utils.filterTagsByType(mediaTags, 'CLOSED-CAPTIONS');
    for (const tag of closedCaptionsTags) {
      asserts.assert(tag.name == 'EXT-X-MEDIA', 'Should only be called on media tags!');
  ...
  private async createStreamInfoFromMediaTag_(tag: Tag): Promise<StreamInfo> {
    ...
    if (type != ManifestParserUtilsExports.ContentType.TEXT && groupId && this.groupIdToCodecsMap_.has(groupId)) {
      codecs = this.groupIdToCodecsMap_.get(groupId);
    }
    const verbatimMediaPlaylistUri = this.variableSubstitution_(tag.getRequiredAttrValue('URI'), this.globalVariables_);
    ...
    const streamInfo = await this.createStreamInfo_(verbatimMediaPlaylistUri, codecs, type, language, primary, name, channelsCount,  
    /* closedCaptions= */ 
  ...
  private async createStreamInfoFromImageTag_(tag: Tag): Promise<StreamInfo> {
    asserts.assert(tag.name == 'EXT-X-IMAGE-STREAM-INF', 'Should only be called on image tags!');
    const type: string = ManifestParserUtilsExports.ContentType.IMAGE;
    const verbatimImagePlaylistUri = this.variableSubstitution_(tag.getRequiredAttrValue('URI'), this.globalVariables_);
    ...
    const name = tag.getAttributeValue('NAME');
    const characteristics = tag.getAttributeValue('CHARACTERISTICS');
    const streamInfo = await this.createStreamInfo_(verbatimImagePlaylistUri, codecs, type, language,  
    /* primary= */ 
    false, name,  
    /* channelsCount= */ 
    null,  
    ...
    if (resolution) {
      ...
      if (layout) {
        streamInfo.stream.width = Number(resolution.split('x')[0]) * Number(layout.split('x')[0]);
        streamInfo.stream.height = Number(resolution.split('x')[1]) * Number(layout.split('x')[1]);
      }
    }
  ...
  /**
     ...
     *
     */ 
  private async createStreamInfoFromVariantTag_(tag: Tag, allCodecs: string[], type: string): Promise<StreamInfo> {
    asserts.assert(tag.name == 'EXT-X-STREAM-INF', 'Should only be called on variant tags!');
    const verbatimMediaPlaylistUri = this.variableSubstitution_(tag.getRequiredAttrValue('URI'), this.globalVariables_);
    if (this.uriToStreamInfosMap_.has(verbatimMediaPlaylistUri)) {
      return this.uriToStreamInfosMap_.get(verbatimMediaPlaylistUri);
    }
    ...
    const streamInfo = await this.createStreamInfo_(verbatimMediaPlaylistUri, codecs, type,  
    /* language= */ 
    'und',  
    /* primary= */ 
    false,  
  ...
  private async createStreamInfo_(verbatimMediaPlaylistUri: string, codecs: string, type: string, language: string, primary: boolean, name: string | null, channelsCount: number | null, closedCaptions: Map<string, string>, characteristics: string | null, forced: boolean, spatialAudio: boolean): Promise<StreamInfo> {
     
    // TODO: Refactor, too many parameters 
    let absoluteMediaPlaylistUri = Utils.constructAbsoluteUri(this.masterPlaylistUri_, verbatimMediaPlaylistUri);
    ...
    const playlist: Playlist = this.manifestTextParser_.parsePlaylist(response.data, absoluteMediaPlaylistUri);
    return this.convertParsedPlaylistIntoStreamInfo_(playlist, verbatimMediaPlaylistUri, absoluteMediaPlaylistUri, codecs, type, language, primary, name, channelsCount, closedCaptions, characteristics, forced, spatialAudio);
  }
   
  private async convertParsedPlaylistIntoStreamInfo_(playlist: Playlist, verbatimMediaPlaylistUri: string, absoluteMediaPlaylistUri: string, codecs: string, type: string, language: string, primary: boolean, name: string | null, channelsCount: number | null, closedCaptions: Map<string, string>, characteristics: string | null, forced: boolean, spatialAudio: boolean, mimeType: string | undefined = undefined): Promise<StreamInfo> {
    if (playlist.type != PlaylistType.MEDIA) {
    ...
    const variablesTags: Tag[] = Utils.filterTagsByName(playlist.tags, 'EXT-X-DEFINE');
    const mediaVariables = this.parseMediaVariables_(variablesTags);
    asserts.assert(playlist.segments != null, 'Media playlist should have segments!');
    this.determinePresentationType_(playlist);
    if (!mimeType) {
      mimeType = await this.guessMimeType_(type, codecs, playlist, mediaVariables);
    }
    const drmTags: Tag[] = [];
    if (playlist.segments) {
      for (const segment of playlist.segments) {
        const segmentKeyTags = Utils.filterTagsByName(segment.tags, 'EXT-X-KEY');
    ...
    if (encrypted && !drmInfos.length && !aesEncrypted) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_KEYFORMATS_NOT_SUPPORTED);
    }
    ...
    const segments = this.createSegments_(verbatimMediaPlaylistUri, playlist, type, mimeType, mediaSequenceToStartTime, mediaVariables, codecs);
    const lastEndTime = segments[segments.length - 1].endTime;
    const segmentIndex: SegmentIndex = new SegmentIndex(segments);
    const kind = type == ManifestParserUtilsExports.ContentType.TEXT ? ManifestParserUtilsExports.TextStreamKind.SUBTITLE : undefined;
    const roles = [];
    if (characteristics) {
    ...
    const serverControlTag = Utils.getFirstTagWithName(playlist.tags, 'EXT-X-SERVER-CONTROL');
    const canSkipSegments = serverControlTag ? serverControlTag.getAttribute('CAN-SKIP-UNTIL') != null : false;
    const stream: shaka.extern.Stream = {id:this.globalId_++, originalId:name, createSegmentIndex: 
    () => Promise.resolve(), segmentIndex, mimeType, codecs, kind, encrypted, drmInfos, keyIds, language, label:name,  
    // For historical reasons, since before "originalId". 
    type, primary,  
    // TODO: trick mode 
    trickModeVideo:null, emsgSchemeIdUris:null, frameRate:undefined, pixelAspectRatio:undefined, width:undefined, height:undefined, bandwidth:undefined, roles:roles, forced:forced, channelsCount, audioSamplingRate:null, spatialAudio:spatialAudio, closedCaptions, hdr:undefined, tilesLayout:undefined};
    return {stream, verbatimMediaPlaylistUri, absoluteMediaPlaylistUri, maxTimestamp:lastEndTime, mediaSequenceToStartTime, canSkipSegments};
  }
   
  private parseAES128DrmTag_(drmTag: Tag, playlist: Playlist): shaka.extern.HlsAes128Key {
     
    // Check if the Web Crypto API is available. 
    if (!window.crypto || !window.crypto.subtle) {
      log.alwaysWarn('Web Crypto API is not available to decrypt ' + 'AES-128. (Web Crypto only exists in secure origins like https)');
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.NO_WEB_CRYPTO_API);
    }
    ...
    if (!ivHex) {
      ...
      firstMediaSequenceNumber = Utils.getFirstTagWithNameAsNumber(playlist.tags, 'EXT-X-MEDIA-SEQUENCE', 0);
    } else {
       
      // Exclude 0x at the start of string. 
      iv = Uint8ArrayUtils.fromHex(ivHex.substr(2));
      if (iv.byteLength != 16) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_AES_128_INVALID_IV_LENGTH);
      }
    }
    const keyUri = Utils.constructAbsoluteUri(playlist.absoluteUri, drmTag.getRequiredAttrValue('URI'));
    const requestType = NetworkingEngineExports.RequestType.KEY;
    const request = NetworkingEngine.makeRequest([keyUri], this.config_.retryParameters);
    ...
    async() => {
      ...
      if (!keyResponse.data || keyResponse.data.byteLength != 16) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_AES_128_INVALID_KEY_LENGTH);
      }
      keyInfo.cryptoKey = await window.crypto.subtle.importKey('raw', keyResponse.data, 'AES-CBC', true, ['decrypt']);
  ...
  private determinePresentationType_(playlist: Playlist) {
    const PresentationType = PresentationType_;
    const presentationTypeTag = Utils.getFirstTagWithName(playlist.tags, 'EXT-X-PLAYLIST-TYPE');
    const endListTag = Utils.getFirstTagWithName(playlist.tags, 'EXT-X-ENDLIST');
    const isVod = presentationTypeTag && presentationTypeTag.value == 'VOD' || endListTag;
    const isEvent = presentationTypeTag && presentationTypeTag.value == 'EVENT' && !isVod;
    ...
    if (isVod) {
      ...
      const targetDurationTag = this.getRequiredTag_(playlist.tags, 'EXT-X-TARGETDURATION');
      const targetDuration = Number(targetDurationTag.value);
      const partialTargetDurationTag = Utils.getFirstTagWithName(playlist.tags, 'EXT-X-PART-INF');
      ...
      if (this.lowLatencyMode_ && partialTargetDurationTag) {
        ...
        this.partialTargetDuration_ = Number(partialTargetDurationTag.getRequiredAttrValue('PART-TARGET'));
        this.minTargetDuration_ = Math.min(this.partialTargetDuration_, this.minTargetDuration_);
        ...
        this.lowLatencyPresentationDelay_ = serverControlTag ? Number(serverControlTag.getRequiredAttrValue('PART-HOLD-BACK')) : 0;
      } else {
  ...
  /**
     ...
     */ 
  private getInitSegmentReference_(playlistUri: string, tags: Tag[], variables: Map<string, string>): InitSegmentReference {
    ...
    const absoluteInitSegmentUri = this.variableSubstitution_(Utils.constructAbsoluteUri(playlistUri, verbatimInitSegmentUri), variables);
    const mapTagKey = [absoluteInitSegmentUri, mapTag.getAttributeValue('BYTERANGE', '')].join('-');
    if (!this.mapTagToInitSegmentRefMap_.has(mapTagKey)) {
      const initSegmentRef = this.createInitSegmentReference_(absoluteInitSegmentUri, mapTag);
  ...
  /**
     ...
     */ 
  private createInitSegmentReference_(absoluteInitSegmentUri: string, mapTag: Tag): InitSegmentReference {
    let startByte = 0;
    let endByte = null;
  ...
  /**
     ...
     *
     */ 
  private createSegmentReference_(initSegmentReference: InitSegmentReference, previousReference: SegmentReference, hlsSegment: Segment, startTime: number, variables: Map<string, string>, absoluteMediaPlaylistUri: string, type: string, hlsAes128Key?: shaka.extern.HlsAes128Key): SegmentReference {
    const tags = hlsSegment.tags;
    const absoluteSegmentUri = this.variableSubstitution_(hlsSegment.absoluteUri, variables);
    const extinfTag = Utils.getFirstTagWithName(tags, 'EXTINF');
    let endTime = 0;
    let startByte = 0;
    let endByte = null;
    if (hlsSegment.partialSegments.length && !this.lowLatencyMode_) {
      log.alwaysWarn('Low-latency HLS live stream detected, but ' + 'low-latency streaming mode is not enabled in Shaka ' + 'Player. Set streaming.lowLatencyMode configuration to ' + 'true, and see https://bit.ly/3clctcj for details.');
    }
    let syncTime = null;
    if (!this.config_.hls.ignoreManifestProgramDateTime) {
      const dateTimeTag = Utils.getFirstTagWithName(tags, 'EXT-X-PROGRAM-DATE-TIME');
      if (dateTimeTag && dateTimeTag.value) {
        syncTime = XmlUtils.parseDate(dateTimeTag.value);
        asserts.assert(syncTime != null, 'EXT-X-PROGRAM-DATE-TIME format not valid');
      }
    }
    ...
    if (!extinfTag) {
      if (hlsSegment.partialSegments.length == 0) {
        ...
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_REQUIRED_TAG_MISSING, 'EXTINF');
      } else {
    ...
    if (this.lowLatencyMode_) {
      for (let i = 0; i < hlsSegment.partialSegments.length; i++) {
        ...
        const pPreviousReference = i == 0 ? previousReference : partialSegmentRefs[partialSegmentRefs.length - 1];
        ...
        const pDuration = Number(item.getAttributeValue('DURATION')) || this.partialTargetDuration_;
        ...
        if (item.name == 'EXT-X-PRELOAD-HINT') {
          ...
          [pStartByte, pEndByte] = this.parseByteRange_(pPreviousReference, pByterange);
        }
        const pUri = item.getAttributeValue('URI');
        if (!pUri) {
          continue;
        }
        const pAbsoluteUri = Utils.constructAbsoluteUri(absoluteMediaPlaylistUri, pUri);
        ...
        const partial = new SegmentReference(pStartTime, pEndTime,  
        () => [pAbsoluteUri], pStartByte, pEndByte, initSegmentReference,  
        /* timestampOffset= */ 
        0,  
    ...
    if (byterangeTag) {
      [startByte, endByte] = this.parseByteRange_(previousReference, byterangeTag.value);
    } else {
    ...
    return new SegmentReference(startTime, endTime,  
    () => absoluteSegmentUri.length ? [absoluteSegmentUri] : [], startByte, endByte, initSegmentReference,  
    /* timestampOffset= */ 
    0,  
    // This value is ignored in sequence mode. 
    0,  
    ...
    Infinity,  
    /* appendWindowEnd= */ 
    partialSegmentRefs, tilesLayout, tileDuration, syncTime, status, hlsAes128Key);
  }
  ...
  /**
     ...
     */ 
  private parseByteRange_(previousReference: SegmentReference, byterange: string | null): number[] {
    let startByte = 0;
    let endByte = null;
    ...
    if (byterange) {
      ...
      if (blocks[1]) {
        ...
        asserts.assert(previousReference, 'Cannot refer back to previous HLS segment!');
  ...
  /**
     ...
     *
     */ 
  private createSegments_(verbatimMediaPlaylistUri: string, playlist: Playlist, type: string, mimeType: string, mediaSequenceToStartTime: Map<number, number>, variables: Map<string, string>, codecs: string): SegmentReference[] {
    ...
    const skippedSegments = skipTag ? Number(skipTag.getAttributeValue('SKIPPED-SEGMENTS')) : 0;
    ...
    for (let i = 0; i < hlsSegments.length; i++) {
      ...
      for (const drmTag of item.tags) {
        if (drmTag.name == 'EXT-X-KEY' && drmTag.getRequiredAttrValue('METHOD') == 'AES-128') {
          hlsAes128Key = this.parseAES128DrmTag_(drmTag, playlist);
        }
      }
      mediaSequenceToStartTime.set(position, startTime);
      initSegmentRef = this.getInitSegmentReference_(playlist.absoluteUri, item.tags, variables);
      ...
      const reference = this.createSegmentReference_(initSegmentRef, previousReference, item, startTime, variables, playlist.absoluteUri, type, hlsAes128Key);
      previousReference = reference;
      if (reference) {
         
        // This segment is ignored as part of our fallback synchronization
        // method. 
        if (this.config_.hls.ignoreManifestProgramDateTime && this.minSequenceNumber_ != null && position < this.minSequenceNumber_) {
        } else {
    ...
    const someSyncTime = references.some( 
    (ref) => ref.syncTime != null);
    if (someSyncTime) {
  ...
  /**
     ...
     *
     */ 
  private variableSubstitution_(uri: string, variables: Map<string, string>): string {
    ...
    if (uriVariables) {
      for (const variable of uriVariables) {
        ...
        if (replaceValue) {
          ...
          log.error('A variable has been found that is not declared', variableName);
          throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_VARIABLE_NOT_FOUND, variableName);
        }
      }
    }
    return newUri;
  ...
  /**
     ...
     *
     */ 
  private async guessMimeType_(contentType: string, codecs: string, playlist: Playlist, variables: Map<string, string>): Promise<string> {
    ...
    const middleSegmentUri = this.variableSubstitution_(playlist.segments[middleSegmentIdx].absoluteUri, variables);
    ...
    // from the response. 
    const headRequest = NetworkingEngine.makeRequest([middleSegmentUri], this.config_.retryParameters);
  ...
  private getRequiredTag_(tags: Tag[], tagName: string): Tag {
    ...
    if (!tag) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_REQUIRED_TAG_MISSING, tagName);
    }
    return tag;
  }
   
  private addVideoAttributes_(stream: shaka.extern.Stream, width: string | null, height: string | null, frameRate: string | null, videoRange: string | null) {
    if (stream) {
  ...
  /**
     ...
     *
     */ 
  private requestManifest_(absoluteUri: string): Promise<shaka.extern.Response> {
    const requestType = NetworkingEngineExports.RequestType.MANIFEST;
    const request = NetworkingEngine.makeRequest([absoluteUri], this.config_.retryParameters);
  ...
  private async onUpdate_(): Promise {
    ...
    asserts.assert(this.updatePlaylistDelay_ > 0, 'We should only call |onUpdate_| when we are suppose to be updating.');
  ...
  /**
     ...
     *
     */ 
  private makeNetworkRequest_(request: shaka.extern.Request, type: NetworkingEngineExports.RequestType): Promise<shaka.extern.Response> {
    if (!this.operationManager_) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.PLAYER, ErrorExports.Code.OPERATION_ABORTED);
    }
  ...
  private static fairplayDrmParser_(drmTag: Tag, mimeType: string): shaka.extern.DrmInfo | null {
    if (mimeType == 'video/mp2t') {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_MSE_ENCRYPTED_MP2T_NOT_SUPPORTED);
    }
    if (Platform.isMediaKeysPolyfilled()) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.HLS_MSE_ENCRYPTED_LEGACY_APPLE_MEDIA_KEYS_NOT_SUPPORTED);
    }
    ...
    /*
         ...
         */ 
    const drmInfo = ManifestParserUtils.createDrmInfo('com.apple.fps', [{initDataType:'sinf', initData:new Uint8Array(0), keyId:null}]);
    return drmInfo;
  }
  ...
  private static widevineDrmParser_(drmTag: Tag): shaka.extern.DrmInfo | null {
    ...
    if (!VALID_METHODS.includes(method)) {
      log.error('Widevine in HLS is only supported with [', VALID_METHODS.join(', '), '], not', method);
      return null;
    }
    ...
    const drmInfo = ManifestParserUtils.createDrmInfo('com.widevine.alpha', [{initDataType:'cenc', initData:pssh}]);
    const keyId = drmTag.getAttributeValue('KEYID');
    if (keyId) {
      const keyIdLowerCase = keyId.toLowerCase();
       
      // This value should begin with '0x': 
      asserts.assert(keyIdLowerCase.startsWith('0x'), 'Incorrect KEYID format!');
  ...
  private static playreadyDrmParser_(drmTag: Tag): shaka.extern.DrmInfo | null {
    ...
    const VALID_METHODS = ['SAMPLE-AES', 'SAMPLE-AES-CTR'];
    if (!VALID_METHODS.includes(method)) {
      log.error('PlayReady in HLS is only supported with [', VALID_METHODS.join(', '), '], not', method);
      return null;
    }
    const uri = drmTag.getRequiredAttrValue('URI');
    const parsedData = DataUriPlugin.parseRaw(uri);
    ...
    const systemId = new Uint8Array([154, 4, 240, 121, 152, 64, 66, 134, 171, 146, 230, 91, 224, 136, 95, 149]);
    const keyIds = new Set();
    const psshVersion = 0;
    const pssh = Pssh.createPssh(data, systemId, keyIds, psshVersion);
    const drmInfo = ManifestParserUtils.createDrmInfo('com.microsoft.playready', [{initDataType:'cenc', initData:pssh}]);
    return drmInfo;
  }
  ...
  private static identityDrmParser_(drmTag: Tag): shaka.extern.DrmInfo | null {
    ...
    const VALID_METHODS = ['SAMPLE-AES', 'SAMPLE-AES-CTR'];
    if (!VALID_METHODS.includes(method)) {
      log.error('Identity (ClearKey) in HLS is only supported with [', VALID_METHODS.join(', '), '], not', method);
      return null;
    }
    ...
    return ManifestParserUtils.createDrmInfo('org.w3.clearkey',  
    /* initDatas= */ 
    null);
  }
}
type StreamInfo = {stream:shaka.extern.Stream, verbatimMediaPlaylistUri:string, absoluteMediaPlaylistUri:string, maxTimestamp:number, mediaSequenceToStartTime:Map<number, number>, canSkipSegments:boolean};
 
export{StreamInfo};
type StreamInfos = {audio:StreamInfo[], video:StreamInfo[]};
 
export{StreamInfos};
 
export const AUDIO_EXTENSIONS_TO_MIME_TYPES_: {[key:string]:string} = {'mp4':'audio/mp4', 'mp4a':'audio/mp4', 'm4s':'audio/mp4', 'm4i':'audio/mp4', 'm4a':'audio/mp4', 'm4f':'audio/mp4', 'cmfa':'audio/mp4',  
// MPEG2-TS also uses video/ for audio: https://bit.ly/TsMse 
'ts':'video/mp2t',  
// Raw formats: 
'aac':'audio/aac', 'ac3':'audio/ac3', 'ec3':'audio/ec3', 'mp3':'audio/mpeg'};
 
export const VIDEO_EXTENSIONS_TO_MIME_TYPES_: {[key:string]:string} = {'mp4':'video/mp4', 'mp4v':'video/mp4', 'm4s':'video/mp4', 'm4i':'video/mp4', 'm4v':'video/mp4', 'm4f':'video/mp4', 'cmfv':'video/mp4', 'ts':'video/mp2t'};
 
export const TEXT_EXTENSIONS_TO_MIME_TYPES_: {[key:string]:string} = {'mp4':'application/mp4', 'm4s':'application/mp4', 'm4i':'application/mp4', 'm4f':'application/mp4', 'cmft':'application/mp4', 'vtt':'text/vtt', 'ttml':'application/ttml+xml'};
 
export const IMAGE_EXTENSIONS_TO_MIME_TYPES_: {[key:string]:string} = {'jpg':'image/jpeg', 'png':'image/png', 'svg':'image/svg+xml', 'webp':'image/webp', 'avif':'image/avif'};
 
export const EXTENSION_MAP_BY_CONTENT_TYPE_: {[key:string]:{[key:string]:string}} = {'audio':AUDIO_EXTENSIONS_TO_MIME_TYPES_, 'video':VIDEO_EXTENSIONS_TO_MIME_TYPES_, 'text':TEXT_EXTENSIONS_TO_MIME_TYPES_, 'image':IMAGE_EXTENSIONS_TO_MIME_TYPES_};
type DrmParser_ = (p1: Tag, p2: string) => shaka.extern.DrmInfo | null;
 
export{DrmParser_};
 
export const KEYFORMATS_TO_DRM_PARSERS_: {[key:string]:DrmParser_} = {'com.apple.streamingkeydelivery':HlsParser.fairplayDrmParser_, 'urn:uuid:edef8ba9-79d6-4ace-a3c8-27dcd51d21ed':HlsParser.widevineDrmParser_, 'com.microsoft.playready':HlsParser.playreadyDrmParser_, 'identity':HlsParser.identityDrmParser_};
...
if (!Platform.isTizen3() && !Platform.isTizen2() && !Platform.isWebOS3()) {
  ManifestParser.registerParserByExtension('m3u8',  
  () => new HlsParser());
  ManifestParser.registerParserByMime('application/x-mpegurl',  
  () => new HlsParser());
  ManifestParser.registerParserByMime('application/vnd.apple.mpegurl',  
  () => new HlsParser());
}
