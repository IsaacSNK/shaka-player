...
export class Playlist {
   ...
   */
  constructor(
      public readonly absoluteUri: string, type: PlaylistType, tags: Tag[],
      segments?: Segment[]) {
    this.type = type;
    this.tags = tags;
...
export class Tag {
  ...
  constructor(
      public readonly id: number, public name: string,
   ...
   */
  toString(): string {
    const attrToStr = (attr: Attribute): string => {
  ...
  getRequiredAttrValue(name: string): string {
    ...
    if (!attribute) {
      throw new Error(
          ...
          ErrorExports.Code.HLS_REQUIRED_ATTRIBUTE_MISSING, name);
    }
...
export class Segment {
   ...
   */
  constructor(
      public readonly absoluteUri: string, public readonly tags: Tag[],
      public partialSegments: Tag[] = []) {}
}
