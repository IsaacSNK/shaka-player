...
export class Walker implements IDestroyable {
   ...
   */
  constructor(
      startingAt: Node, startingWith: Payload, implementation: Implementation) {
  ...
  private doOneThing_(): Promise {
    ...
    asserts.assert(
        this.waitForWork_ == null, 'We should not have a promise yet.');
  ...
  private async takeNextStep_(): Promise {
    asserts.assert(
        this.currentRoute_, 'We need a current route to take the next step.');

    // Figure out where we are supposed to go next.
    this.currentlyAt_ = this.implementation_.getNext(
        this.currentlyAt_, this.currentlyWith_, this.currentRoute_.node,
    ...
    try {
      ...
      this.currentlyAt_ =
          await this.implementation_.handleError(this.currentlyWith_, error);
    }
  }
...
type Implementation = {
  getNext: (p1: Node, p2: Payload, p3: Node, p4: Payload) => Node,
  enterNode: (p1: Node, p2: Payload, p3: Payload) => AbortableOperation,
  handleError: (p1: Payload, p2: Error) => Promise<Node>,
  onIdle: (p1: Node) => any
};
