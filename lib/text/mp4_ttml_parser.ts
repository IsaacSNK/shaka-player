 ...
 * @export
 */ 
export class Mp4TtmlParser implements shaka.extern.TextParser {
  ...
  parseInit(data) {
    ...
    let sawSTPP = false;
    (new Mp4Parser()).box('moov', Mp4Parser.children).box('trak', Mp4Parser.children).box('mdia', Mp4Parser.children).box('minf', Mp4Parser.children).box('stbl', Mp4Parser.children).fullBox('stsd', Mp4Parser.sampleDescription).box('stpp',  
    (box) => {
      sawSTPP = true;
      box.parser.stop();
    }).parse(data);
    if (!sawSTPP) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_MP4_TTML);
    }
  }
  ...
  parseMedia(data, time) {
    ...
    let sawMDAT = false;
    let payload = [];
    const parser = (new Mp4Parser()).box('mdat', Mp4Parser.allData( 
    (data) => {
      sawMDAT = true;
...
TextEngine.registerParser('application/mp4; codecs="stpp"',  
() => new Mp4TtmlParser());
TextEngine.registerParser('application/mp4; codecs="stpp.ttml"',  
() => new Mp4TtmlParser());
TextEngine.registerParser('application/mp4; codecs="stpp.ttml.im1t"',  
() => new Mp4TtmlParser());
