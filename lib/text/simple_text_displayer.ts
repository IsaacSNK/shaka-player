...
namespace shaka.text {
  ...
  export class SimpleTextDisplayer implements shaka.extern.TextDisplayer {
    ...
    private static convertToTextTrackCue_(
      ...
      const vttCue: VTTCue = new VTTCue(
        shakaCue.startTime,
        shakaCue.endTime,
        shakaCue.payload
      );
      ...
      if (shakaCue.writingMode == Cue.writingMode.VERTICAL_LEFT_TO_RIGHT) {
        ...
        if (shakaCue.writingMode == Cue.writingMode.VERTICAL_RIGHT_TO_LEFT) {
          vttCue.vertical = "rl";
        }
      }
    ...
    private static removeWhere_(
      track: TextTrack,
      predicate: (p1: TextTrackCue) => boolean
    ) {
