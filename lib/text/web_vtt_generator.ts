...
export class WebVttGenerator {
  ...
  static convert(cues: Cue[], adCuePoints: shaka.extern.AdCuePoint[]): string {
    ...
    (time) => {
      ...
      return (hours < 10 ? '0' : '') + hours + ':' + (minutes < 10 ? '0' : '') + minutes + ':' + (seconds < 10 ? '0' : '') + seconds + '.' + (milliseconds < 100 ? milliseconds < 10 ? '00' : '0' : '') + milliseconds;
    };
    ...
    for (const cue of flattenedCues) {
      ...
      webvttString += webvttTimeString(cue.startTime) + ' --\x3e ' + webvttTimeString(cue.endTime) + webvttSettings(cue) + '\n';
