 ...
 */ 
import{ParsedTRUNSample}from './mp4_box_parsers';
...
import*as VttTextParserExports from './vtt_text_parser';
...
import{Mp4BoxParsers}from './mp4_box_parsers';
...
export class Mp4VttParser implements shaka.extern.TextParser {
  ...
  parseInit(data) {
    ...
    let sawWVTT = false;
    (new Mp4Parser()).box('moov', Mp4Parser.children).box('trak', Mp4Parser.children).box('mdia', Mp4Parser.children).fullBox('mdhd',  
    (box) => {
      asserts.assert(box.version == 0 || box.version == 1, 'MDHD version can only be 0 or 1');
      const parsedMDHDBox = Mp4BoxParsers.parseMDHD(box.reader, box.version);
      this.timescale_ = parsedMDHDBox.timescale;
    }).box('minf', Mp4Parser.children).box('stbl', Mp4Parser.children).fullBox('stsd', Mp4Parser.sampleDescription).box('wvtt',  
    (box) => {
    ...
    if (!this.timescale_) {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_MP4_VTT);
    }
    if (!sawWVTT) {
  ...
  parseMedia(data, time) {
    ...
    const parser = (new Mp4Parser()).box('moof', Mp4Parser.children).box('traf', Mp4Parser.children).fullBox('tfdt',  
    (box) => {
      sawTFDT = true;
      asserts.assert(box.version == 0 || box.version == 1, 'TFDT version can only be 0 or 1');
      const parsedTFDTBox = Mp4BoxParsers.parseTFDT(box.reader, box.version);
      baseTime = parsedTFDTBox.baseMediaDecodeTime;
    }).fullBox('tfhd',  
    (box) => {
      asserts.assert(box.flags != null, 'A TFHD box should have a valid flags value');
      const parsedTFHDBox = Mp4BoxParsers.parseTFHD(box.reader, box.flags);
      defaultDuration = parsedTFHDBox.defaultSampleDuration;
    }).fullBox('trun',  
    (box) => {
      sawTRUN = true;
      asserts.assert(box.version != null, 'A TRUN box should have a valid version value');
      asserts.assert(box.flags != null, 'A TRUN box should have a valid flags value');
      const parsedTRUNBox = Mp4BoxParsers.parseTRUN(box.reader, box.version, box.flags);
      presentations = parsedTRUNBox.sampleData;
    }).box('mdat', Mp4Parser.allData( 
    (data) => {
      asserts.assert(!sawMDAT, 'VTT cues in mp4 with multiple MDAT are not currently supported');
      sawMDAT = true;
      rawPayload = data;
    }));
    parser.parse(data,  
    ...
    const reader: DataViewReader = new DataViewReader(rawPayload, DataViewReaderExports.Endianness.BIG_ENDIAN);
    for (const presentation of presentations) {
      ...
      const startTime = presentation.sampleCompositionTimeOffset ? baseTime + presentation.sampleCompositionTimeOffset : currentTime;
      ...
      do {
        ...
        if (duration) {
          if (payload) {
            asserts.assert(this.timescale_ != null, 'Timescale should not be null!');
            const cue = Mp4VttParser.parseVTTC_(payload, time.periodStart + startTime / this.timescale_, time.periodStart + currentTime / this.timescale_);
            cues.push(cue);
          }
        } else {
          log.error('WVTT sample duration unknown, and no default found!');
        }
        asserts.assert(!presentation.sampleSize || totalSize <= presentation.sampleSize, 'The samples do not fit evenly into the sample sizes given in ' + 'the TRUN box!');
      } while ( 
    ...
    asserts.assert(!reader.hasMoreData(), 'MDAT which contain VTT cues and non-VTT data are not currently ' + 'supported!');
  ...
  /**
     ...
     *
     */ 
  private static parseVTTC_(data: Uint8Array, startTime: number, endTime: number): Cue {
    let payload;
    let id;
    let settings;
    (new Mp4Parser()).box('payl', Mp4Parser.allData( 
    (data) => {
      payload = StringUtils.fromUTF8(data);
    })).box('iden', Mp4Parser.allData( 
    (data) => {
      id = StringUtils.fromUTF8(data);
    })).box('sttg', Mp4Parser.allData( 
    (data) => {
      settings = StringUtils.fromUTF8(data);
    })).parse(data);
    if (payload) {
      return Mp4VttParser.assembleCue_(payload, id, settings, startTime, endTime);
    } else {
      return null;
    }
  }
  ...
  /**
     ...
     *
     */ 
  private static assembleCue_(payload: string, id: string | null, settings: string | null, startTime: number, endTime: number): Cue {
    ...
    if (settings) {
      ...
      while (word) {
        ...
        if (!VttTextParser.parseCueSetting(cue, word,  
        /* VTTRegions= */ 
        [])) {
...
TextEngine.registerParser('application/mp4; codecs="wvtt"',  
() => new Mp4VttParser());
