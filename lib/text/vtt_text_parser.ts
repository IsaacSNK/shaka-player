  ...
  export class VttTextParser implements shaka.extern.TextParser {
    ...
    parseMedia(data, time) {
      ...
      if (blocks[0].includes("X-TIMESTAMP-MAP") && this.sequenceMode_) {
        ...
        const cueTimeMatch = blocks[0].match(
    ...
    private static parseRegion_(text: string): shaka.extern.CueRegion {
      ...
      while (word) {
        if (!VttTextParser.parseRegionSetting_(region, word)) {
          shaka.log.warning(
            "VTT parser encountered an invalid VTTRegion setting: ",
            word,
      ...
      for (let i = 0; i < propertyLines.length; i++) {
        ...
        if (lineParts) {
          ...
          switch (name) {
            ...
            default:
              shaka.log.warning(
                "VTT parser encountered an unsupported style: ",
                lineParts
              );
              break;
          }
    ...
    private static parseCue_(
      text: string[],
      timeOffset: number,
      regions: shaka.extern.CueRegion[],
          ...
          if ((results = /^size:([\d.]+)%$/.exec(word))) {
            ...
            if (
              ...
              if ((results = /^region:(.*)$/.exec(word))) {
                ...
                return VttTextParser.parsedLineValueAndInterpretation_(
                  cue,
                  word
                );
              }
    ...
    private static getRegionById_(
      ...
      if (!regionsWithId.length) {
        shaka.log.warning(
          "VTT parser could not find a region with id: ",
          id,
     ...
     * @return True on success.
     */
    private static parseRegionSetting_(
      region: CueRegion,
      word: string
    ): boolean {
      let results = null;
...
namespace shaka.text.VttTextParser {
