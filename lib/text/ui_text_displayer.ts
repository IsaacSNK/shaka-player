 ...
 * @export
 */
export class UITextDisplayer implements shaka.
extern.TextDisplayer {
   ...
   * }>}
   */
  private currentCuesMap_: Map<shaka.extern.Cue, {
    cueElement: HTMLElement,
    regionElement: HTMLElement,
    wrapper: HTMLElement
  }>;
  ...
  append(cues) {
    ...
    for (const cue of cues) {
      ...
      const containsCue =
          cuesList.some((cueInList) => Cue.equal(cueInList, cue));
      if (!containsCue) {
        this.cues_.push(cue);
      }
    }
  ...
  remove(start, end) {
    ...
    this.cues_ =
        this.cues_.filter((cue) => cue.startTime < start || cue.endTime >= end);
  ...
  private updateCuesRecursive_(
      cues: shaka.extern.Cue[], container: HTMLElement, currentTime: number,
  ...
  private updateCaptions_(forceUpdate: boolean = false) {
    ...
    if (this.isTextVisible_) {
      ...
      // Update the cues.
      this.updateCuesRecursive_(
          this.cues_, this.textContainer_, currentTime,
          /* parents= */
          []);
      if (goog.DEBUG) {
  ...
  private setCaptionStyles_(
      cueElement: HTMLElement, cue: shaka.extern.Cue,
      parents: shaka.extern.Cue[], hasWrapper: boolean) {
    const Cue = Cue;
    const inherit = (cb) => UITextDisplayer.inheritProperty_(parents, cb);
    ...
    style.paddingLeft = UITextDisplayer.convertLengthValue_(
        cue.linePadding, cue, this.videoContainer_);
    style.paddingRight = UITextDisplayer.convertLengthValue_(
        cue.linePadding, cue, this.videoContainer_);
    ...
    style.fontSize = UITextDisplayer.convertLengthValue_(
   ...
   *
   */
  private static getLengthValueInfo_(lengthValue: string|
                                     null): {value: number, unit: string}|null {
  ...
  private static convertLengthValue_(
      lengthValue: string, cue: shaka.extern.Cue,
    ...
    switch (unit) {
      case '%':
        return UITextDisplayer.getAbsoluteLengthInPixels_(
  ...
  private static getAbsoluteLengthInPixels_(
      value: number, cue: shaka.extern.Cue,
      videoContainer: HTMLElement): string {
  ...
  private static inheritProperty_(
      parents: shaka.extern.Cue[], cb: (p1: shaka.extern.Cue) => T | null): T|
      null {
