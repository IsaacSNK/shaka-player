...
export class UITextDisplayer implements shaka.extern.TextDisplayer {
  ...
  /**
       ...
       * }>}
       */ 
  private currentCuesMap_: Map<shaka.extern.Cue, {cueElement:HTMLElement, regionElement:HTMLElement, wrapper:HTMLElement}>;
  ...
  constructor(video: HTMLMediaElement, videoContainer: HTMLElement) {
    asserts.assert(videoContainer, 'videoContainer should be valid.');
    this.video_ = video;
    this.videoContainer_ = videoContainer;
    this.textContainer_ = Dom.createHTMLElement('div');
  ...
  append(cues) {
    ...
    for (const cue of cues) {
      ...
      (cueInList) => Cue.equal(cueInList, cue));
      if (!containsCue) {
        this.cues_.push(cue);
      }
    }
  ...
  private updateCuesRecursive_(cues: shaka.extern.Cue[], container: HTMLElement, currentTime: number, parents: shaka.extern.Cue[]) {
    ...
    for (const cue of cues) {
      ...
      asserts.assert(topCue == cue, 'Parent cues should be kept in order');
    }
    if (updateDOM) {
      ...
      for (const cue of toPlant) {
        ...
        asserts.assert(cueRegistry, 'cueRegistry should exist.');
        if (cueRegistry.regionElement) {
  ...
  private updateCaptions_(forceUpdate: boolean = false) {
    ...
    if (!this.isTextVisible_ || forceUpdate) {
      ...
      for (const regionElement of this.regionElements_.values()) {
        Dom.removeAllChildren(regionElement);
      }
       
      // Remove all top-level elements in the text container. 
      Dom.removeAllChildren(this.textContainer_);
      ...
      if (goog.DEBUG) {
        ...
        for (const cue of previousCuesMap.keys()) {
          if (!this.currentCuesMap_.has(cue)) {
            ...
            asserts.assert(!cueElement.parentNode, 'Cue was not properly removed!');
          }
        }
      }
    }
  ...
  private getRegionElement_(cue: shaka.extern.Cue): HTMLElement {
    ...
    const regionElement = Dom.createHTMLElement('span');
    const percentageUnit = CueRegionExports.units.PERCENTAGE;
    ...
    if (cue.displayAlign == CueExports.displayAlign.BEFORE) {
      regionElement.style.justifyContent = 'flex-start';
    } else {
      if (cue.displayAlign == CueExports.displayAlign.CENTER) {
  ...
  private createCue_(cue: shaka.extern.Cue, parents: shaka.extern.Cue[]) {
    ...
    const cueElement = Dom.createHTMLElement(type);
    if (type != 'br') {
    ...
    if (needWrapper) {
      ...
      wrapper = Dom.createHTMLElement('span');
  ...
  private setCaptionStyles_(cueElement: HTMLElement, cue: shaka.extern.Cue, parents: shaka.extern.Cue[], hasWrapper: boolean) {
    const Cue = Cue;
    const inherit =  
    (cb) => UITextDisplayer.inheritProperty_(parents, cb);
    ...
    const text = cue.payload.replace(/\s+$/g,  
    (match) => {
    ...
    style.paddingLeft = UITextDisplayer.convertLengthValue_(cue.linePadding, cue, this.videoContainer_);
    style.paddingRight = UITextDisplayer.convertLengthValue_(cue.linePadding, cue, this.videoContainer_);
    ...
    if (cue.backgroundImage) {
      ...
      if (cue.nestedCues.length) {
        ...
        elem = Dom.createHTMLElement('span');
    ...
    style.fontSize = UITextDisplayer.convertLengthValue_(cue.fontSize, cue, this.videoContainer_);
  ...
  /**
     ...
     *
     */ 
  private static getLengthValueInfo_(lengthValue: string | null): {value:number, unit:string} | null {
  ...
  /**
     ...
     *
    */ 
  private static convertLengthValue_(lengthValue: string, cue: shaka.extern.Cue, videoContainer: HTMLElement): string {
    const lengthValueInfo = UITextDisplayer.getLengthValueInfo_(lengthValue);
    if (!lengthValueInfo) {
      return lengthValue;
    }
    ...
    switch(unit) {
      case '%':
        return UITextDisplayer.getAbsoluteLengthInPixels_(value / 100, cue, videoContainer);
      case 'c':
        return UITextDisplayer.getAbsoluteLengthInPixels_(value, cue, videoContainer);
      default:
        return lengthValue;
    }
  }
  ...
  /**
     ...
     *
     * */ 
  private static getAbsoluteLengthInPixels_(value: number, cue: shaka.extern.Cue, videoContainer: HTMLElement): string {
  ...
  /**
     ...
     * @template T
     */ 
  private static inheritProperty_(parents: shaka.extern.Cue[], cb: (p1: shaka.extern.Cue) => T | null): T | null {
