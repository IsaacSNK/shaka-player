...
export class TextEngine implements IDestroyable {
  ...
  /**
     * @export
     */ 
  static registerParser(mimeType: string, plugin: shaka.extern.TextParserPlugin) {
    parserMap_[mimeType] = plugin;
  }
  ...
  static findParser(mimeType): shaka.extern.TextParserPlugin | null {
    return parserMap_[mimeType];
  }
   
  static isTypeSupported(mimeType: string): boolean {
    if (parserMap_[mimeType]) {
       
      // An actual parser is available. 
      return true;
    }
    if (mimeType == MimeUtilsExports.CEA608_CLOSED_CAPTION_MIMETYPE || mimeType == MimeUtilsExports.CEA708_CLOSED_CAPTION_MIMETYPE) {
  ...
  /**
     ...
     *
     */ 
  initParser(mimeType: string, sequenceMode: boolean, segmentRelativeVttTiming: boolean) {
    ...
    const factory = parserMap_[mimeType];
    ...
    if (this.parser_.setSequenceMode) {
      ...
      log.alwaysWarn('Text parsers should have a "setSequenceMode" method!');
    }
  ...
  /**
     ...
     */ 
  async appendBuffer(buffer: BufferSource, startTime: number | null, endTime: number | null): Promise {
    ...
    const time: shaka.extern.TextParser.TimeContext = {periodStart:this.timestampOffset_, segmentStart:startTime, segmentEnd:endTime, vttOffset:vttOffset};
     
    // Parse the buffer and add the new cues. 
    const allCues = this.parser_.parseMedia(BufferUtils.toUint8(buffer), time);
    const cuesToAppend = allCues.filter( 
    (cue) => {
  ...
  setSelectedClosedCaptionId(id: string, bufferEndTime: number) {
    ...
    if (captionsMap) {
      for (const startAndEndTime of captionsMap.keys()) {
        const cues: Cue[] = captionsMap.get(startAndEndTime).filter( 
        (c) => c.endTime <= bufferEndTime);
        if (cues) {
  ...
  convertMuxjsCaptionsToShakaCaptions(closedCaptions: muxjs.mp4.ClosedCaption[]): ICaptionDecoderExports.ClosedCaption[] {
    const cues = [];
  ...
  /**
     ...
     */ 
  private applyVideoTimestampOffsetRecursive_(cue: Cue, videoTimestampOffset: number) {
  ...
  /**
     ...
     */ 
  storeAndAppendClosedCaptions(closedCaptions: ICaptionDecoderExports.ClosedCaption[], startTime: number | null, endTime: number | null, videoTimestampOffset: number) {
