...
export class TextEngine implements IDestroyable {
  ...
  static registerParser(
      mimeType: string, plugin: shaka.extern.TextParserPlugin) {
  ...
  static isTypeSupported(mimeType: string): boolean {
    ...
    if (mimeType == MimeUtilsExports.CEA608_CLOSED_CAPTION_MIMETYPE ||
        mimeType == MimeUtilsExports.CEA708_CLOSED_CAPTION_MIMETYPE) {
      // Closed captions.
      return true;
    }
    return false;
   ...
   */
  initParser(
      mimeType: string, sequenceMode: boolean,
   ...
   */
  async appendBuffer(
      buffer: BufferSource, startTime: number|null,
      endTime: number|null): Promise {
    ...
    const vttOffset =
        this.segmentRelativeVttTiming_ ? startTime : this.timestampOffset_;
          ...
          if (startTime <= this.bufferStart_ && endTime >= this.bufferEnd_) {
            ...
            if (startTime <= this.bufferStart_ && endTime < this.bufferEnd_) {
              ...
              if (startTime > this.bufferStart_ && endTime >= this.bufferEnd_) {
                ...
                // We removed from the middle?  StreamingEngine isn't supposed
                // to.
                asserts.assert(
                    false,
  ...
  setSelectedClosedCaptionId(id: string, bufferEndTime: number) {
    ...
    if (captionsMap) {
      for (const startAndEndTime of captionsMap.keys()) {
        const cues: Cue[] = captionsMap.get(startAndEndTime)
  ...
  private applyVideoTimestampOffsetRecursive_(
      cue: Cue, videoTimestampOffset: number) {
  ...
  storeAndAppendClosedCaptions(
      closedCaptions: ICaptionDecoderExports.ClosedCaption[],
    ...
    for (const caption of closedCaptions) {
      ...
      const keepThisCue = cue.startTime >= this.appendWindowStart_ &&
          cue.startTime < this.appendWindowEnd_;
      if (!keepThisCue) {
        continue;
      }
    ...
    this.bufferEnd_ =
        Math.max(this.bufferEnd_, Math.min(endTime, this.appendWindowEnd_));
  }
