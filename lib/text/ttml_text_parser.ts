 ...
 * @export
 */ 
export class TtmlTextParser implements shaka.extern.TextParser {
  ...
  parseMedia(data, time) {
    ...
    try {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_XML, 'Failed to parse TTML.');
    }
    if (xml) {
      const parserError = xml.getElementsByTagName('parsererror')[0];
      if (parserError) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_XML, parserError.textContent);
      }
      const tt = xml.getElementsByTagName('tt')[0];
       
      // TTML should always have tt element. 
      if (!tt) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_XML, 'TTML does not contain <tt> tag.');
      }
      ...
      const subFrameRate = XmlUtils.getAttributeNSList(tt, ttpNs, 'subFrameRate');
      const frameRateMultiplier = XmlUtils.getAttributeNSList(tt, ttpNs, 'frameRateMultiplier');
      const tickRate = XmlUtils.getAttributeNSList(tt, ttpNs, 'tickRate');
      const cellResolution = XmlUtils.getAttributeNSList(tt, ttpNs, 'cellResolution');
      ...
      if (spaceStyle != 'default' && spaceStyle != 'preserve') {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_XML, 'Invalid xml:space value: ' + spaceStyle);
      }
      const whitespaceTrim = spaceStyle == 'default';
      const rateInfo = new TtmlTextParser.RateInfo_(frameRate, subFrameRate, frameRateMultiplier, tickRate);
      const cellResolutionInfo = TtmlTextParser.getCellResolution_(cellResolution);
      ...
      for (const region of regionElements) {
        const cueRegion = TtmlTextParser.parseCueRegion_(region, styles, extent);
        if (cueRegion) {
      ...
      if (XmlUtils.findChildren(body, 'p').length) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_TEXT_CUE, '<p> can only be inside <div> in TTML');
      }
      for (const div of XmlUtils.findChildren(body, 'div')) {
        ...
        if (XmlUtils.findChildren(div, 'span').length) {
          throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_TEXT_CUE, '<span> can only be inside <p> in TTML');
        }
      }
      const cue = TtmlTextParser.parseCue_(body, time.periodStart, rateInfo, metadataElements, styles, regionElements, cueRegions, whitespaceTrim, cellResolutionInfo,  
      /* parentCueElement= */ 
      null,  
      /* isContent= */ 
      false);
  ...
  /**
     ...
     *
     */ 
  private static parseCue_(cueNode: Node, offset: number, rateInfo: RateInfo_, metadataElements: Element[], styles: Element[], regionElements: Element[], cueRegions: CueRegion[], whitespaceTrim: boolean, cellResolution: {columns:number, rows:number} | null, parentCueElement: Element | null, isContent: boolean): Cue {
    ...
    if (cueNode.nodeType == Node.TEXT_NODE) {
      ...
      asserts.assert(cueNode.nodeType == Node.ELEMENT_NODE, 'nodeType should be ELEMENT_NODE!');
    ...
    const spaceStyle = cueElement.getAttribute('xml:space') || (whitespaceTrim ? 'default' : 'preserve');
    ...
    if (!isLeafNode) {
      ...
      for (const childNode of cueElement.childNodes) {
        const nestedCue = TtmlTextParser.parseCue_(childNode, offset, rateInfo, metadataElements, styles, regionElements, cueRegions, localWhitespaceTrim, cellResolution, cueElement, isContent);
    ...
    const hasTimeAttributes = cueElement.hasAttribute('begin') || cueElement.hasAttribute('end') || cueElement.hasAttribute('dur');
    if (!hasTimeAttributes && !hasTextContent && cueElement.tagName != 'br' && nestedCues.length == 0) {
      if (!isNested) {
    ...
    while (parentElement && parentElement.nodeType == Node.ELEMENT_NODE && parentElement.tagName != 'tt') {
    ...
    const regionElement = TtmlTextParser.getElementsFromCollection_(cueElement, 'region', regionElements,  
    /* prefix= */ 
    '')[0];
    ...
    if (cueElement.hasAttribute('region')) {
      if (regionElement && regionElement.getAttribute('xml:id')) {
        ...
        cue.region = cueRegions.filter( 
        (region) => region.id == regionId)[0];
      }
    }
    let regionElementForStyle = regionElement;
    if (parentCueElement && isNested && !cueElement.getAttribute('region') && !cueElement.getAttribute('style')) {
      regionElementForStyle = TtmlTextParser.getElementsFromCollection_(parentCueElement, 'region', regionElements,  
      /* prefix= */ 
      '')[0];
    }
    TtmlTextParser.addStyle_(cue, cueElement, regionElementForStyle, imageElement, styles, parentIsContent,  
    /** isNested= */
  ...
  /**
     ...
     */ 
  private static parseCueRegion_(regionElement: Element, styles: Element[], globalExtent: string | null): CueRegion {
    const TtmlTextParser = TtmlTextParser;
    ...
    if (globalExtent) {
      globalResults = TtmlTextParser.percentValues_.exec(globalExtent) || TtmlTextParser.pixelValues_.exec(globalExtent);
    }
    const globalWidth = globalResults ? Number(globalResults[1]) : null;
    const globalHeight = globalResults ? Number(globalResults[2]) : null;
    let results = null;
    let percentage = null;
    const extent = TtmlTextParser.getStyleAttributeFromRegion_(regionElement, styles, 'extent');
    if (extent) {
      ...
      if (results != null) {
        ...
        region.widthUnits = percentage || globalWidth != null ? CueRegionExports.units.PERCENTAGE : CueRegionExports.units.PX;
        region.heightUnits = percentage || globalHeight != null ? CueRegionExports.units.PERCENTAGE : CueRegionExports.units.PX;
      }
    }
    const origin = TtmlTextParser.getStyleAttributeFromRegion_(regionElement, styles, 'origin');
    if (origin) {
      ...
      if (results != null) {
        ...
        region.viewportAnchorUnits = percentage || globalWidth != null ? CueRegionExports.units.PERCENTAGE : CueRegionExports.units.PX;
      }
    }
    return region;
  }
  ...
  /**
     ...
     *
     */ 
  private static addStyle_(cue: Cue, cueElement: Element, region: Element, imageElement: Element, styles: Element[], isNested: boolean, isLeaf: boolean) {
    const TtmlTextParser = TtmlTextParser;
    const Cue = Cue;
    ...
    const direction = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'direction', shouldInheritRegionStyles);
    if (direction == 'rtl') {
    ...
    const writingMode = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'writingMode', shouldInheritRegionStyles);
    ...
    const align = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'textAlign', true);
    if (align) {
    ...
    const displayAlign = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'displayAlign', true);
    if (displayAlign) {
      asserts.assert(displayAlign.toUpperCase() in Cue.displayAlign, displayAlign.toUpperCase() + ' Should be in Cue.displayAlign values!');
      cue.displayAlign = Cue.displayAlign[displayAlign.toUpperCase()];
    }
    const color = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'color', shouldInheritRegionStyles);
    if (color) {
      cue.color = color;
    }
    ...
    const backgroundColor = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'backgroundColor', shouldInheritRegionStyles);
    if (backgroundColor) {
      cue.backgroundColor = backgroundColor;
    }
    const border = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'border', shouldInheritRegionStyles);
    if (border) {
      cue.border = border;
    }
    const fontFamily = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontFamily', shouldInheritRegionStyles);
    if (fontFamily) {
      cue.fontFamily = fontFamily;
    }
    const fontWeight = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontWeight', shouldInheritRegionStyles);
    if (fontWeight && fontWeight == 'bold') {
      cue.fontWeight = Cue.fontWeight.BOLD;
    }
    const wrapOption = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'wrapOption', shouldInheritRegionStyles);
    if (wrapOption && wrapOption == 'noWrap') {
      cue.wrapLine = false;
    } else {
      cue.wrapLine = true;
    }
    const lineHeight = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'lineHeight', shouldInheritRegionStyles);
    if (lineHeight && lineHeight.match(TtmlTextParser.unitValues_)) {
      cue.lineHeight = lineHeight;
    }
    const fontSize = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontSize', shouldInheritRegionStyles);
    if (fontSize) {
      const isValidFontSizeUnit = fontSize.match(TtmlTextParser.unitValues_) || fontSize.match(TtmlTextParser.percentValue_);
      if (isValidFontSizeUnit) {
        cue.fontSize = fontSize;
      }
    }
    const fontStyle = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontStyle', shouldInheritRegionStyles);
    if (fontStyle) {
      asserts.assert(fontStyle.toUpperCase() in Cue.fontStyle, fontStyle.toUpperCase() + ' Should be in Cue.fontStyle values!');
    ...
    if (imageElement) {
      ...
      const backgroundImageType = imageElement.getAttribute('imageType') || imageElement.getAttribute('imagetype');
      const backgroundImageEncoding = imageElement.getAttribute('encoding');
      const backgroundImageData = imageElement.textContent.trim();
      if (backgroundImageType == 'PNG' && backgroundImageEncoding == 'Base64' && backgroundImageData) {
        cue.backgroundImage = 'data:image/png;base64,' + backgroundImageData;
      }
    }
    const textOutline = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'textOutline', shouldInheritRegionStyles);
    if (textOutline) {
    ...
    const letterSpacing = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'letterSpacing', shouldInheritRegionStyles);
    if (letterSpacing && letterSpacing.match(TtmlTextParser.unitValues_)) {
      cue.letterSpacing = letterSpacing;
    }
    const linePadding = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'linePadding', shouldInheritRegionStyles);
    if (linePadding && linePadding.match(TtmlTextParser.unitValues_)) {
      cue.linePadding = linePadding;
    }
    const opacity = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'opacity', shouldInheritRegionStyles);
    if (opacity) {
    ...
    const textDecorationRegion = TtmlTextParser.getStyleAttributeFromRegion_(region, styles, 'textDecoration');
    if (textDecorationRegion) {
      TtmlTextParser.addTextDecoration_(cue, textDecorationRegion);
    }
    const textDecorationElement = TtmlTextParser.getStyleAttributeFromElement_(cueElement, styles, 'textDecoration');
  ...
  private static addTextDecoration_(cue: Cue, decoration: string) {
    ...
    for (const value of decoration.split(' ')) {
      switch(value) {
        ...
        case 'noLineThrough':
          if (cue.textDecoration.includes(Cue.textDecoration.LINE_THROUGH)) {
            ArrayUtils.remove(cue.textDecoration, Cue.textDecoration.LINE_THROUGH);
          }
          break;
        case 'overline':
  ...
  /**
     ...
     *
     */ 
  private static getStyleAttribute_(cueElement: Element, region: Element, styles: Element[], attribute: string, shouldInheritRegionStyles: boolean = true): string | null {
     
    // An attribute can be specified on region level or in a styling block
    // associated with the region or original element. 
    const TtmlTextParser = TtmlTextParser;
    const attr = TtmlTextParser.getStyleAttributeFromElement_(cueElement, styles, attribute);
    if (attr) {
      return attr;
    }
  ...
  /**
     ...
     *
     */ 
  private static getStyleAttributeFromRegion_(region: Element, styles: Element[], attribute: string): string | null {
    const XmlUtils = XmlUtils;
  ...
  /**
     ...
     *
     */ 
  private static getStyleAttributeFromElement_(cueElement: Element, styles: Element[], attribute: string): string | null {
    const XmlUtils = XmlUtils;
    const ttsNs = styleNs_;
     
    // Styling on elements should take precedence
    // over the main styling attributes 
    const elementAttribute = XmlUtils.getAttributeNSList(cueElement, ttsNs, attribute);
    if (elementAttribute) {
  ...
  /**
     ...
     *
     */ 
  private static getInheritedStyleAttribute_(element: Element, styles: Element[], attribute: string): string | null {
    const XmlUtils = XmlUtils;
    const ttsNs = styleNs_;
    const ebuttsNs = styleEbuttsNs_;
    const inheritedStyles = TtmlTextParser.getElementsFromCollection_(element, 'style', styles,  
    /* prefix= */ 
    '');
    let styleValue = null;
    ...
    for (let i = 0; i < inheritedStyles.length; i++) {
      ...
      let styleAttributeValue = XmlUtils.getAttributeNS(inheritedStyles[i], ebuttsNs, attribute);
      if (!styleAttributeValue) {
         
        // Fall back to tts namespace. 
        styleAttributeValue = XmlUtils.getAttributeNSList(inheritedStyles[i], ttsNs, attribute);
      }
      if (!styleAttributeValue) {
         
        // Next, check inheritance.
        // Styles can inherit from other styles, so traverse up that chain. 
        styleAttributeValue = TtmlTextParser.getStyleAttributeFromElement_(inheritedStyles[i], styles, attribute);
      }
      if (styleAttributeValue) {
  ...
  /**
     ...
     *
     */ 
  private static getElementsFromCollection_(element: Element, attributeName: string, collection: Element[], prefixName: string, nsName?: string): Element[] {
    const items = [];
    if (!element || collection.length < 1) {
      return items;
    }
    const attributeValue = TtmlTextParser.getInheritedAttribute_(element, attributeName, nsName);
    if (attributeValue) {
  ...
  /**
     ...
     *
     */ 
  private static getInheritedAttribute_(element: Element, attributeName: string, nsName?: string): string | null {
    let ret = null;
    const XmlUtils = XmlUtils;
    while (element) {
      ret = nsName ? XmlUtils.getAttributeNS(element, nsName, attributeName) : element.getAttribute(attributeName);
      if (ret) {
        break;
      }
  ...
  /**
     ...
     */ 
  private static resolveTime_(parentElement: Element, rateInfo: RateInfo_, start: number | null, end: number | null): {start:number | null, end:number | null} {
  ...
  /**
     ...
     *
     */ 
  private static parseTime_(element: Element, rateInfo: RateInfo_): {start:number | null, end:number | null} {
    const start = TtmlTextParser.parseTimeAttribute_(element.getAttribute('begin'), rateInfo);
    let end = TtmlTextParser.parseTimeAttribute_(element.getAttribute('end'), rateInfo);
    const duration = TtmlTextParser.parseTimeAttribute_(element.getAttribute('dur'), rateInfo);
  ...
  /**
     ...
     *
     */ 
  private static parseTimeAttribute_(text: string, rateInfo: RateInfo_): number | null {
    let ret = null;
    const TtmlTextParser = TtmlTextParser;
            ...
            if (TtmlTextParser.timeTickFormat_.test(text)) {
              ...
              if (TtmlTextParser.timeHMSFormat_.test(text)) {
                ...
                if (text) {
                  ...
                  throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.TEXT, ErrorExports.Code.INVALID_TEXT_CUE, 'Could not parse cue time range in TTML');
                }
              }
            }
          }
  ...
  /**
     ...
     *
     */ 
  private static parseFramesTime_(rateInfo: RateInfo_, text: string): number | null {
  ...
  /**
     ...
     *
     */ 
  private static parseTickTime_(rateInfo: RateInfo_, text: string): number | null {
  ...
  /**
     ...
     *
     */ 
  private static parseColonTimeWithFrames_(rateInfo: RateInfo_, text: string): number | null {
  ...
  /**
     ...
     *
     */ 
  private static getCellResolution_(cellResolution: string | null): {columns:number, rows:number} | null {
    if (!cellResolution) {
      return null;
    }
...
export class RateInfo_ {
  ...
  constructor(frameRate: string | null, subFrameRate: string | null, frameRateMultiplier: string | null, tickRate: string | null) {
 ...
 * @example 50.17% 10%
 */ 
export const percentValues_: RegExp = /^(\d{1,2}(?:\.\d+)?|100(?:\.0+)?)% (\d{1,2}(?:\.\d+)?|100(?:\.0+)?)%$/;
 ...
 */ 
export const timeColonFormatFrames_: RegExp = /^(\d{2,}):(\d{2}):(\d{2}):(\d{2})\.?(\d+)?$/;
 ...
 */ 
export const timeColonFormatMilliseconds_: RegExp = /^(?:(\d{2,}):)?(\d{2}):(\d{2}\.\d{2,})$/;
 ...
 */ 
export const timeHMSFormat_: RegExp = new RegExp(['^(?:(\\d*(?:\\.\\d*)?)h)?', '(?:(\\d*(?:\\.\\d*)?)m)?', '(?:(\\d*(?:\\.\\d*)?)s)?', '(?:(\\d*(?:\\.\\d*)?)ms)?$'].join(''));
 
export const textAlignToLineAlign_: {[key:string]:CueExports.lineAlign} = {'left':CueExports.lineAlign.START, 'center':CueExports.lineAlign.CENTER, 'right':CueExports.lineAlign.END, 'start':CueExports.lineAlign.START, 'end':CueExports.lineAlign.END};
 
export const textAlignToPositionAlign_: {[key:string]:CueExports.positionAlign} = {'left':CueExports.positionAlign.LEFT, 'center':CueExports.positionAlign.CENTER, 'right':CueExports.positionAlign.RIGHT};
 ...
 *
 */ 
export const parameterNs_: string[] = ['http://www.w3.org/ns/ttml#parameter', 'http://www.w3.org/2006/10/ttaf1#parameter'];
 ...
 *
 */ 
export const styleNs_: string[] = ['http://www.w3.org/ns/ttml#styling', 'http://www.w3.org/2006/10/ttaf1#styling'];
 ...
 */ 
export const smpteNsList_: string[] = ['http://www.smpte-ra.org/schemas/2052-1/2010/smpte-tt', 'http://www.smpte-ra.org/schemas/2052-1/2013/smpte-tt'];
TextEngine.registerParser('application/ttml+xml',  
() => new TtmlTextParser());
