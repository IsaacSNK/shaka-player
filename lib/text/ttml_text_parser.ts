  ...
  export class TtmlTextParser implements shaka.extern.TextParser {
    ...
    parseMedia(data, time) {
      ...
      if (xml) {
        ...
        const frameRateMultiplier = XmlUtils.getAttributeNSList(
          tt,
          ttpNs,
          "frameRateMultiplier"
        );
    ...
    private static parseCue_(
      ...
      const spaceStyle =
        cueElement.getAttribute("xml:space") ||
      ...
      if (
        parentCueElement &&
        isNested &&
        !cueElement.getAttribute("region") &&
          ...
          if (!percentage) {
            ...
            if (globalWidth != null) {
              region.viewportAnchorX =
                (region.viewportAnchorX * 100) / globalWidth;
            }
          }
    ...
    private static addStyle_(
      ...
      const textDecorationElement =
        TtmlTextParser.getStyleAttributeFromElement_(
          cueElement,
          styles,
          "textDecoration"
        );
    ...
    private static getStyleAttribute_(
      ...
      styles: Element[],
      attribute: string,
      shouldInheritRegionStyles: boolean = true
    ): string | null {
     ...
     *
     */
    private static parseTimeAttribute_(
      text: string,
      rateInfo: RateInfo_
    ): number | null {
      let ret = null;
     ...
     *
     */
    private static parseFramesTime_(
      rateInfo: RateInfo_,
      text: string
    ): number | null {
      // 75f or 75.5f
    ...
    private static parseColonTimeWithFrames_(
      ...
      const results =
        shaka.text.TtmlTextParser.timeColonFormatFrames_.exec(text);
     ...
     *
     */
    private static parseTimeFromRegex_(
      regex: RegExp,
      text: string
    ): number | null {
     ...
     *
     */
    private static getCellResolution_(
      cellResolution: string | null
...
namespace shaka.text.TtmlTextParser {
  /**
   * @summary
  ...
  export class RateInfo_ {
    ...
    constructor(
      ...
      subFrameRate: string | null,
      frameRateMultiplier: string | null,
...
namespace shaka.text.TtmlTextParser {
  export const textAlignToLineAlign_: { [key: string]: lineAlign } = {
...
namespace shaka.text.TtmlTextParser {
  export const textAlignToPositionAlign_: { [key: string]: positionAlign } = {
