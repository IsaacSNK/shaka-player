...
export class CastUtils {
      ...
      if (value == 'NaN') {
        ...
        if (value == '-Infinity') {
          ...
          if (value == 'Infinity') {
            ...
            if (value && typeof value == 'object' && value['__type__'] == 'TimeRanges') {
              ...
              if (value && typeof value == 'object' && value['__type__'] == 'Uint8Array') {
                return CastUtils.makeUint8Array_(value);
              } else {
                if (value && typeof value == 'object' && value['__type__'] == 'Error') {
  ...
  private static unpackTimeRanges_(ranges: TimeRanges): Object {
    const obj = {'__type__':'TimeRanges',  
    // a signal to deserialize 
    'length':ranges.length, 'start':[], 'end':[]};
  ...
  /**
     ...
     * }}
     */ 
  private static simulateTimeRanges_(obj: any): {length:number, start:(p1: number) => number, end:(p1: number) => number} {
 ...
 */ 
export const VideoEvents: string[] = ['ended', 'play', 'playing', 'pause', 'pausing', 'ratechange', 'seeked', 'seeking', 'timeupdate', 'volumechange'];
 
/**
 * HTMLMediaElement attributes that are proxied while casting.
 */ 
export const VideoAttributes: string[] = ['buffered', 'currentTime', 'duration', 'ended', 'loop', 'muted', 'paused', 'playbackRate', 'seeking', 'videoHeight', 'videoWidth', 'volume'];
...
'getAssetUri':2, 'getAudioLanguages':4, 'getAudioLanguagesAndRoles':4, 'getBufferFullness':1, 'getBufferedInfo':2,  
// NOTE: The 'getSharedConfiguration' property is not proxied as it would
//       not be possible to share a reference. 
'getConfiguration':4, 'getExpiration':2, 'getKeyStatuses':2,  
...
// not serialize. 
'getPlaybackRate':2, 'getTextLanguages':4, 'getTextLanguagesAndRoles':4, 'getTextTracks':2, 'getStats':5, 'getVariantTracks':2, 'getImageTracks':2, 'getThumbnails':2, 'isAudioOnly':10, 'isBuffering':1, 'isInProgress':1, 'isLive':10, 'isTextTrackVisible':1, 'keySystem':10, 'seekRange':1, 'getLoadMode':10};
 ...
 * Frequency 1 translates to every update; frequency 2 to every 2 updates, etc.
 */ 
export const PlayerGetterMethodsThatRequireLive: {[key:string]:number} = {'getPlayheadTimeAsDate':1, 'getPresentationStartTimeAsDate':20};
 ...
 * load() is resolved.
 */ 
export const PlayerInitAfterLoadState: string[][] = [['isTextTrackVisible', 'setTextTrackVisibility']];
 
/**
 * Player methods with no return value that are proxied while casting.
 */ 
export const PlayerVoidMethods: string[] = ['addChaptersTrack', 'addTextTrackAsync', 'cancelTrickPlay', 'configure', 'getChapters', 'getChaptersTracks', 'resetConfiguration', 'retryStreaming', 'selectAudioLanguage', 'selectTextLanguage', 'selectTextTrack', 'selectVariantTrack', 'selectVariantsByLabel', 'setTextTrackVisibility', 'trickPlay', 'updateStartTime', 'goToLive'];
 
/**
 * Player methods returning a Promise that are proxied while casting.
 */ 
export const PlayerPromiseMethods: string[] = ['attach', 'detach',  
// The manifestFactory parameter of load is not supported. 
'load', 'unload'];
type InitStateType = {video:Object, player:Object, manifest:string | null, startTime:number | null};
 ...
 */ 
export const GENERIC_MESSAGE_NAMESPACE: string = 'urn:x-cast:com.google.cast.media';
