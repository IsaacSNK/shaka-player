...
export class CastSender implements IDestroyable {
   ...
   */
  constructor(
      receiverAppId: string, onStatusChanged: () => any,
      onFirstCastStateUpdate: () => any,
      onRemoteEvent: (p1: string, p2: FakeEvent) => any,
      onResumeLocal: () => any, onInitStateRequired: () => any,
  ...
  async cast(initState: CastUtilsExports.InitStateType): Promise {
    if (!this.apiReady_) {
      throw new Error(
          ErrorExports.Severity.RECOVERABLE, ErrorExports.Category.CAST,
          ErrorExports.Code.CAST_API_UNAVAILABLE);
    }
    if (!hasReceivers_) {
      throw new Error(
          ErrorExports.Severity.RECOVERABLE, ErrorExports.Category.CAST,
          ErrorExports.Code.NO_CAST_RECEIVERS);
    }
    if (this.isCasting_) {
      throw new Error(
          ErrorExports.Severity.RECOVERABLE, ErrorExports.Category.CAST,
          ErrorExports.Code.ALREADY_CASTING);
    }
    ...
    if (targetName == 'video') {
      ...
      if (targetName == 'player') {
        if (CastUtils.PlayerGetterMethodsThatRequireLive[property]) {
          ...
          asserts.assert(
              isLive, property + ' should be called on a live stream!');
  ...
  private onSessionInitiated_(
      initState: CastUtilsExports.InitStateType, session: chrome.cast.Session) {
    log.debug('CastSender: onSessionInitiated');
    this.onSessionCreated_(session);
    this.sendMessage_(
        {'type': 'init', 'initState': initState, 'appData': this.appData_});
  ...
  private onConnectionError_(error: chrome.cast.Error) {
    ...
    this.castPromise_.reject(new Error(
        ErrorExports.Severity.CRITICAL, ErrorExports.Category.CAST, code,
        error));
  }
  ...
  private remoteAsyncCall_(targetName: string, methodName: string, ...varArgs):
      Promise {
    asserts.assert(
        targetName == 'video' || targetName == 'player',
        'Unexpected target name');
  ...
  private onSessionCreated_(session: chrome.cast.Session) {
    ...
    session.addMessageListener(
        CastUtilsExports.SHAKA_MESSAGE_NAMESPACE, this.onMessageReceivedBound_);
    this.onConnectionStatusChanged_();
  }
      ...
      case 'asyncComplete': {
        ...
        if (error) {
          ...
          const reconstructedError =
              new Error(error.severity, error.category, error.code);
  ...
  private sendMessage_(message: Object) {
    ...
    try {
      session.sendMessage(
          CastUtilsExports.SHAKA_MESSAGE_NAMESPACE, serialized, () => {},
          // success callback
          log.error);
    } catch (error) {
      ...
      // Translate the error
      const shakaError = new Error(
          ErrorExports.Severity.CRITICAL, ErrorExports.Category.CAST,
          ErrorExports.Code.CAST_CONNECTION_TIMED_OUT, error);

      // Dispatch it through the Player proxy
      const fakeEvent =
          new FakeEvent('error', (new Map()).set('detail', shakaError));
