    ...
    implements IDestroyable
  {
    ...
    private init_() {
      ...
      for (const name of shaka.cast.CastUtils.VideoEvents) {
        this.eventManager_.listen(this.video_, name, (event) =>
          this.proxyEvent_("video", event)
        );
      }
      for (const key in shaka.util.FakeEvent.EventName) {
        const name = shaka.util.FakeEvent.EventName[key];
        this.eventManager_.listen(this.player_, name, (event) =>
          this.proxyEvent_("player", event)
        );
      }
     ...
     */
    private async initState_(initState: InitStateType, appData: Object) {
    ...
    private maybeSendMediaInfoMessage_(): boolean {
      if (
        this.initialStatusUpdatePending_ &&
        ...
        this.sendMessage_(
          {
            type: "update",
            update: {
              video: { volume: systemVolume.level, muted: systemVolume.muted },
            },
          },
          this.shakaBus_
        );
    ...
    private onGenericMessage_(event: cast.receiver.CastMessageBus.Event) {
      ...
      switch (message["type"]) {
        ...
        case "VOLUME": {
          ...
          if (
            oldVolumeLevel != this.video_.volume ||
        ...
        case "LOAD": {
          ...
          this.player_
            ...
            .catch((error) => {
              ...
              if (
                error.category == shaka.util.Error.Category.PLAYER &&
    ...
    private sendAsyncComplete_(senderId: string, id: string, error: Error) {
      ...
      this.sendMessage_(
        { type: "asyncComplete", id: id, error: error },
        this.shakaBus_,
        senderId
      );
    }
...
namespace shaka.cast.CastReceiver {
