...
export class CastReceiver extends FakeEventTarget implements IDestroyable {
   ...
   */
  constructor(
      video: HTMLMediaElement, player: Player,
      appDataCallback?: (p1: Object) => any,
      contentIdCallback?: (p1: string) => string) {
    super();
    this.video_ = video;
    this.player_ = player;
    ...
    this.contentIdCallback_ =
        contentIdCallback || ((contentId: string): string => contentId);
  ...
  private init_() {
    ...
    this.genericBus_ =
        manager.getCastMessageBus(CastUtilsExports.GENERIC_MESSAGE_NAMESPACE);
    this.genericBus_.onMessage = (event) => this.onGenericMessage_(event);
    this.shakaBus_ =
        manager.getCastMessageBus(CastUtilsExports.SHAKA_MESSAGE_NAMESPACE);
   ...
   */
  private async initState_(
      initState: CastUtilsExports.InitStateType, appData: Object) {
  ...
  private proxyEvent_(targetName: string, event: Event) {
    ...
    this.pollAttributes_();
    this.sendMessage_(
        {'type': 'event', 'targetName': targetName, 'event': event},
        this.shakaBus_);
  }
  ...
  private pollAttributes_() {
    ...
    if (this.player_.isLive()) {
      const PlayerGetterMethodsThatRequireLive =
          CastUtilsExports.PlayerGetterMethodsThatRequireLive;
      ...
      this.sendMessage_(
          {
            ...
            'update': {
              'video':
                  {'volume': systemVolume.level, 'muted': systemVolume.muted}
            }
          },
          this.shakaBus_);
    }
  ...
  private onGenericMessage_(event: cast.receiver.CastMessageBus.Event) {
    ...
    switch (message['type']) {
      ...
      case 'LOAD': {
        ...
        this.player_.load(assetUri, currentTime)
            .then(() => {
              if (!this.player_) {
                // We've already been destroyed.
                return;
            ...
            .catch((error) => {
              ...
              if (error.category == ErrorExports.Category.PLAYER &&
                  error.code == ErrorExports.Code.LOAD_INTERRUPTED) {
                type = 'LOAD_CANCELLED';
              }
              this.sendMessage_(
                  {'requestId': Number(message['requestId']), 'type': type},
                  this.genericBus_);
            });
        break;
      }
  ...
  private sendAsyncComplete_(senderId: string, id: string, error: Error) {
    ...
    this.sendMessage_(
        {'type': 'asyncComplete', 'id': id, 'error': error}, this.shakaBus_,
        senderId);
  }
  ...
  private sendMessage_(
      message: Object, bus: cast.receiver.CastMessageBus, senderId?: string) {
