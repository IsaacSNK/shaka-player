...
export class CastProxy extends FakeEventTarget implements IDestroyable {
   ...
   */
  constructor(
      video: HTMLMediaElement, player: Player, receiverAppId: string,
  ...
  async changeReceiverId(
      newAppId: string, newCastAndroidReceiver: boolean = false) {
    ...
    // Create the new one
    this.sender_ = new CastSender(
        newAppId, () => this.onCastStatusChanged_(),
        () => this.onFirstCastStateUpdate_(),
        (targetName, event) => this.onRemoteEvent_(targetName, event),
        () => this.onResumeLocal_(), () => this.getInitState_(),
