...
namespace shaka.cast {
    ...
    implements IDestroyable
  {
     ...
     */
    getPlayer(): Player {
      return this.playerProxy_ as Player;
    }
     ...
     * @export
     */
    async changeReceiverId(
      newAppId: string,
      newCastAndroidReceiver: boolean = false
    ) {
      if (
        newAppId == this.receiverAppId_ &&
    ...
    private init_() {
      ...
      for (const name of shaka.cast.CastUtils.VideoEvents) {
        this.eventManager_.listen(this.localVideo_, name, (event) =>
          this.videoProxyLocalEvent_(event)
        );
      }
      for (const key in shaka.util.FakeEvent.EventName) {
        const name = shaka.util.FakeEvent.EventName[key];
        this.eventManager_.listen(this.localPlayer_, name, (event) =>
          this.playerProxyLocalEvent_(event)
        );
      }
     ...
     */
    private iterateOverPlayerMethods_(
      operation: (p1: string, p2: () => any) => any
    ) {
     ...
     */
    private getInitState_(): InitStateType {
      const initState = {
        video: {},
        player: {},
        playerAfterLoad: {},
    ...
    private videoProxyGet_(name: string): any {
      if (name == "addEventListener") {
        ...
        (type, listener, options) => {
          return this.videoEventTarget_.addEventListener(
            type,
            listener,
            options
          );
