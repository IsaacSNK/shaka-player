...
import{EbmlElement}from './ebml_parser';
import{EbmlParser}from './ebml_parser';
import*as EbmlParserExports from './ebml_parser';
...
class WebmSegmentIndexParser {
  ...
  /**
     ...
     */ 
  static parse(cuesData: BufferSource, initData: BufferSource, uris: string[], initSegmentReference: InitSegmentReference, timestampOffset: number, appendWindowStart: number, appendWindowEnd: number): SegmentReference[] {
    ...
    if (cuesElement.id != CUES_ID) {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.WEBM_CUES_ELEMENT_MISSING);
    }
    return WebmSegmentIndexParser.parseCues_(cuesElement, tuple.segmentOffset, tuple.timecodeScale, tuple.duration, uris, initSegmentReference, timestampOffset, appendWindowStart, appendWindowEnd);
  }
  ...
  /**
     ...
     */ 
  private static parseWebmContainer_(initData: BufferSource): {segmentOffset:number, timecodeScale:number, duration:number} {
    ...
    if (ebmlElement.id != EBML_ID) {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.WEBM_EBML_HEADER_ELEMENT_MISSING);
    }
    const segmentElement = parser.parseElement();
    if (segmentElement.id != SEGMENT_ID) {
      log.error('Not a Segment element.');
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.WEBM_SEGMENT_ELEMENT_MISSING);
    }
    ...
    return {segmentOffset:segmentOffset, timecodeScale:segmentInfo.timecodeScale, duration:segmentInfo.duration};
  }
  ...
  /**
     ...
     */ 
  private static parseSegment_(segmentElement: EbmlElement): {timecodeScale:number, duration:number} {
    ...
    if (!infoElement) {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.WEBM_INFO_ELEMENT_MISSING);
    }
  ...
  /**
     ...
     *   scale in seconds and duration in seconds.
     */ 
  private static parseInfo_(infoElement: EbmlElement): {timecodeScale:number, duration:number} {
    ...
    if (durationScale == null) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.WEBM_DURATION_ELEMENT_MISSING);
    }
  ...
  /**
     ...
     */ 
  private static parseCues_(cuesElement: EbmlElement, segmentOffset: number, timecodeScale: number, duration: number, uris: string[], initSegmentReference: InitSegmentReference, timestampOffset: number, appendWindowStart: number, appendWindowEnd: number): SegmentReference[] {
    const references = [];
    const getUris =  
    () => uris;
    ...
    while (parser.hasMoreData()) {
      ...
      if (lastTime != null) {
        ...
        references.push(new SegmentReference(lastTime + timestampOffset, currentTime + timestampOffset, getUris,  
        /* startByte= */ 
        lastOffset,  
        /* endByte= */ 
        currentOffset - 1, initSegmentReference, timestampOffset, appendWindowStart, appendWindowEnd));
      }
    ...
    if (lastTime != null) {
      asserts.assert(lastOffset != null, 'last offset cannot be null');
      references.push(new SegmentReference(lastTime + timestampOffset, duration + timestampOffset, getUris,  
      /* startByte= */ 
      lastOffset,  
      /* endByte= */ 
  ...
  /**
     ...
     *   element.
     */ 
  private static parseCuePoint_(cuePointElement: EbmlElement): {unscaledTime:number, relativeOffset:number} {
    ...
    if (cueTimeElement.id != CUE_TIME_ID) {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.WEBM_CUE_TIME_ELEMENT_MISSING);
    }
    ...
    if (cueTrackPositionsElement.id != CUE_TRACK_POSITIONS_ID) {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.WEBM_CUE_TRACK_POSITIONS_ELEMENT_MISSING);
    }
