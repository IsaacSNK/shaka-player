...
export class AdaptationSet {
  ...
  /**
     ...
     *    adaptation set.
     */ 
  constructor(root: shaka.extern.Variant, candidates?: Iterable<shaka.extern.Variant>) {
    this.root_ = root;
  ...
  static areAdaptable(a: shaka.extern.Variant, b: shaka.extern.Variant): boolean {
  ...
  /**
     ...
     *
     */ 
  private static areAudiosCompatible_(a: shaka.extern.Stream, b: shaka.extern.Stream): boolean {
    const AdaptationSet = AdaptationSet;
  ...
  /**
     ...
     *
     */ 
  private static areVideosCompatible_(a: shaka.extern.Stream, b: shaka.extern.Stream): boolean {
    const AdaptationSet = AdaptationSet;
  ...
  /**
     ...
     *
     */ 
  private static canTransitionBetween_(a: shaka.extern.Stream, b: shaka.extern.Stream): boolean {
    if (a.mimeType != b.mimeType) {
      return false;
    }
     
    // Get the base codec of each codec in each stream. 
    const codecsA = MimeUtils.splitCodecs(a.codecs).map( 
    (codec) => {
      return MimeUtils.getCodecBase(codec);
    });
    const codecsB = MimeUtils.splitCodecs(b.codecs).map( 
    (codec) => {
      return MimeUtils.getCodecBase(codec);
    });
  ...
  /**
     ...
     *
     */ 
  private static areRolesEqual_(a: Iterable<string>, b: Iterable<string>): boolean {
