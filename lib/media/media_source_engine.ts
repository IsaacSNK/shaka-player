...
export class MediaSourceEngine implements IDestroyable {
  ...
  /**
     {!Object.<shaka.util.ManifestParserUtils.ContentType,
                             SourceBuffer>}
   */
  private sourceBuffers_:
      {[key: ManifestParserUtilsExports.ContentType]: SourceBuffer} = {};

  /**
     {!Object.<shaka.util.ManifestParserUtils.ContentType,
                             string>}
   */
  private sourceBufferTypes_:
      {[key: ManifestParserUtilsExports.ContentType]: string} = {};

  /**
     {!Object.<shaka.util.ManifestParserUtils.ContentType,
                             boolean>}
   */
  private expectedEncryption_:
      {[key: ManifestParserUtilsExports.ContentType]: boolean} = {};
   ...
   */
  constructor(
      video: HTMLMediaElement, closedCaptionParser: IClosedCaptionParser,
      textDisplayer: shaka.extern.TextDisplayer,
      onMetadata?:
          (p1: shaka.extern.ID3Metadata[], p2: number,
           p3: number|null) => any) {
    this.video_ = video;
  ...
  static probeSupport(): {[key: string]: boolean} {
    const testMimeTypes = [
      ...
      // HDR HEVC
      'video/mp4; codecs="vp9"', 'video/mp4; codecs="vp09.00.10.08"',
      'video/mp4; codecs="av01.0.01M.08"', 'audio/mp4; codecs="mp4a.40.2"',
      'audio/mp4; codecs="ac-3"', 'audio/mp4; codecs="ec-3"',
      'audio/mp4; codecs="opus"', 'audio/mp4; codecs="flac"',
      // WebM types
      'video/webm; codecs="vp8"', 'video/webm; codecs="vp9"',
      'video/webm; codecs="vp09.00.10.08"', 'audio/webm; codecs="vorbis"',
      'audio/webm; codecs="opus"',
      // MPEG2 TS types (video/ is also used for audio: https://bit.ly/TsMse)
      'video/mp2t; codecs="avc1.42E01E"', 'video/mp2t; codecs="avc3.42E01E"',
      'video/mp2t; codecs="hvc1.1.6.L93.90"', 'video/mp2t; codecs="mp4a.40.2"',
      'video/mp2t; codecs="ac-3"', 'video/mp2t; codecs="ec-3"',
      // WebVTT types
    ...
    for (const type of testMimeTypes) {
      if (Platform.supportsMediaSource()) {
        ...
        if (TextEngine.isTypeSupported(type)) {
          ...
          support[type] =
              MediaSource.isTypeSupported(type) || Transmuxer.isSupported(type);
        }
      } else {
  ...
  async init(
      ...
      forceTransmuxTS: boolean, sequenceMode: boolean = false): Promise {
    ...
    for (const contentType of streamsByType.keys()) {
      ...
      if (contentType == ContentType.TEXT) {
        ...
        this.eventManager_.listen(
            sourceBuffer, 'error', () => this.onError_(contentType));
        this.eventManager_.listen(
            sourceBuffer, 'updateend', () => this.onUpdateEnd_(contentType));
  ...
  reinitText(mimeType: string, sequenceMode: boolean) {
    ...
    this.textEngine_.initParser(
        mimeType, sequenceMode, this.segmentRelativeVttTiming_);
  }
   ...
   */
  bufferStart(contentType: ManifestParserUtilsExports.ContentType): number
      |null {
    const ContentType = ManifestParserUtilsExports.ContentType;
    if (contentType == ContentType.TEXT) {
   ...
   */
  isBuffered(contentType: ManifestParserUtilsExports.ContentType, time: number):
      boolean {
    const ContentType = ManifestParserUtilsExports.ContentType;
    if (contentType == ContentType.TEXT) {
   ...
   */
  bufferedAheadOf(
      contentType: ManifestParserUtilsExports.ContentType,
      time: number): number {
    const ContentType = ManifestParserUtilsExports.ContentType;
    if (contentType == ContentType.TEXT) {
   ...
   */
  private getBuffered_(contentType: ManifestParserUtilsExports.ContentType):
      TimeRanges {
    try {
   ...
   */
  async appendBuffer(
      contentType: ManifestParserUtilsExports.ContentType, data: BufferSource,
      reference: SegmentReference|null, hasClosedCaptions: boolean|null,
      seeked?: boolean): Promise {
    const ContentType = ManifestParserUtilsExports.ContentType;
    if (contentType == ContentType.TEXT) {
      ...
      await this.textEngine_.appendBuffer(
          data, reference ? reference.startTime : null,
    ...
    if (this.transmuxers_[contentType]) {
      ...
      if (transmuxedData.metadata) {
        ...
        this.onMetadata_(
            transmuxedData.metadata, timestampOffset,
            reference ? reference.endTime : null);
      }
      ...
      if (transmuxedData.captions && transmuxedData.captions.length) {
        ...
        this.textEngine_.storeAndAppendClosedCaptions(
            closedCaptions, reference ? reference.startTime : null,
            reference ? reference.endTime : null, videoOffset);
      }
    ...
    if (this.sequenceMode_ && sourceBuffer.mode != SEQUENCE && reference) {
      ...
      if (isBestSourceBufferForTimestamps) {
        ...
        const mediaStartTime =
            TimeRangesUtils.bufferStart(this.getBuffered_(contentType));
    ...
    if (goog.DEBUG && reference) {
      ...
      const newBuffered =
          TimeRangesUtils.computeAddedRange(bufferedBefore, bufferedAfter);
      if (newBuffered) {
        ...
        if (Math.abs(newBuffered.start - reference.startTime) >
          ...
          log.error(
              ...
              reference.getUris(), 'should be',
              {start: reference.startTime, end: reference.endTime}, 'but got',
              newBuffered);
        }
      }
    }
   ...
   */
  async remove(
      contentType: ManifestParserUtilsExports.ContentType, startTime: number,
      endTime: number): Promise {
    const ContentType = ManifestParserUtilsExports.ContentType;
    if (contentType == ContentType.TEXT) {
      await this.textEngine_.remove(startTime, endTime);
    } else {
      await this.enqueueOperation_(
          contentType, () => this.remove_(contentType, startTime, endTime));
    }
  }
  ...
  async setStreamProperties(
      ...
      appendWindowEnd: number, sequenceMode: boolean): Promise {
    const ContentType = ManifestParserUtilsExports.ContentType;
    if (contentType == ContentType.TEXT) {
      if (!sequenceMode) {
  ...
  private onError_(contentType: ManifestParserUtilsExports.ContentType) {
    ...
    operation.p.reject(new Error(
        ...
        ErrorExports.Code.MEDIA_SOURCE_OPERATION_FAILED, code));
  }
  ...
  private async enqueueBlockingOperation_(run: () => any): Promise {
    ...
    try {
      ...
      throw new Error(
          ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA,
          ErrorExports.Code.MEDIA_SOURCE_OPERATION_THREW, exception);
    } finally {
      // Unblock the queues.
      for (const contentType in this.sourceBuffers_) {
        this.popFromQueue_(contentType);
      ...
      try {
        ...
        if (exception.name == 'QuotaExceededError') {
          next.p.reject(new Error(
              ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA,
              ErrorExports.Code.QUOTA_EXCEEDED_ERROR, contentType));
        } else {
          next.p.reject(new Error(
              ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA,
              ErrorExports.Code.MEDIA_SOURCE_OPERATION_THREW, exception));
  ...
  private workAroundBrokenPlatforms_(
      segment: BufferSource, startTime: number|null,
      contentType: ManifestParserUtilsExports.ContentType): BufferSource {
    ...
    if (isInitSegment && encryptionExpected &&
        Platform.requiresEncryptionInfoInAllInitSegments() &&
        MimeUtils.getContainerType(this.sourceBufferTypes_[contentType]) ==
            'mp4') {
