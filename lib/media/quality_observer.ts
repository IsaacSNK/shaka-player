...
namespace shaka.media {
    ...
    implements IPlayheadObserver
  {
    ...
    addMediaQualityChange(
      ...
      const contentTypeState = this.getContentTypeState_(
        mediaQuality.contentType
      );
    ...
    ): shaka.extern.MediaQualityInfo | null {
    ...
    private static mediaQualitiesAreTheSame_(
      ...
      mq2: shaka.extern.MediaQualityInfo | null
    ): boolean {
      if (mq1 === mq2) {
        return true;
      }
      ...
      for (const contentTypeState of this.contentTypeStates_.values()) {
        ...
        if (
          ...
          if (
            ...
            shaka.log.debug(
              "Media quality changed at position " +
                positionInSeconds +
                " " +
