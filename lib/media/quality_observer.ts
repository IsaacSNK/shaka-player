 ...
 * @final
 */ 
export class QualityObserver extends FakeEventTarget implements IPlayheadObserver {
  ...
  private getContentTypeState_(contentType: string): ContentTypeState {
    ...
    if (!contentTypeState) {
      contentTypeState = {qualityChangePositions:[], currentQuality:null, contentType:contentType};
  ...
  /**
     ...
     */ 
  addMediaQualityChange(mediaQuality: shaka.extern.MediaQualityInfo, position: number) {
  ...
  /**
     ...
     *  Position in seconds
     */ 
  private static getMediaQualityAtPosition_(position: number, contentTypeState: ContentTypeState): shaka.extern.MediaQualityInfo | null {
  ...
  /**
     ...
     *
     */ 
  private static mediaQualitiesAreTheSame_(mq1: shaka.extern.MediaQualityInfo | null, mq2: shaka.extern.MediaQualityInfo | null): boolean {
    if (mq1 === mq2) {
      return true;
    }
    if (!mq1 || !mq2) {
    ...
    return mq1.bandwidth == mq2.bandwidth && mq1.audioSamplingRate == mq2.audioSamplingRate && mq1.codecs == mq2.codecs && mq1.contentType == mq2.contentType && mq1.frameRate == mq2.frameRate && mq1.height == mq2.height && mq1.mimeType == mq2.mimeType && mq1.channelsCount == mq2.channelsCount && mq1.pixelAspectRatio == mq2.pixelAspectRatio && mq1.width == mq2.width;
  }
  ...
  poll(positionInSeconds, wasSeeking) {
    for (const contentTypeState of this.contentTypeStates_.values()) {
      ...
      if (qualityAtPosition && !QualityObserver.mediaQualitiesAreTheSame_(contentTypeState.currentQuality, qualityAtPosition)) {
        if (this.positionIsBuffered_(positionInSeconds, qualityAtPosition.contentType)) {
          ...
          const event = new FakeEvent('qualitychange', new Map([['quality', qualityAtPosition], ['position', positionInSeconds]]));
  ...
  private purgeQualityChangePositions_(contentTypeState: ContentTypeState) {
    ...
    if (bufferedRanges && bufferedRanges.length > 0) {
      ...
      contentTypeState.qualityChangePositions = oldChangePositions.filter( 
      (qualityChange, index) => {
         
        // Remove all but last quality change before bufferStart. 
        if (qualityChange.position <= bufferStart && index + 1 < oldChangePositions.length && oldChangePositions[index + 1].position <= bufferStart) {
          return false;
        }
...
type QualityChangePosition = {mediaQuality:shaka.extern.MediaQualityInfo, position:number};
 
export{QualityChangePosition};
type ContentTypeState = {qualityChangePositions:QualityChangePosition[], currentQuality:shaka.extern.MediaQualityInfo | null, contentType:string};
