...
export class ContentWorkarounds {
  ...
  static fakeEncryption(initSegmentBuffer: BufferSource): Uint8Array {
    const ContentWorkarounds = ContentWorkarounds;
    let initSegment = BufferUtils.toUint8(initSegmentBuffer);
    let isEncrypted = false;
    let stsdBox: shaka.extern.ParsedBox;
    const ancestorBoxes = [];
    const onSimpleAncestorBox =  
    (box) => {
      ancestorBoxes.push(box);
      Mp4Parser.children(box);
    };
    const onEncryptionMetadataBox =  
    (box) => {
      isEncrypted = true;
    };
    ...
    (new Mp4Parser()).box('moov', onSimpleAncestorBox).box('trak', onSimpleAncestorBox).box('mdia', onSimpleAncestorBox).box('minf', onSimpleAncestorBox).box('stbl', onSimpleAncestorBox).fullBox('stsd',  
    (box) => {
      stsdBox = box;
      ancestorBoxes.push(box);
      Mp4Parser.sampleDescription(box);
    }).fullBox('encv', onEncryptionMetadataBox).fullBox('enca', onEncryptionMetadataBox).fullBox('avc1',  
    (box) => {
      boxesToModify.push({box, newType:ContentWorkarounds.BOX_TYPE_ENCV_});
    }).fullBox('avc3',  
    (box) => {
      boxesToModify.push({box, newType:ContentWorkarounds.BOX_TYPE_ENCV_});
    }).fullBox('ac-3',  
    (box) => {
      boxesToModify.push({box, newType:ContentWorkarounds.BOX_TYPE_ENCA_});
    }).fullBox('ec-3',  
    (box) => {
      boxesToModify.push({box, newType:ContentWorkarounds.BOX_TYPE_ENCA_});
    }).fullBox('mp4a',  
    ...
    if (isEncrypted) {
      log.debug('Init segment already indicates encryption.');
      return initSegment;
    }
    if (boxesToModify.length == 0 || !stsdBox) {
      log.error('Failed to find boxes needed to fake encryption!');
      log.v2('Failed init segment (hex):', Uint8ArrayUtils.toHex(initSegment));
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.CONTENT_TRANSFORMATION_FAILED);
    }
    ...
    for (const workItem of boxesToModify) {
      const insertedBoxType = Mp4Parser.typeToString(workItem.newType);
      log.debug(`Inserting "${insertedBoxType}" box into init segment.`);
      initSegment = ContentWorkarounds.insertEncryptionMetadata_(initSegment, stsdBox, workItem.box, ancestorBoxes, workItem.newType);
    }
    return initSegment;
  }
  ...
  /**
     ...
     *
     */ 
  private static insertEncryptionMetadata_(initSegment: Uint8Array, stsdBox: shaka.extern.ParsedBox, sourceBox: shaka.extern.ParsedBox, ancestorBoxes: shaka.extern.ParsedBox[], metadataBoxType: number): Uint8Array {
    const ContentWorkarounds = ContentWorkarounds;
    const metadataBoxArray = ContentWorkarounds.createEncryptionMetadata_(initSegment, sourceBox, metadataBoxType);
     
    // Construct a new init segment array with room for the encryption metadata
    // box we're adding. 
    const newInitSegment = new Uint8Array(initSegment.byteLength + metadataBoxArray.byteLength);
    ...
    const cutPoint = Platform.isXboxOne() ? sourceBox.start : sourceBox.start + sourceBox.size;
    ...
    for (const box of ancestorBoxes) {
      asserts.assert(box.start < cutPoint, 'Ancestor MP4 box found in the wrong location!  ' + 'Modified init segment will not make sense!');
      ContentWorkarounds.updateBoxSize_(newInitSegment, box.start, box.size + metadataBoxArray.byteLength);
    }
     
    // Add one to the sample entries field of the "stsd" box.  This is a 4-byte
    // field just past the box header. 
    const stsdBoxView = BufferUtils.toDataView(newInitSegment, stsdBox.start);
    const stsdBoxHeaderSize = Mp4Parser.headerSize(stsdBox);
  ...
  /**
     ...
     *
     */ 
  private static createEncryptionMetadata_(initSegment: Uint8Array, sourceBox: shaka.extern.ParsedBox, metadataBoxType: number): Uint8Array {
    const ContentWorkarounds = ContentWorkarounds;
    ...
    const sourceBoxView = BufferUtils.toDataView(sourceBoxArray);
     
    // Create an array to hold the new encryption metadata box, which is based
    // on the source box. 
    const metadataBoxArray = new Uint8Array(sourceBox.size + sinfBoxArray.byteLength);
     
    // Copy the source box into the new array. 
    metadataBoxArray.set(sourceBoxArray,  
    /* targetOffset= */ 
    0);
     
    // Change the box type. 
    const metadataBoxView = BufferUtils.toDataView(metadataBoxArray);
    metadataBoxView.setUint32(ContentWorkarounds.BOX_TYPE_OFFSET_, metadataBoxType);
     
    // Append the "sinf" box to the encryption metadata box. 
    metadataBoxArray.set(sinfBoxArray,  
    /* targetOffset= */ 
    sourceBox.size);
     
    // Update the "sinf" box's format field (in the child "frma" box) to reflect
    // the format of the original source box. 
    const sourceBoxType = sourceBoxView.getUint32(ContentWorkarounds.BOX_TYPE_OFFSET_);
    metadataBoxView.setUint32(sourceBox.size + ContentWorkarounds.CANNED_SINF_BOX_FORMAT_OFFSET_, sourceBoxType);
     
    // Now update the encryption metadata box size. 
    ContentWorkarounds.updateBoxSize_(metadataBoxArray,  
    /* boxStart= */ 
    0, metadataBoxArray.byteLength);
  ...
  /**
     ...
     */ 
  private static updateBoxSize_(dataArray: Uint8Array, boxStart: number, newBoxSize: number) {
    const ContentWorkarounds = ContentWorkarounds;
    const boxView = BufferUtils.toDataView(dataArray, boxStart);
    ...
    if (sizeField == 0) {
      ...
      if (sizeField == 1) {
        ...
        // thorough here. 
        boxView.setUint32(ContentWorkarounds.BOX_SIZE_64_OFFSET_, newBoxSize >> 32);
        boxView.setUint32(ContentWorkarounds.BOX_SIZE_64_OFFSET_ + 4, newBoxSize & 4294967295);
      } else {
 ...
 * @see https://github.com/shaka-project/shaka-player/issues/2759
 */ 
export const CANNED_SINF_BOX_: Lazy<Uint8Array> = new Lazy( 
() => new Uint8Array([ 
// sinf box
// Size: 0x50 = 80 
0, 0, 0, 80,  
