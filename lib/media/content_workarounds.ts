...
export class ContentWorkarounds {
  ...
  static fakeEncryption(initSegmentBuffer: BufferSource): Uint8Array {
    ...
    (new Mp4Parser())
        ...
        .fullBox(
            ...
            (box) => {
              boxesToModify.push(
                  {box, newType: ContentWorkarounds.BOX_TYPE_ENCV_});
            })
        .fullBox(
            'avc3',
            (box) => {
        ...
        .fullBox(
            ...
            (box) => {
              boxesToModify.push(
                  {box, newType: ContentWorkarounds.BOX_TYPE_ENCA_});
            })
        .fullBox(
            'ec-3',
            (box) => {
    ...
    if (boxesToModify.length == 0 || !stsdBox) {
      ...
      throw new Error(
          ...
          ErrorExports.Code.CONTENT_TRANSFORMATION_FAILED);
    }
  ...
  private static insertEncryptionMetadata_(
      initSegment: Uint8Array, stsdBox: shaka.extern.ParsedBox,
      sourceBox: shaka.extern.ParsedBox,
      ancestorBoxes: shaka.extern.ParsedBox[],
      metadataBoxType: number): Uint8Array {
    const ContentWorkarounds = ContentWorkarounds;
    ...
    const cutPoint = Platform.isXboxOne() ? sourceBox.start :
  ...
  private static createEncryptionMetadata_(
      initSegment: Uint8Array, sourceBox: shaka.extern.ParsedBox,
      metadataBoxType: number): Uint8Array {
    const ContentWorkarounds = ContentWorkarounds;
    ...
    // on the source box.
    const metadataBoxArray =
        new Uint8Array(sourceBox.size + sinfBoxArray.byteLength);
    ...
    const sourceBoxType =
        sourceBoxView.getUint32(ContentWorkarounds.BOX_TYPE_OFFSET_);
    metadataBoxView.setUint32(
  ...
  private static updateBoxSize_(
      dataArray: Uint8Array, boxStart: number, newBoxSize: number) {
    const ContentWorkarounds = ContentWorkarounds;
    ...
    if (sizeField == 0) {
      ...
      if (sizeField == 1) {
        ...
        // thorough here.
        boxView.setUint32(
            ContentWorkarounds.BOX_SIZE_64_OFFSET_, newBoxSize >> 32);
        boxView.setUint32(
            ContentWorkarounds.BOX_SIZE_64_OFFSET_ + 4,
            newBoxSize & 4294967295);
      } else {
        // Normal 32-bit size field.
        // Not checking the size of the value here, since a box larger than 4GB
        // is unrealistic.
