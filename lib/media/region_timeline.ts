...
export class RegionTimeline extends FakeEventTarget implements IReleasable {
  ...
  private filterBySeekRange_() {
    ...
    for (const region of this.regions_) {
      ...
      if (region.endTime < seekRange.start) {
        ...
        const event =
            new FakeEvent('regionremove', new Map([['region', region]]));
        this.dispatchEvent(event);
      }
    }
  }
   ...
   *
   */
  private findSimilarRegion_(region: shaka.extern.TimelineRegionInfo):
      shaka.extern.TimelineRegionInfo|null {
    for (const existing of this.regions_) {
      ...
      const isSimilar = existing.schemeIdUri == region.schemeIdUri &&
          existing.id == region.id && existing.startTime == region.startTime &&
