...
export class StallDetector implements IReleasable {
  ...
  /**
     ...
     */ 
  constructor(implementation: Implementation, stallThresholdSeconds: number, onEvent: (p1: Event) => any) {
  ...
  poll(): boolean {
    ...
    if (triggerCallback) {
      ...
      this.onEvent_(new FakeEvent(FakeEventExports.EventName.StallDetected));
    }
...
export class MediaElementImplementation implements Implementation {
  ...
  shouldBeMakingProgress() {
    ...
    return MediaElementImplementation.hasContentFor_(this.mediaElement_.buffered,  
    /* timeInSeconds= */ 
  ...
  /**
     ...
     *
     */ 
  private static hasContentFor_(buffered: TimeRanges, timeInSeconds: number): boolean {
