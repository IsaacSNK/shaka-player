...
export class TimeRangesUtils {
  ...
  /**
     ...
     *   a gap.
     */ 
  static getGapIndex(b: TimeRanges, time: number, threshold: number): number | null {
    ...
    const idx = TimeRangesUtils.getBufferedInfo(b).findIndex( 
    (item, i, arr) => {
  ...
  /**
     ...
     */ 
  static computeAddedRange(oldRanges: TimeRanges, newRanges: TimeRanges): shaka.extern.BufferedRange | null {
    const TimeRangesUtils = TimeRangesUtils;
