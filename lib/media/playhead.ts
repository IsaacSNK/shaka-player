...
import{MediaElementImplementation}from './stall_detector';
...
export class SrcEqualsPlayhead implements Playhead {
  ...
  constructor(mediaElement: HTMLMediaElement) {
    ...
    MediaReadyState.waitForReadyState(this.mediaElement_, HTMLMediaElement.HAVE_CURRENT_DATA, this.eventManager_,  
    () => {
      onLoaded();
    });
  }
...
export class MediaSourcePlayhead implements Playhead {
  ...
  /**
     ...
     */ 
  constructor(mediaElement: HTMLMediaElement, manifest: shaka.extern.Manifest, config: shaka.extern.StreamingConfiguration, startTime: number | null, onSeek: () => any, onEvent: (p1: Event) => any) {
    this.mediaElement_ = mediaElement;
    ...
    this.gapController_ = new GapJumpingController(mediaElement, manifest.presentationTimeline, config, this.stallDetector_, onEvent);
    this.videoWrapper_ = new VideoWrapper(mediaElement,  
    () => this.onSeeking_(), this.getStartTime_(startTime));
  ...
  private reposition_(currentTime: number): number {
    ...
    const isBuffered: (p1: number) => any =  
    (playheadTime) => TimeRangesUtils.isBuffered(this.mediaElement_.buffered, playheadTime);
  ...
  /**
     ...
     *     Called when an event is raised to be sent to the application.
     */ 
  private createStallDetector_(mediaElement: HTMLMediaElement, config: shaka.extern.StreamingConfiguration, onEvent: (p1: Event) => any): StallDetector {
