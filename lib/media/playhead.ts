...
export class SrcEqualsPlayhead implements Playhead {
  ...
  constructor(mediaElement: HTMLMediaElement) {
    ...
    MediaReadyState.waitForReadyState(
        this.mediaElement_, HTMLMediaElement.HAVE_CURRENT_DATA,
        this.eventManager_, () => {
          onLoaded();
        });
  }
  ...
  getTime() {
    ...
    const time =
        this.started_ ? this.mediaElement_.currentTime : this.startTime_;
...
export class MediaSourcePlayhead implements Playhead {
   ...
   */
  constructor(
      mediaElement: HTMLMediaElement, manifest: shaka.extern.Manifest,
      config: shaka.extern.StreamingConfiguration, startTime: number|null,
      onSeek: () => any, onEvent: (p1: Event) => any) {
    this.mediaElement_ = mediaElement;
    ...
    this.gapController_ = new GapJumpingController(
        mediaElement, manifest.presentationTimeline, config,
        this.stallDetector_, onEvent);
    this.videoWrapper_ = new VideoWrapper(
        mediaElement, () => this.onSeeking_(), this.getStartTime_(startTime));
  ...
  private onPollWindow_() {
    ...
    if (currentTime < seekStart) {
      ...
      log.info(
          'Jumping forward ' + (targetTime - currentTime) +
  ...
  private onSeeking_() {
    ...
    const currentTime = this.videoWrapper_.getTime();
    const targetTime = this.reposition_(currentTime);
    const gapLimit = GapJumpingControllerExports.BROWSER_GAP_TOLERANCE;
  ...
  private reposition_(currentTime: number): number {
    asserts.assert(
        this.config_, 'Cannot reposition playhead when it has beeen destroyed');
    const isBuffered: (p1: number) => any = (playheadTime) =>
        TimeRangesUtils.isBuffered(this.mediaElement_.buffered, playheadTime);
    const rebufferingGoal =
        Math.max(this.minBufferTime_, this.config_.rebufferingGoal);
    ...
    const seekSafe =
        this.timeline_.getSafeSeekRangeStart(rebufferingGoal + safeSeekOffset);
  ...
  private createStallDetector_(
      ...
      onEvent: (p1: Event) => any): StallDetector {
    ...
    const detector = new StallDetector(
        new MediaElementImplementation(mediaElement), threshold, onEvent);
