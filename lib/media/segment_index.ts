 ...
 * @export
 */
export class SegmentIndex implements IReleasable, Iterable<SegmentReference> {
  private timer_: Timer = null;
  protected numEvicted: number = 0;
  private immutable_: boolean = false;
  references: any;

  constructor(protected references: SegmentReference[]) {
    if (goog.DEBUG) {
  ...
  forEachTopLevelReference(fn: (p1: SegmentReference) => any) {
    for (const reference of this.references) {
      fn(reference);
    }
  }

  earliestReference(): SegmentReference {
  ...
  find(time: number): number|null {
    ...
    for (let i = lastReferenceIndex; i >= 0; --i) {
      ...
      const end =
          i < lastReferenceIndex ? this.references[i + 1].startTime : r.endTime;
  ...
  merge(references: SegmentReference[]) {
    ...
    if (goog.DEBUG) {
      SegmentIndex.assertCorrectReferences_(this.references);
    }
  }
  ...
  mergeAndEvict(references: SegmentReference[], windowStart: number) {
    ...
    if (oldFirstRef) {
      asserts.assert(
          oldFirstRef.startTime == newFirstRef.startTime,
  ...
  fit(windowStart: number, windowEnd: number|null, isNew: boolean = false) {
    asserts.assert(
        windowEnd != null,
    ...
    const lastReference = this.references[this.references.length - 1];
    this.references[this.references.length - 1] = new SegmentReference(
        lastReference.startTime, windowEnd, lastReference.getUrisInner,
        lastReference.startByte, lastReference.endByte,
        lastReference.initSegmentReference, lastReference.timestampOffset,
        lastReference.appendWindowStart, lastReference.appendWindowEnd,
        lastReference.partialReferences, lastReference.tilesLayout,
        lastReference.tileDuration, lastReference.syncTime);
  }

  updateEvery(interval: number, updateCallback: () => SegmentReference[]) {
    asserts.assert(!this.timer_, 'SegmentIndex timer already started!');
    if (this.immutable_) {
      return;
    }
    if (this.timer_) {
  ...
  [
       ...
       */
      /**
       * Marks the index as immutable.  Segments cannot be added or removed
       * after this point.  This doesn't affect the references themselves.  This
       * also makes the destroy/release methods do nothing.
       *
       ...
       */
      /**
       * Finds the position of the segment for the given time, in seconds,
       * relative to the start of the presentation.  Returns the position of the
       * segment with the largest end time if more than one segment is known for
       * the given time.
       *
       ...
       * @export
       */
      // For live streams, searching from the end is faster.  For VOD, it
      // balances out either way.  In both cases, references.length is small
      // enough that the difference isn't huge. A rounding error can cause
      // /time/ to equal e.endTime or fall in between the references by a
      // fraction of a second. To account for this, we use the start of the next
      // segment as /end/, unless this is the last reference, in which case we
      // use its end time as /end/. Note that a segment ends immediately before
      // the end time.
      /**
       ...
       * references only.  Will replace old references with equivalent new ones,
       * and keep any unique old ones.
       *
       * Used, for example, by the DASH and HLS parser, where manifests may not
       * list all available references, so we must keep available references in
       * memory to fill the availability window.
       *
       * @param references The list of
       *   SegmentReferences, which must be sorted first by their start times
       *   (ascending) and second by their end times (ascending).
       */
      // Partial segments are used for live edge, and should be removed when
      // they get older. Remove the old SegmentReferences after the first new
      // reference's start time.
      /**
       * Merges the given SegmentReferences and evicts the ones that end before
       * the given time.  Supports extending the original references only. Will
       * not replace old references or interleave new ones. Used, for example,
       * by the DASH and HLS parser, where manifests may not list all available
       * references, so we must keep available references in memory to fill the
       * availability window.
       *
       * @param references The list of
       *   SegmentReferences, which must be sorted first by their start times
       ...
       *
       * Do not call on the last period of a live presentation (unknown
       * duration). It is okay to call on the other periods of a live
       * presentation, where the duration is known and another period has been
       * added.
       *
    ...
    asserts.assert(iter != null, 'Iterator for 0 should never be null!');
    return iter;
  }
   ...
   * @export
   */
  getIteratorForTime(time: number): SegmentIterator|null {
    ...
    return new SegmentIterator(this, index, partialSegmentIndex);
  }
   ...
   *
   */
  private static assertCorrectReferences_(references: SegmentReference[]) {
    asserts.assert(
 ...
 * @export
 */
export class SegmentIterator implements Iterator<SegmentReference> {
  private segmentIndex_: SegmentIndex;
  private currentPosition_: number;
  private currentPartialPosition_: number;

  constructor(
      segmentIndex: SegmentIndex, index: number, partialSegmentIndex: number) {
 ...
 * @export
 */
export class MetaSegmentIndex extends SegmentIndex implements
    IReleasable, Iterable<SegmentReference> {
