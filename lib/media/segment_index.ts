...
export class SegmentIndex implements IReleasable, Iterable<SegmentReference> {
  ...
  find(time: number): number | null {
    ...
    for (let i = lastReferenceIndex; i >= 0; --i) {
      ...
      const end = i < lastReferenceIndex ? this.references[i + 1].startTime : r.endTime;
  ...
  mergeAndEvict(references: SegmentReference[], windowStart: number) {
    ...
    if (oldFirstRef) {
      asserts.assert(oldFirstRef.startTime == newFirstRef.startTime, 'SegmentIndex.merge should not change the first reference time!');
    }
  ...
  fit(windowStart: number, windowEnd: number | null, isNew: boolean = false) {
    asserts.assert(windowEnd != null, 'Content duration must be known for static content!');
    asserts.assert(windowEnd != Infinity, 'Content duration must be finite for static content!');
    if (this.immutable_) {
      return;
    }
  ...
  /**
     ...
     * @export
     */ 
  static forSingleSegment(startTime: number, duration: number, uris: string[]): SegmentIndex {
  ...
  private static assertCorrectReferences_(references: SegmentReference[]) {
    asserts.assert(references.every( 
    (r2, i) => {
      if (i == 0) {
        return true;
      }
...
export class SegmentIterator implements Iterator<SegmentReference> {
  ...
  constructor(segmentIndex: SegmentIndex, index: number, partialSegmentIndex: number) {
  ...
  current(): SegmentReference {
    ...
    if (ref && ref.hasPartialSegments() && ref.getUris().length && this.currentPartialPosition_ >= ref.partialReferences.length) {
  ...
  next() {
    ...
    if (ref && ref.hasPartialSegments()) {
      ...
      if (ref.getUris().length && this.currentPartialPosition_ == ref.partialReferences.length) {
        this.currentPosition_++;
        this.currentPartialPosition_ = 0;
      }
    } else {
 ...
 * @export
 */ 
export class MetaSegmentIndex extends SegmentIndex implements IReleasable, Iterable<SegmentReference> {
  ...
  appendSegmentIndex(segmentIndex: SegmentIndex) {
    asserts.assert(this.indexes_.length == 0 || segmentIndex.numEvicted == 0, 'Should not append a new segment index with already-evicted segments');
  ...
  get(position) {
    ...
    for (const index of this.indexes_) {
      asserts.assert(!sawSegments || index.numEvicted == 0, 'Should not see evicted segments after available segments');
