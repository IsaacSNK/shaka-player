...
export class PresentationTimeline {
  ...
  /**
     ...
     */ 
  constructor(presentationStartTime: number | null, presentationDelay: number, autoCorrectDrift: boolean = true) {
  ...
  notifySegments(references: SegmentReference[]) {
    ...
    this.maxSegmentEndTime_ = Math.max(this.maxSegmentEndTime_, lastReferenceEndTime);
  ...
  getSafeSeekRangeStart(offset: number): number {
    ...
    const earliestSegmentTime = Math.max(this.minSegmentStartTime_, this.userSeekStart_);
  ...
  assertIsValid() {
    if (goog.DEBUG) {
      if (this.isLive()) {
        ...
        asserts.assert(this.presentationStartTime_ != null, 'Detected as live stream, but does not match our model of live!');
      } else {
        if (this.isInProgress()) {
           
          // Implied by isInProgress(): finite and dynamic.
          // IPR streams should have a start time, and segments should not expire. 
          asserts.assert(this.presentationStartTime_ != null && this.segmentAvailabilityDuration_ == Infinity, 'Detected as IPR stream, but does not match our model of IPR!');
        } else {
           
          // VOD
          // VOD segments should not expire and the presentation should be finite
          // and static. 
          asserts.assert(this.segmentAvailabilityDuration_ == Infinity && this.duration_ != Infinity && this.static_, 'Detected as VOD stream, but does not match our model of VOD!');
        }
      }
    }
  }
