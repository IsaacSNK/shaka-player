...
export class PresentationTimeline {
   ...
   */
  constructor(
      presentationStartTime: number|null, presentationDelay: number,
  ...
  notifyMaxSegmentDuration(maxSegmentDuration: number) {
    this.maxSegmentDuration_ =
        Math.max(this.maxSegmentDuration_, maxSegmentDuration);
    log.v1(
  ...
  getSegmentAvailabilityEnd(): number {
    ...
    return Math.min(
        this.getLiveEdge_() + this.availabilityTimeOffset_, this.duration_);
  }
    ...
    if (goog.DEBUG) {
      if (this.isLive()) {
        ...
        if (this.isInProgress()) {
          ...
          // IPR streams should have a start time, and segments should not
          // expire.
          asserts.assert(
              this.presentationStartTime_ != null &&
          ...
          // VOD
          // VOD segments should not expire and the presentation should be
          // finite and static.
          asserts.assert(
              this.segmentAvailabilityDuration_ == Infinity &&
                  this.duration_ != Infinity && this.static_,
