...
namespace shaka.media {
  ...
  export class StreamingEngine implements IDestroyable {
     ...
     */
    private mediaStates_: Map<ContentType, MediaState_>;
    ...
    private shouldAbortCurrentRequest_(mediaState: MediaState_): boolean {
      ...
      const bufferEnd = this.playerInterface_.mediaSourceEngine.bufferEnd(
        mediaState.type
      );
    ...
    private async initStreams_(): Promise {
      ...
      const streamsByType: Map<ContentType, shaka.extern.Stream> = new Map();
    ...
    private async onUpdate_(mediaState: MediaState_) {
      ...
      if (mediaState.waitingToClearBuffer) {
        ...
        await this.clearBuffer_(
          mediaState,
          mediaState.waitingToFlushBuffer,
    ...
    private update_(mediaState: MediaState_): number | null {
      ...
      for (const otherState of mediaStates) {
        ...
        if (
          otherState.segmentIterator &&
          !otherState.segmentIterator.current()
        ) {
          continue;
        }
    ...
    private getSegmentReferenceNeeded_(
      ...
      if (mediaState.segmentIterator) {
        ...
        if (mediaState.lastSegmentReference || bufferEnd) {
          ...
          const lookupTime = Math.max(
            presentationTime - inaccurateTolerance,
            0
          );
          shaka.log.v1(
            logPrefix,
    ...
    private async fetchAndAppend_(
      ...
      try {
        if (
          reference.getStatus() == shaka.media.SegmentReference.Status.MISSING
        ) {
          throw new shaka.util.Error(
        ...
        if (
          ...
          if (reference.hlsAes128Key) {
            goog.asserts.assert(
              iter,
              "mediaState.segmentIterator should exist"
            );
          ...
          if (
            ...
            if (error.code == shaka.util.Error.Code.QUOTA_EXCEEDED_ERROR) {
              ...
              if (
                ...
                shaka.log.error(
                  logPrefix,
                  "failed fetch and append: code=" + error.code
                );
    ...
    private async aes128Decrypt_(
      ...
      if (!key.cryptoKey) {
        ...
        goog.asserts.assert(
          key.cryptoKey,
          "AES-128 cryptoKey should now be set"
        );
      }
      let iv = key.iv;
      if (!iv) {
    ...
    private async initSourceBuffer_(
      ...
      const appendWindowStart = Math.max(
        0,
        reference.appendWindowStart - StreamingEngine.APPEND_WINDOW_START_FUDGE_
      );
      ...
      if (
        ...
        shaka.log.v1(
          logPrefix,
          "setting timestamp offset to " + timestampOffset
        );
        shaka.log.v1(
          logPrefix,
          ...
          const append = async () => {
            try {
              ...
              const hasClosedCaptions =
                mediaState.stream.closedCaptions &&
    ...
    private parseEMSG_(
      ...
      if (
        (emsgSchemeIdUris && emsgSchemeIdUris.includes(schemeId)) ||
        this.config_.dispatchAllEmsgBoxes
      ) {
    ...
    private parsePrft_(
      ...
      const programStartDate = new Date(
        wallClockTime - (mediaTime / timescale) * 1000
      );
    ...
    private async evict_(mediaState: MediaState_, presentationTime: number) {
      ...
      const startTime = this.playerInterface_.mediaSourceEngine.bufferStart(
        mediaState.type
      );
      if (startTime == null) {
        shaka.log.v2(
    ...
    private scheduleUpdate_(mediaState: MediaState_, delay: number) {
      ...
      if (
        ...
        shaka.log.v1(
          logPrefix,
          "Text stream is unloaded. No update is needed."
        );
        return;
      }
...
export interface PlayerInterface {
  ...
  onSegmentAppended: (p1: number, p2: number, p3: ContentType) => any;
  onInitSegmentAppended: (p1: number, p2: InitSegmentReference) => any;
}

export { PlayerInterface };

export interface MediaState_ {
  type: ContentType;
  stream: shaka.extern.Stream;
...
namespace shaka.media.StreamingEngine {
  /**
