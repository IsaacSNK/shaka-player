 ...
 */ 
import{SegmentInfo}from './cmcd_manager';
...
export class StreamingEngine implements IDestroyable {
  ...
  /**
       ...
       */ 
  private mediaStates_: Map<ManifestParserUtilsExports.ContentType, MediaState_>;
  ...
  constructor(manifest: shaka.extern.Manifest, playerInterface: PlayerInterface) {
  ...
  configure(config: shaka.extern.StreamingConfiguration) {
    ...
    maxAttempts:Math.max(config.retryParameters.maxAttempts, 2), baseDelay:config.retryParameters.baseDelay, backoffFactor:config.retryParameters.backoffFactor, fuzzFactor:config.retryParameters.fuzzFactor, timeout:0,  
    // irrelevant 
    stallTimeout:0,  
    // irrelevant 
    connectionTimeout:0};
  ...
  private async loadNewTextStream_(stream: shaka.extern.Stream): Promise {
    ...
    asserts.assert(!this.mediaStates_.has(ContentType.TEXT), 'Should not call loadNewTextStream_ while streaming text!');
    try {
    ...
    this.playerInterface_.mediaSourceEngine.reinitText(mimeType, this.manifest_.sequenceMode);
    const textDisplayer = this.playerInterface_.mediaSourceEngine.getTextDisplayer();
    const streamText = textDisplayer.isTextVisible() || this.config_.alwaysStreamText;
    if (streamText) {
  ...
  setTrickPlay(on: boolean) {
    ...
    if (on) {
      ...
      this.switchInternal_(trickModeVideo,  
      /* clearBuffer= */ 
      false,  
      /* safeMargin= */ 
      0,  
      ...
      this.switchInternal_(normalVideo,  
      /* clearBuffer= */ 
      true,  
      /* safeMargin= */ 
      0,  
  ...
  /**
     ...
     */ 
  switchVariant(variant: shaka.extern.Variant, clearBuffer: boolean = false, safeMargin: number = 0, force: boolean = false) {
    ...
    if (variant.video) {
      this.switchInternal_(variant.video,  
      /* clearBuffer= */ 
      clearBuffer,  
      /* safeMargin= */ 
      safeMargin,  
    ...
    if (variant.audio) {
      this.switchInternal_(variant.audio,  
      /* clearBuffer= */ 
      clearBuffer,  
      /* safeMargin= */ 
      safeMargin,  
  ...
  switchTextStream(textStream: shaka.extern.Stream) {
    ...
    const ContentType = ManifestParserUtilsExports.ContentType;
    asserts.assert(textStream && textStream.type == ContentType.TEXT, 'Wrong stream type passed to switchTextStream!');
    this.switchInternal_(textStream,  
    /* clearBuffer= */ 
    true,  
    /* safeMargin= */ 
    0,  
  ...
  reloadTextStream() {
    ...
    if (mediaState) {
      ...
      this.switchInternal_(mediaState.stream,  
      /* clearBuffer= */ 
      true,  
      /* safeMargin= */ 
      0,  
  ...
  /**
     ...
     */ 
  private switchInternal_(stream: shaka.extern.Stream, clearBuffer: boolean, safeMargin: number, force: boolean) {
    const ContentType = ManifestParserUtilsExports.ContentType;
    ...
    if (stream.type == ContentType.TEXT) {
      ...
      this.playerInterface_.mediaSourceEngine.reinitText(fullMimeType, this.manifest_.sequenceMode);
    }
    ...
    this.makeAbortDecision_(mediaState).catch( 
    (error) => {
      if (this.playerInterface_) {
        asserts.assert(error instanceof Error, 'Wrong error type!');
        this.playerInterface_.onError(error);
  ...
  private forceClearBuffer_(mediaState: MediaState_) {
    ...
    /* flush= */ 
    this.clearBuffer_(mediaState, false, 0).catch( 
    (error) => {
      if (this.playerInterface_) {
        asserts.assert(error instanceof Error, 'Wrong error type!');
        this.playerInterface_.onError(error);
  ...
  private async initStreams_(): Promise {
    ...
    if (!this.currentVariant_) {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STREAMING, ErrorExports.Code.STREAMING_ENGINE_STARTUP_INVALID_STATE);
    }
    const streamsByType: Map<ManifestParserUtilsExports.ContentType, shaka.extern.Stream> = new Map();
    ...
    await mediaSourceEngine.init(streamsByType, forceTransmuxTS, this.manifest_.sequenceMode);
    this.destroyer_.ensureNotDestroyed();
    this.setDuration_();
  ...
  private async onUpdate_(mediaState: MediaState_) {
    ...
    // Sanity check. 
    asserts.assert(!mediaState.performingUpdate && mediaState.updateTimer != null, logPrefix + ' unexpected call to onUpdate_()');
    if (mediaState.performingUpdate || mediaState.updateTimer == null) {
      return;
    }
    asserts.assert(!mediaState.clearingBuffer, logPrefix + ' onUpdate_() should not be called when clearing the buffer');
    if (mediaState.clearingBuffer) {
      return;
    }
    ...
    if (this.startupComplete_ && mediaStates.every( 
    (ms) => ms.endOfStream)) {
  ...
  private update_(mediaState: MediaState_): number | null {
    ...
    const unscaledBufferingGoal = Math.max(this.manifest_.minBufferTime || 0, this.config_.rebufferingGoal, this.config_.bufferingGoal);
  ...
  /**
     ...
     */ 
  private getTimeNeeded_(mediaState: MediaState_, presentationTime: number): number {
  ...
  /**
     ...
     */ 
  private getSegmentReferenceNeeded_(mediaState: MediaState_, presentationTime: number, bufferEnd: number | null): SegmentReference {
    const logPrefix = StreamingEngine.logPrefix_(mediaState);
    ...
    if (mediaState.segmentIterator) {
      ...
      if (mediaState.lastSegmentReference || bufferEnd) {
        ...
        mediaState.segmentIterator = mediaState.stream.segmentIndex.getIteratorForTime(time);
        ...
        if (inaccurateTolerance) {
          mediaState.segmentIterator = mediaState.stream.segmentIndex.getIteratorForTime(lookupTime);
        ...
        if (!ref) {
          ...
          mediaState.segmentIterator = mediaState.stream.segmentIndex.getIteratorForTime(presentationTime);
          ref = mediaState.segmentIterator && mediaState.segmentIterator.next().value;
        }
        if (ref == null) {
  ...
  /**
     ...
     *
     */ 
  private async fetchAndAppend_(mediaState: MediaState_, presentationTime: number, reference: SegmentReference) {
    const ContentType = ManifestParserUtilsExports.ContentType;
    const StreamingEngine = StreamingEngine;
    const logPrefix = StreamingEngine.logPrefix_(mediaState);
    log.v1(logPrefix, 'fetchAndAppend_:', 'presentationTime=' + presentationTime, 'reference.startTime=' + reference.startTime, 'reference.endTime=' + reference.endTime);
    ...
    try {
      if (reference.getStatus() == SegmentReferenceExports.Status.MISSING) {
        throw new Error(ErrorExports.Severity.RECOVERABLE, ErrorExports.Category.NETWORK, ErrorExports.Code.SEGMENT_MISSING);
      }
      ...
      // segment. 
      if (this.config_.lowLatencyMode && isReadableStreamSupported && isMP4 && !reference.hlsAes128Key) {
        ...
        async(data) => {
          ...
          if (sawMDAT) {
            ...
            await this.append_(mediaState, presentationTime, stream, reference, dataToAppend);
          }
        };
        await this.fetch_(mediaState, reference, streamDataCallback);
      } else {
        if (this.config_.lowLatencyMode && !isReadableStreamSupported) {
          log.warning('Low latency streaming mode is enabled, but ' + 'ReadableStream is not supported by the browser.');
        }
      ...
      if (!mediaState.waitingToClearBuffer) {
        this.playerInterface_.onSegmentAppended(reference.startTime, reference.endTime, mediaState.type);
      }
      ...
      if (error.code == ErrorExports.Code.OPERATION_ABORTED) {
        ...
        if (mediaState.type == ContentType.TEXT && this.config_.ignoreTextStreamFailures) {
          ...
          if (error.code == ErrorExports.Code.QUOTA_EXCEEDED_ERROR) {
            ...
            if (error.code == ErrorExports.Code.BAD_HTTP_STATUS && error.data && error.data[1] == 404) {
  ...
  /**
     * @return finalResult
     */ 
  private async aes128Decrypt_(rawResult: BufferSource, reference: SegmentReference, iter: SegmentIterator): Promise<BufferSource> {
    const key = reference.hlsAes128Key;
    if (!key.cryptoKey) {
      asserts.assert(key.fetchKey, 'If AES-128 cryptoKey was not ' + 'preloaded, fetchKey function should be provided');
      await key.fetchKey();
    ...
    return window.crypto.subtle.decrypt({name:'AES-CBC', iv}, key.cryptoKey, rawResult);
  }
  ...
  private handleQuotaExceeded_(mediaState: MediaState_, error: Error) {
    ...
    const mediaStates = Array.from(this.mediaStates_.values());
    const waitingForAnotherStreamToRecover = mediaStates.some( 
    (ms) => {
    ...
    if (!waitingForAnotherStreamToRecover) {
      ...
      log.warning(logPrefix, 'MediaSource threw QuotaExceededError:', 'reducing buffering goals by ' + (100 - percentAfter) + '%');
  ...
  /**
     ...
     *
     */ 
  private async initSourceBuffer_(mediaState: MediaState_, reference: SegmentReference): Promise {
    const StreamingEngine = StreamingEngine;
    const logPrefix = StreamingEngine.logPrefix_(mediaState);
    ...
    asserts.assert(reference.startTime <= appendWindowEnd, logPrefix + ' segment should start before append window end');
    const timestampOffset = reference.timestampOffset;
    if (timestampOffset != mediaState.lastTimestampOffset || appendWindowStart != mediaState.lastAppendWindowStart || appendWindowEnd != mediaState.lastAppendWindowEnd) {
      ...
      async() => {
        try {
          ...
          await this.playerInterface_.mediaSourceEngine.setStreamProperties(mediaState.type, timestampOffset, appendWindowStart, appendWindowEnd, this.manifest_.sequenceMode);
        } catch (error) {
      ...
      if (reference.initSegmentReference) {
        ...
        async() => {
          try {
            ...
            await this.playerInterface_.mediaSourceEngine.appendBuffer(mediaState.type, initSegment,  
            /* reference= */ 
  ...
  /**
     ...
     *
     */ 
  private async append_(mediaState: MediaState_, presentationTime: number, stream: shaka.extern.Stream, reference: SegmentReference, segment: BufferSource): Promise {
    const logPrefix = StreamingEngine.logPrefix_(mediaState);
    const hasClosedCaptions = stream.closedCaptions && stream.closedCaptions.size > 0;
    if (stream.emsgSchemeIdUris != null && stream.emsgSchemeIdUris.length > 0 || this.config_.dispatchAllEmsgBoxes) {
      (new Mp4Parser()).fullBox('emsg',  
      (box) => this.parseEMSG_(reference, stream.emsgSchemeIdUris, box)).parse(segment);
    }
    await this.evict_(mediaState, presentationTime);
    this.destroyer_.ensureNotDestroyed();
    log.v1(logPrefix, 'appending media segment at', reference.syncTime == null ? 'unknown' : reference.syncTime);
    const seeked = mediaState.seeked;
    mediaState.seeked = false;
    await this.playerInterface_.mediaSourceEngine.appendBuffer(mediaState.type, segment, reference, hasClosedCaptions, seeked);
    this.destroyer_.ensureNotDestroyed();
  ...
  /**
     ...
     */ 
  private parseEMSG_(reference: SegmentReference, emsgSchemeIdUris: string[] | null, box: shaka.extern.ParsedBox) {
    let timescale;
    let id;
    let eventDuration;
    let schemeId;
    ...
    if (emsgSchemeIdUris && emsgSchemeIdUris.includes(schemeId) || this.config_.dispatchAllEmsgBoxes) {
      ...
      if (schemeId == 'urn:mpeg:dash:event:2012') {
        ...
        const emsg: shaka.extern.EmsgInfo = {startTime:startTime, endTime:startTime + eventDuration / timescale, schemeIdUri:schemeId, value:value, timescale:timescale, presentationTimeDelta:presentationTimeDelta, eventDuration:eventDuration, id:id, messageData:messageData};
  ...
  private async evict_(mediaState: MediaState_, presentationTime: number) {
    ...
    const bufferBehind = Math.max(this.config_.bufferBehind, this.manifest_.presentationTimeline.getMaxSegmentDuration());
    const startTime = this.playerInterface_.mediaSourceEngine.bufferStart(mediaState.type);
    if (startTime == null) {
      log.v2(logPrefix, 'buffer behind okay because nothing buffered:', 'presentationTime=' + presentationTime, 'bufferBehind=' + bufferBehind);
      return;
    }
    ...
    if (overflow <= 0.01) {
      log.v2(logPrefix, 'buffer behind okay:', 'presentationTime=' + presentationTime, 'bufferedBehind=' + bufferedBehind, 'bufferBehind=' + bufferBehind, 'underflow=' + Math.abs(overflow));
      return;
    }
    log.v1(logPrefix, 'buffer behind too large:', 'presentationTime=' + presentationTime, 'bufferedBehind=' + bufferedBehind, 'bufferBehind=' + bufferBehind, 'overflow=' + overflow);
  ...
  private static isEmbeddedText_(mediaState: MediaState_): boolean {
    ...
    return mediaState && mediaState.type == ManifestParserUtilsExports.ContentType.TEXT && (mediaState.stream.mimeType == CEA608_MIME || mediaState.stream.mimeType == CEA708_MIME);
  }
  ...
  /**
     ...
     */ 
  private async fetch_(mediaState: MediaState_, reference: InitSegmentReference | SegmentReference, streamDataCallback?: ((p1: BufferSource) => Promise) | null): Promise<BufferSource> {
    const requestType = NetworkingEngineExports.RequestType.SEGMENT;
    const request = Networking.createSegmentRequest(reference.getUris(), reference.startByte, reference.endByte, this.config_.retryParameters, streamDataCallback);
    log.v2('fetching: reference=', reference);
    const stream = mediaState.stream;
    this.playerInterface_.modifySegmentRequest(request, {type:stream.type, init:reference instanceof InitSegmentReference, duration:reference.endTime - reference.startTime, mimeType:stream.mimeType, codecs:stream.codecs, bandwidth:stream.bandwidth});
  ...
  /**
     ...
     *
     */ 
  private async clearBuffer_(mediaState: MediaState_, flush: boolean, safeMargin: number) {
    const logPrefix = StreamingEngine.logPrefix_(mediaState);
    asserts.assert(!mediaState.performingUpdate && mediaState.updateTimer == null, logPrefix + ' unexpected call to clearBuffer_()');
  ...
  private scheduleUpdate_(mediaState: MediaState_, delay: number) {
    ...
    const type = mediaState.type;
    if (type == ManifestParserUtilsExports.ContentType.TEXT && !this.mediaStates_.has(type)) {
    ...
    mediaState.updateTimer = (new DelayedTick( 
    async() => {
      try {
...
type PlayerInterface = {getPresentationTime:() => number, getBandwidthEstimate:() => number, modifySegmentRequest:(p1: shaka.extern.Request, p2: SegmentInfo) => any, mediaSourceEngine:MediaSourceEngine, netEngine:NetworkingEngine, onError:(p1: Error) => any, onEvent:(p1: Event) => any, onManifestUpdate:() => any, onSegmentAppended:(p1: number, p2: number, p3: ManifestParserUtilsExports.ContentType) => any, onInitSegmentAppended:(p1: number, p2: InitSegmentReference) => any};
 
export{PlayerInterface};
type MediaState_ = {type:ManifestParserUtilsExports.ContentType, stream:shaka.extern.Stream, segmentIterator:SegmentIterator, lastSegmentReference:SegmentReference, lastInitSegmentReference:InitSegmentReference, lastTimestampOffset:number | null, lastAppendWindowStart:number | null, lastAppendWindowEnd:number | null, restoreStreamAfterTrickPlay:shaka.extern.Stream | null, endOfStream:boolean, performingUpdate:boolean, updateTimer:DelayedTick, waitingToClearBuffer:boolean, waitingToFlushBuffer:boolean, 
clearBufferSafeMargin:number, clearingBuffer:boolean, seeked:boolean, recovering:boolean, hasError:boolean, operation:NetworkingEngineExports.PendingRequest};
