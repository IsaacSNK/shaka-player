...
import{dependencies}from './all';
import*as dependenciesExports from './all';
...
export class Transmuxer implements IDestroyable {
  ...
  constructor() {
    ...
    this.muxTransmuxer_.on('data',  
    (segment) => this.onTransmuxed_(segment));
    this.muxTransmuxer_.on('done',  
    () => this.onTransmuxDone_());
  }
  ...
  static isSupported(mimeType: string, contentType?: string): boolean {
    ...
    if (contentType) {
      return MediaSource.isTypeSupported(Transmuxer.convertTsCodecs(contentType, mimeType));
    }
    ...
    return MediaSource.isTypeSupported(audioMime) || MediaSource.isTypeSupported(videoMime);
  }
    ...
    if (match) {
      ...
      if (profile == '66') {
        ...
        if (profile == '77') {
          ...
          asserts.assert(profile == '100', 'Legacy avc1 parsing code out of sync with regex!');
          newCodecString += '6400';
        }
      }
  ...
  /**
     ...
     */ 
  transmux(data: BufferSource): Promise<{data:Uint8Array, captions:muxjs.mp4.ClosedCaption[], metadata:Object[]}> {
    asserts.assert(!this.isTransmuxing_, 'No transmuxing should be in progress.');
    ...
    if (this.isTransmuxing_) {
      this.transmuxPromise_.reject(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MEDIA, ErrorExports.Code.TRANSMUXING_FAILED));
    }
  ...
  private onTransmuxed_(segment: muxjs.mp4.Transmuxer.Segment) {
    ...
    this.transmuxedData_.push(Uint8ArrayUtils.concat(segment.initSegment, segment.data));
  }
  ...
  private onTransmuxDone_() {
    const output = {data:Uint8ArrayUtils.concat(...this.transmuxedData_), captions:this.captions_, metadata:this.metadata_};
