...
export class ManifestParser {
  ...
  /**
     ...
     * @export
     */ 
  static registerParserByExtension(extension: string, parserFactory: shaka.extern.ManifestParser.Factory) {
  ...
  /**
     ...
     * @export
     */ 
  static registerParserByMime(mimeType: string, parserFactory: shaka.extern.ManifestParser.Factory) {
  ...
  /**
     ...
     *
     */ 
  static async getFactory(uri: string, netEngine: NetworkingEngine, retryParams: shaka.extern.RetryParameters, mimeType: string | null): Promise<shaka.extern.ManifestParser.Factory> {
    const ManifestParser = ManifestParser;
    ...
    if (extension) {
      ...
      log.warning('Could not determine manifest type for extension ', extension);
    } else {
    ...
    throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.MANIFEST, ErrorExports.Code.UNABLE_TO_GUESS_MANIFEST_TYPE, uri);
  }
   
  static async getMimeType(uri: string, netEngine: NetworkingEngine, retryParams: shaka.extern.RetryParameters): Promise<string> {
