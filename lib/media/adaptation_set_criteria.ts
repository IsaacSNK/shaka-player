...
export class ExampleBasedCriteria implements AdaptationSetCriteria {
  ...
  constructor(example: shaka.extern.Variant) {
    ...
    this.fallback_ = new PreferenceBasedCriteria(example.language, role, channelCount, label);
  }
  ...
  create(variants) {
    ...
    const shortList = variants.filter( 
    (variant) => {
...
export class PreferenceBasedCriteria implements AdaptationSetCriteria {
  ...
  constructor(language: string, role: string, channelCount: number, label: string = '') {
  ...
  create(variants) {
    ...
    const byPrimary = variants.filter( 
    ...
    if (this.channelCount_) {
      const byChannel = StreamUtils.filterVariantsByAudioChannelCount(current, this.channelCount_);
      if (byChannel.length) {
        current = byChannel;
      } else {
  ...
  private static filterByLanguage_(variants: shaka.extern.Variant[], preferredLanguage: string): shaka.extern.Variant[] {
    const LanguageUtils = LanguageUtils;
    const preferredLocale: string = LanguageUtils.normalize(preferredLanguage);
    const closestLocale: string | null = LanguageUtils.findClosestLocale(preferredLocale, variants.map( 
    (variant) => LanguageUtils.getLocaleForVariant(variant)));
  ...
  /**
     ...
     *
     */ 
  private static filterVariantsByRole_(variants: shaka.extern.Variant[], preferredRole: string): shaka.extern.Variant[] {
    return variants.filter( 
    (variant) => {
      if (!variant.audio) {
        return false;
  ...
  /**
     ...
     *
     */ 
  private static filterVariantsByLabel_(variants: shaka.extern.Variant[], preferredLabel: string): shaka.extern.Variant[] {
    return variants.filter( 
    (variant) => {
      if (!variant.audio) {
        return false;
