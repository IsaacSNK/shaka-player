...
export class GapJumpingController implements IReleasable {
  ...
  /**
     ...
     */ 
  constructor(video: HTMLMediaElement, timeline: PresentationTimeline, config: shaka.extern.StreamingConfiguration, stallDetector: StallDetector, onEvent: (p1: Event) => any) {
    ...
    this.eventManager_.listen(video, 'waiting',  
    () => this.onPollGapJump_());
  ...
  private onPollGapJump_() {
    ...
    // autoplay is enabled. 
    if (this.video_.paused && (this.video_.currentTime != 0 || !this.video_.autoplay && this.video_.currentTime == 0)) {
      return;
    }
    ...
    if (jumpSize < BROWSER_GAP_TOLERANCE) {
      return;
    }
    if (gapIndex == 0) {
      log.info('Jumping forward', jumpSize, 'seconds because of gap before start time of', jumpTo);
    } else {
      log.info('Jumping forward', jumpSize, 'seconds because of gap starting at', buffered.end(gapIndex - 1), 'and ending at', jumpTo);
    }
    this.video_.currentTime = jumpTo;
    this.gapsJumped_++;
    this.onEvent_(new FakeEvent(FakeEventExports.EventName.GapJumped));
  }
}
