  ...
  export class DrmEngine implements IDestroyable {
    ...
    willSupport(contentType: string): boolean {
      ...
      if (
        ...
        return (
          this.supportedTypes_.has(contentType) ||
    ...
    private static computeMimeType_(
      ...
      const realMimeType = shaka.util.MimeUtils.getFullType(
        stream.mimeType,
    ...
    private configureClearKey_(): shaka.extern.DrmInfo | null {
      ...
      const initData = shaka.util.BufferUtils.toUint8(
        StringUtils.toUTF8(initDataStr)
      );
      const initDatas = [{ initData: initData, initDataType: "keyids" }];
      return {
        keySystem: "org.w3.clearkey",
        licenseServerUri:
          "data:application/json;base64," + window.btoa(license),
        distinctiveIdentifierRequired: false,
    ...
    private async loadOfflineSession_(
      sessionId: string
      ...
      this.eventManager_.listen(
        session,
        "message",
        (event) => this.onSessionMessage_(event) as ListenerType
      );
    ...
    private async sendLicenseRequest_(event: MediaKeyMessageEvent) {
      ...
      if (
        shaka.media.DrmEngine.isPlayReadyKeySystem(
          this.currentDrmInfo_.keySystem
        )
      ) {
    ...
    private onKeyStatusesChange_(event: Event) {
      ...
      if (
        msUntilExpiration < 0 ||
        (hasExpiredKeys && msUntilExpiration < 1000)
      ) {
      ...
      this.keyStatusTimer_.tickAfter(
        /* seconds= */
        shaka.media.DrmEngine.KEY_STATUS_BATCH_TIME
      );
    }
    ...
    private processKeyStatusChanges_() {
      ...
      this.playerInterface_.onKeyStatus(
        shaka.util.MapUtils.asObject(publicMap)
      );
    }
     ...
     *
     */
    static async probeSupport(): Promise<{
      [key: string]: shaka.extern.DrmSupportType | null;
    }> {
      goog.asserts.assert(
      ...
      const support: Map<string, shaka.extern.DrmSupportType | null> =
        new Map();
    ...
    static areDrmCompatible(
      ...
      drms2: shaka.extern.DrmInfo[]
    ): boolean {
      if (!drms1.length || !drms2.length) {
        return true;
      }
      return shaka.media.DrmEngine.getCommonDrmInfos(drms1, drms2).length > 0;
    }
          ...
          if (drm1.keySystem == drm2.keySystem) {
            ...
            const mergedDrm = {
              ...
              distinctiveIdentifierRequired:
                drm1.distinctiveIdentifierRequired ||
              ...
              serverCertificate:
                drm1.serverCertificate || drm2.serverCertificate,
              serverCertificateUri:
    ...
    private createDrmInfoByInfos_(
      ...
      const res: shaka.extern.DrmInfo = {
        ...
        distinctiveIdentifierRequired:
          drmInfos[0].distinctiveIdentifierRequired,
      ...
      for (const info of drmInfos) {
        if (info.distinctiveIdentifierRequired) {
          res.distinctiveIdentifierRequired =
            info.distinctiveIdentifierRequired;
        }
    ...
    private static createDrmInfoByConfigs_(
      ...
      const audioRobustness = config.audioCapabilities
        ? config.audioCapabilities[0].robustness
        : "";
      const videoRobustness = config.videoCapabilities
    ...
    private static processDrmInfos_(
      ...
      serverCerts: Uint8Array[],
      serverCertificateUris: string[],
      initDatas: shaka.extern.InitDataOverride[],
      keyIds: Set<string>
    ) {
      const initDataOverrideEqual: (
        p1: shaka.extern.InitDataOverride,
        p2: shaka.extern.InitDataOverride
      ) => boolean = (a, b) => {
...
namespace shaka.media.DrmEngine {
  /**
...
namespace shaka.media.DrmEngine {
   ...
   */
  export const DUMMY_KEY_ID: Lazy<ArrayBuffer> = new shaka.util.Lazy(() =>
