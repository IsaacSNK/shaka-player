...
export 
class DrmEngine implements IDestroyable {
  ...
  constructor(playerInterface: PlayerInterface, updateExpirationTime: number = 1) {
    ...
    this.keyStatusTimer_ = new Timer( 
  ...
  /**
     ...
     *    |manifest|.
     */ 
  initForStorage(variants: shaka.extern.Variant[], usePersistentLicenses: boolean): Promise {
  ...
  /**
     ...
     */ 
  initForPlayback(variants: shaka.extern.Variant[], offlineSessionIds: string[]): Promise {
  ...
  /**
     ...
     *
     */ 
  initForRemoval(keySystem: string, licenseServerUri: string, serverCertificate: Uint8Array, audioCapabilities: MediaKeySystemMediaCapability[], videoCapabilities: MediaKeySystemMediaCapability[]): Promise {
    const configsByKeySystem: Map<string, MediaKeySystemConfiguration> = new Map();
    const config: MediaKeySystemConfiguration = {audioCapabilities:audioCapabilities, videoCapabilities:videoCapabilities, distinctiveIdentifier:'optional', persistentState:'required', sessionTypes:['persistent-license'], label:keySystem};
    ...
    keySystem:keySystem, licenseServerUri:licenseServerUri, distinctiveIdentifierRequired:false, persistentStateRequired:true, audioRobustness:'',  
    // Not required by queryMediaKeys_ 
    videoRobustness:'',  
    // Same 
    serverCertificate:serverCertificate, serverCertificateUri:'', initData:null, keyIds:null}];
  ...
  private async init_(variants: shaka.extern.Variant[]): Promise {
    asserts.assert(this.config_, 'DrmEngine configure() must be called before init()!');
    ...
    const clearKeyDrmInfo: shaka.extern.DrmInfo | null = this.configureClearKey_();
    if (clearKeyDrmInfo) {
    ...
    const hadDrmInfo = variants.some( 
    (variant) => {
    ...
    for (const variant of variants) {
      ...
      for (const info of drmInfos) {
        DrmEngine.fillInDrmInfoDefaults_(info, MapUtils.asMap(this.config_.servers), MapUtils.asMap(this.config_.advanced || {}), this.config_.keySystemsMapping);
      }
    }
    ...
    await StreamUtils.getDecodingInfosForVariants(variants, this.usePersistentLicenses_, this.srcEquals_);
  ...
  private async attachMediaKeys_(): Promise {
    ...
    try {
      ...
      this.onError_(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.FAILED_TO_ATTACH_TO_VIDEO, exception.message));
    }
  ...
  async attach(video: HTMLMediaElement): Promise {
    if (!this.mediaKeys_) {
      ...
      // platforms (Safari). 
      this.eventManager_.listenOnce(video, 'encrypted',  
      (event) => {
        this.onError_(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.ENCRYPTED_CONTENT_WITHOUT_DRM_INFO));
      });
      return;
    }
    this.video_ = video;
    ...
    if ('webkitCurrentPlaybackTargetIsWireless' in this.video_) {
      this.eventManager_.listen(this.video_, 'webkitcurrentplaybacktargetiswirelesschanged',  
    ...
    /**
         ...
         */ 
    if (manifestInitData || Platform.isMediaKeysPolyfilled() || this.offlineSessionIds_.length) {
      await this.attachMediaKeys_();
    }
    this.createOrLoad();
    ...
    if (!manifestInitData && !this.offlineSessionIds_.length) {
      this.eventManager_.listen(this.video_, 'encrypted',  
      (e) => this.onEncryptedEvent_(e));
    }
  }
  ...
  async setServerCertificate(): Promise {
    asserts.assert(this.initialized_, 'Must call init() before setServerCertificate');
    if (!this.mediaKeys_ || !this.currentDrmInfo_) {
      return;
    }
    if (this.currentDrmInfo_.serverCertificateUri && (!this.currentDrmInfo_.serverCertificate || !this.currentDrmInfo_.serverCertificate.length)) {
      const request = NetworkingEngine.makeRequest([this.currentDrmInfo_.serverCertificateUri], this.config_.retryParameters);
      try {
        const operation = this.playerInterface_.netEngine.request(NetworkingEngineExports.RequestType.SERVER_CERTIFICATE, request);
        const response = await operation.promise;
        this.currentDrmInfo_.serverCertificate = BufferUtils.toUint8(response.data);
      } catch (error) {
         
        // Request failed! 
        asserts.assert(error instanceof Error, 'Wrong NetworkingEngine error type!');
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.SERVER_CERTIFICATE_REQUEST_FAILED, error);
      }
      if (this.destroyer_.destroyed()) {
        return;
      }
    }
    if (!this.currentDrmInfo_.serverCertificate || !this.currentDrmInfo_.serverCertificate.length) {
      return;
    }
    try {
      const supported = await this.mediaKeys_.setServerCertificate(this.currentDrmInfo_.serverCertificate);
      if (!supported) {
        log.warning('Server certificates are not supported by the ' + 'key system.  The server certificate has been ' + 'ignored.');
      }
    } catch (exception) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.INVALID_SERVER_CERTIFICATE, exception.message);
    }
  }
  ...
  createOrLoad(): Promise {
    ...
    for (const initDataOverride of initDatas) {
      this.newInitData(initDataOverride.initDataType, initDataOverride.initData);
    }
  ...
  newInitData(initDataType: string, initData: Uint8Array) {
    ...
    for (const metadata of metadatas) {
      ...
      if (BufferUtils.equal(initData, metadata.initData) && !Platform.isTizen2()) {
        log.debug('Ignoring duplicate init data.');
        return;
      }
    }
    this.createSession(initDataType, initData, this.currentDrmInfo_.sessionType);
  }
  ...
  willSupport(contentType: string): boolean {
    ...
    if (Platform.isTizen() && contentType.includes('codecs="ac-3"')) {
      ...
      return this.supportedTypes_.has(contentType) || this.supportedTypes_.has(fallback);
    }
  ...
  getSessionIds(): string[] {
    ...
    const ids = Iterables.map(sessions,  
    (s) => s.sessionId);
  ...
  private static computeMimeType_(stream: shaka.extern.Stream, codecOverride?: string): string {
    const realMimeType = MimeUtils.getFullType(stream.mimeType, codecOverride || stream.codecs);
  ...
  /**
     ...
     * @return Resolved if/when a key system has been chosen.
     */ 
  private async queryMediaKeys_(configsByKeySystem: Map<string, MediaKeySystemConfiguration>, variants: shaka.extern.Variant[]): Promise {
    const drmInfosByKeySystem = new Map();
    const mediaKeySystemAccess = variants.length ? this.getKeySystemAccessFromVariants_(variants, drmInfosByKeySystem) : await this.getKeySystemAccessByConfigs_(configsByKeySystem);
    if (!mediaKeySystemAccess) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.REQUESTED_KEY_SYSTEM_CONFIG_UNAVAILABLE);
    }
    this.destroyer_.ensureNotDestroyed();
    try {
      ...
      asserts.assert(this.supportedTypes_.size, 'We should get at least one supported MIME type');
      if (variants.length) {
        this.currentDrmInfo_ = this.createDrmInfoByInfos_(mediaKeySystemAccess.keySystem, drmInfosByKeySystem.get(mediaKeySystemAccess.keySystem));
      } else {
        this.currentDrmInfo_ = DrmEngine.createDrmInfoByConfigs_(mediaKeySystemAccess.keySystem, configsByKeySystem.get(mediaKeySystemAccess.keySystem));
      }
      if (!this.currentDrmInfo_.licenseServerUri) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.NO_LICENSE_SERVER_GIVEN, this.currentDrmInfo_.keySystem);
      }
      const mediaKeys = await mediaKeySystemAccess.createMediaKeys();
      this.destroyer_.ensureNotDestroyed();
      log.info('Created MediaKeys object for key system', this.currentDrmInfo_.keySystem);
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.FAILED_TO_CREATE_CDM, exception.message);
    }
  }
  ...
  /**
     ...
     */ 
  private getKeySystemAccessFromVariants_(variants: shaka.extern.Variant[], drmInfosByKeySystem: Map<string, shaka.extern.DrmInfo[]>): MediaKeySystemAccess {
    for (const variant of variants) {
    ...
    if (drmInfosByKeySystem.size == 1 && drmInfosByKeySystem.has('')) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.NO_RECOGNIZED_KEY_SYSTEMS);
    }
    ...
    for (const preferredKeySystem of this.config_.preferredKeySystems) {
      for (const variant of variants) {
        const decodingInfo = variant.decodingInfos.find( 
        (decodingInfo) => {
          return decodingInfo.supported && decodingInfo.keySystemAccess != null && decodingInfo.keySystemAccess.keySystem == preferredKeySystem;
        });
        if (decodingInfo) {
    ...
    for (const shouldHaveLicenseServer of [true, false]) {
      for (const variant of variants) {
        for (const decodingInfo of variant.decodingInfos) {
          ...
          const drmInfos = drmInfosByKeySystem.get(decodingInfo.keySystemAccess.keySystem);
          for (const info of drmInfos) {
  ...
  /**
     ...
     */ 
  private async getKeySystemAccessByConfigs_(configsByKeySystem: Map<string, MediaKeySystemConfiguration>): Promise<MediaKeySystemAccess> {
    ...
    for (const shouldHaveLicenseServer of [true, false]) {
      for (const keySystem of configsByKeySystem.keys()) {
        ...
        const hasLicenseServer = config['drmInfos'].some( 
        (info) => {
  ...
  private configureClearKey_(): shaka.extern.DrmInfo | null {
    ...
    (keyHex, keyIdHex) => {
      ...
      const keyObj = {kty:'oct', kid:Uint8ArrayUtils.toBase64(keyId, false), k:Uint8ArrayUtils.toBase64(key, false)};
      keys.push(keyObj);
    ...
    return {keySystem:'org.w3.clearkey', licenseServerUri:'data:application/json;base64,' + window.btoa(license), distinctiveIdentifierRequired:false, persistentStateRequired:false, audioRobustness:'', videoRobustness:'', serverCertificate:null, serverCertificateUri:'', sessionType:'', initData:initDatas, keyIds:new Set(keyIds)};
  }
   
  private async loadOfflineSession_(sessionId: string): Promise<MediaKeySession> {
    let session;
    ...
    try {
      ...
      const error = new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.FAILED_TO_CREATE_SESSION, exception.message);
      this.onError_(error);
      return Promise.reject(error);
    }
    this.eventManager_.listen(session, 'message', ( 
    (event) => this.onSessionMessage_(event) as EventManagerExports.ListenerType));
    this.eventManager_.listen(session, 'keystatuseschange',  
    (event) => this.onKeyStatusesChange_(event));
    const metadata = {initData:null, initDataType:null, loaded:false, oldExpiration:Infinity, updatePromise:null, type:sessionType};
    ...
    try {
      ...
      if (!present) {
        this.activeSessions_.delete(session);
        this.onError_(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.OFFLINE_SESSION_REMOVED));
        return Promise.resolve();
      }
      ...
      this.activeSessions_.delete(session);
      this.onError_(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.FAILED_TO_CREATE_SESSION, error.message));
    }
    return Promise.resolve();
  }
   
  createSession(initDataType: string, initData: Uint8Array, sessionType: string) {
    asserts.assert(this.mediaKeys_, 'mediaKeys_ should be valid when creating temporary session.');
    let session;
    try {
      log.info('Creating new', sessionType, 'session');
      session = this.mediaKeys_.createSession(sessionType);
    } catch (exception) {
      this.onError_(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.FAILED_TO_CREATE_SESSION, exception.message));
      return;
    }
    this.eventManager_.listen(session, 'message', ( 
    (event) => this.onSessionMessage_(event) as EventManagerExports.ListenerType));
    this.eventManager_.listen(session, 'keystatuseschange',  
    (event) => this.onKeyStatusesChange_(event));
    const metadata = {initData:initData, initDataType:initDataType, loaded:false, oldExpiration:Infinity, updatePromise:null, type:sessionType};
    this.activeSessions_.set(session, metadata);
    try {
      initData = this.config_.initDataTransform(initData, initDataType, this.currentDrmInfo_);
    } catch (error) {
      let shakaError = error;
      if (!(error instanceof Error)) {
        shakaError = new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.INIT_DATA_TRANSFORM_ERROR, error);
      }
    ...
    (error) => {
      ...
      this.onError_(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.FAILED_TO_GENERATE_LICENSE_REQUEST, error.message, error, extended));
    });
  }
  ...
  private async sendLicenseRequest_(event: MediaKeyMessageEvent) {
    ...
    log.v1('Sending license request for session', session.sessionId, 'of type', event.messageType);
    if (this.config_.logLicenseExchange) {
    ...
    if (event.messageType == 'individualization-request' && advancedConfig && advancedConfig.individualizationServer) {
      url = advancedConfig.individualizationServer;
    }
    const requestType = NetworkingEngineExports.RequestType.LICENSE;
    const request = NetworkingEngine.makeRequest([url], this.config_.retryParameters);
    ...
    try {
      ...
      // Request failed! 
      asserts.assert(error instanceof Error, 'Wrong NetworkingEngine error type!');
      const shakaErr = new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.LICENSE_REQUEST_FAILED, error);
    ...
    try {
      ...
      const shakaErr = new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.LICENSE_RESPONSE_REJECTED, error.message);
      this.onError_(shakaErr);
      if (metadata && metadata.updatePromise) {
        metadata.updatePromise.reject(shakaErr);
      }
  ...
  private unpackPlayReadyRequest_(request: shaka.extern.Request) {
    ...
    //   </LicenseAcquisition>
    // </PlayReadyKeyMessage> 
    const xml = StringUtils.fromUTF16(request.body,  
    /* littleEndian= */ 
    true,  
    /* noThrow= */ 
    true);
    ...
    asserts.assert(challenge.getAttribute('encoding') == 'base64encoded', 'Unexpected PlayReady challenge encoding!');
  ...
  private onKeyStatusesChange_(event: Event) {
    ...
    (status, keyId) => {
      ...
      if (DrmEngine.isPlayReadyKeySystem(this.currentDrmInfo_.keySystem) && keyId.byteLength == 16 && (Platform.isEdge() || Platform.isPS4())) {
         
        // Read out some fields in little-endian: 
        const dataView = BufferUtils.toDataView(keyId);
        const part0 = dataView.getUint32(0,  
        /* LE= */ 
        true);
        const part1 = dataView.getUint16(4,  
        /* LE= */ 
        true);
        const part2 = dataView.getUint16(6,  
        /* LE= */ 
        true);
         
        // Write it back in big-endian: 
        dataView.setUint32(0, part0,  
        /* BE= */ 
        false);
        dataView.setUint16(4, part1,  
        /* BE= */ 
        false);
        dataView.setUint16(6, part2,  
        /* BE= */ 
        false);
      }
      ...
      if (!found) {
        ...
        asserts.assert(status != 'usable', 'Usable keys found in closed session');
      }
      if (status == 'expired') {
        hasExpiredKeys = true;
      }
  ...
  private processKeyStatusChanges_() {
    ...
    publicMap.clear();
    privateMap.forEach( 
    (status, keyId) => publicMap.set(keyId, status));
    ...
    const allExpired = statuses.length && statuses.every( 
    (status) => status == 'expired');
    if (allExpired) {
      this.onError_(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.DRM, ErrorExports.Code.EXPIRED));
    }
  ...
  static isBrowserSupported(): boolean {
    const basic = !!window.MediaKeys && !!window.navigator && !!window.navigator.requestMediaKeySystemAccess && !!window.MediaKeySystemAccess &&  
  ...
  /**
     ...
     *
     */ 
  static async probeSupport(): Promise<{[key:string]:shaka.extern.DrmSupportType | null}> {
    asserts.assert(DrmEngine.isBrowserSupported(), 'Must have basic EME support');
    const testKeySystems = ['org.w3.clearkey', 'com.widevine.alpha', 'com.microsoft.playready', 'com.microsoft.playready.recommendation', 'com.apple.fps.1_0', 'com.apple.fps', 'com.adobe.primetime'];
    const basicVideoCapabilities = [{contentType:'video/mp4; codecs="avc1.42E01E"'}, {contentType:'video/webm; codecs="vp8"'}];
    const basicConfig = {initDataTypes:['cenc'], videoCapabilities:basicVideoCapabilities};
    const offlineConfig = {videoCapabilities:basicVideoCapabilities, persistentState:'required', sessionTypes:['persistent-license']};
    ...
    async(keySystem) => {
      try {
        ...
        let persistentState = sessionTypes ? sessionTypes.includes('persistent-license') : false;
    ...
    const tests = testKeySystems.map( 
    (keySystem) => testSystem(keySystem));
  ...
  private async closeSession_(session: MediaKeySession): Promise {
    ...
    try {
      await Promise.race([Promise.all([session.close(), session.closed]), timeout]);
    } catch (e) {
  ...
  private async closeOpenSessions_() {
    ...
    await Promise.all(openSessions.map( 
    async([session, metadata]) => {
      try {
        ...
        /**
                 ...
                 */ 
        if (!this.initializedForStorage_ && !this.offlineSessionIds_.includes(session.sessionId) && metadata.type === 'persistent-license') {
  ...
  supportsVariant(variant: shaka.extern.Variant): boolean {
    ...
    const drmInfos = this.getVariantDrmInfos_(variant);
    return drmInfos.length == 0 || drmInfos.some( 
    (drmInfo) => drmInfo.keySystem == keySystem);
  }
  ...
  /**
     ...
     *
     */ 
  static areDrmCompatible(drms1: shaka.extern.DrmInfo[], drms2: shaka.extern.DrmInfo[]): boolean {
  ...
  /**
     ...
     *
     */ 
  static getCommonDrmInfos(drms1: shaka.extern.DrmInfo[], drms2: shaka.extern.DrmInfo[]): shaka.extern.DrmInfo[] {
    if (!drms1.length) {
      return drms2;
    }
    if (!drms2.length) {
    ...
    for (const drm1 of drms1) {
      for (const drm2 of drms2) {
        ...
        if (drm1.keySystem == drm2.keySystem) {
          ...
          initData = initData.filter( 
          (d, i) => {
            return d.keyId === undefined || i === initData.findIndex( 
            (d2) => {
              return d2.keyId === d.keyId;
            });
          });
          const keyIds = drm1.keyIds && drm2.keyIds ? new Set([...drm1.keyIds, ...drm2.keyIds]) : drm1.keyIds || drm2.keyIds;
          const mergedDrm = {keySystem:drm1.keySystem, licenseServerUri:drm1.licenseServerUri || drm2.licenseServerUri, distinctiveIdentifierRequired:drm1.distinctiveIdentifierRequired || drm2.distinctiveIdentifierRequired, persistentStateRequired:drm1.persistentStateRequired || drm2.persistentStateRequired, videoRobustness:drm1.videoRobustness || drm2.videoRobustness, audioRobustness:drm1.audioRobustness || drm2.audioRobustness, serverCertificate:drm1.serverCertificate || drm2.serverCertificate, 
          serverCertificateUri:drm1.serverCertificateUri || drm2.serverCertificateUri, initData, keyIds};
  ...
  /**
     ...
     */ 
  private getVariantDrmInfos_(variant: shaka.extern.Variant): shaka.extern.DrmInfo[] {
  ...
  private areAllSessionsLoaded_(): boolean {
    const metadatas = this.activeSessions_.values();
    return Iterables.every(metadatas,  
    (data) => data.loaded);
  }
  ...
  /**
     ...
     *
     */ 
  private static replaceDrmInfo_(variants: shaka.extern.Variant[], keySystems: Map<string, string>) {
    const drmInfos = [];
    keySystems.forEach( 
    (uri, keySystem) => {
      drmInfos.push({keySystem:keySystem, licenseServerUri:uri, distinctiveIdentifierRequired:false, persistentStateRequired:false, audioRobustness:'', videoRobustness:'', serverCertificate:null, serverCertificateUri:'', initData:[], keyIds:new Set()});
    });
    for (const variant of variants) {
      if (variant.video) {
  ...
  /**
     ...
     *
     */ 
  private createDrmInfoByInfos_(keySystem: string, drmInfos: shaka.extern.DrmInfo[]): shaka.extern.DrmInfo {
    ...
    DrmEngine.processDrmInfos_(drmInfos, licenseServers, serverCerts, serverCertificateUris, initDatas, keyIds);
    if (serverCerts.length > 1) {
      log.warning('Multiple unique server certificates found! ' + 'Only the first will be used.');
    }
    if (licenseServers.length > 1) {
      log.warning('Multiple unique license server URIs found! ' + 'Only the first will be used.');
    }
    if (serverCertificateUris.length > 1) {
      log.warning('Multiple unique server certificate URIs found! ' + 'Only the first will be used.');
    }
    const defaultSessionType = this.usePersistentLicenses_ ? 'persistent-license' : 'temporary';
    const res: shaka.extern.DrmInfo = {keySystem, licenseServerUri:licenseServers[0], distinctiveIdentifierRequired:drmInfos[0].distinctiveIdentifierRequired, persistentStateRequired:drmInfos[0].persistentStateRequired, sessionType:drmInfos[0].sessionType || defaultSessionType, audioRobustness:drmInfos[0].audioRobustness || '', videoRobustness:drmInfos[0].videoRobustness || '', serverCertificate:serverCerts[0], serverCertificateUri:serverCertificateUris[0], initData:initDatas, keyIds};
    for (const info of drmInfos) {
  ...
  /**
     ...
     *
     */ 
  private static createDrmInfoByConfigs_(keySystem: string, config: MediaKeySystemConfiguration): shaka.extern.DrmInfo {
    const licenseServers: string[] = [];
    const serverCertificateUris: string[] = [];
    const serverCerts: Uint8Array[] = [];
    const initDatas: shaka.extern.InitDataOverride[] = [];
    ...
    // TODO: refactor, don't stick drmInfos onto MediaKeySystemConfiguration 
    DrmEngine.processDrmInfos_(config['drmInfos'], licenseServers, serverCerts, serverCertificateUris, initDatas, keyIds);
    if (serverCerts.length > 1) {
      log.warning('Multiple unique server certificates found! ' + 'Only the first will be used.');
    }
    if (serverCertificateUris.length > 1) {
    ...
    const audioRobustness = config.audioCapabilities ? config.audioCapabilities[0].robustness : '';
    const videoRobustness = config.videoCapabilities ? config.videoCapabilities[0].robustness : '';
    const distinctiveIdentifier = config.distinctiveIdentifier;
    return {keySystem, licenseServerUri:licenseServers[0], distinctiveIdentifierRequired:distinctiveIdentifier == 'required', persistentStateRequired:config.persistentState == 'required', sessionType:config.sessionTypes[0] || 'temporary', audioRobustness:audioRobustness || '', videoRobustness:videoRobustness || '', serverCertificate:serverCerts[0], serverCertificateUri:serverCertificateUris[0], initData:initDatas, keyIds};
  }
  ...
  /**
     ...
     *
     */ 
  private static processDrmInfos_(drmInfos: shaka.extern.DrmInfo[], licenseServers: string[], serverCerts: Uint8Array[], serverCertificateUris: string[], initDatas: shaka.extern.InitDataOverride[], keyIds: Set<string>) {
    const initDataOverrideEqual: (p1: shaka.extern.InitDataOverride, p2: shaka.extern.InitDataOverride) => boolean =  
    (a, b) => {
      ...
      return a.initDataType == b.initDataType && BufferUtils.equal(a.initData, b.initData);
    };
  ...
  /**
     ...
     *   advancedConfigs
     */ 
  private static fillInDrmInfoDefaults_(drmInfo: shaka.extern.DrmInfo, servers: Map<string, string>, advancedConfigs: Map<string, shaka.extern.AdvancedDrmConfiguration>, keySystemsMapping: {[key:string]:string}) {
    ...
    if (advancedConfig) {
      if (!drmInfo.distinctiveIdentifierRequired) {
        drmInfo.distinctiveIdentifierRequired = advancedConfig.distinctiveIdentifierRequired;
      }
...
type SessionMetaData = {loaded:boolean, initData:Uint8Array, initDataType:string | null, oldExpiration:number, type:string, updatePromise:PublicPromise};
 
export{SessionMetaData};
type PlayerInterface = {netEngine:NetworkingEngine, onError:(p1: Error) => any, onKeyStatus:(p1: {[key:string]:string}) => any, onExpirationUpdated:(p1: string, p2: number) => any, onEvent:(p1: Event) => any};
 ...
 */ 
export const DUMMY_KEY_ID: Lazy<ArrayBuffer> = new Lazy( 
() => BufferUtils.toArrayBuffer(new Uint8Array([0])));
