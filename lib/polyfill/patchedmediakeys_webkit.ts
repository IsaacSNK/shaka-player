  ...
  static install() {
    ...
    if (HTMLMediaElement.prototype.webkitGenerateKeyRequest) {
      ...
      if (HTMLMediaElement.prototype.generateKeyRequest) {
        log.info('Using nonprefixed EME v0.1b');
      } else {
        return;
      }
    }
...
export class MediaKeys implements MediaKeys {
  ...
  private onWebkitKeyMessage_(event: MediaKeyEvent) {
    ...
    if (!session) {
      log.error('Session not found', event.sessionId);
      return;
    }
  ...
  private onWebkitKeyAdded_(event: MediaKeyEvent) {
    ...
    const session = this.findSession_(event.sessionId);
    asserts.assert(session, 'unable to find session in onWebkitKeyAdded_');
    if (session) {
      session.ready();
    }
  }
  ...
  private onWebkitKeyError_(event: MediaKeyEvent) {
    ...
    const session = this.findSession_(event.sessionId);
    asserts.assert(session, 'unable to find session in onWebkitKeyError_');
    if (session) {
  ...
  private findSession_(sessionId: string): MediaKeySession {
    ...
    if (session) {
      log.debug('PatchedMediaKeysWebkit.MediaKeys.findSession_', session);
      return session;
    }
...
export class MediaKeySession extends FakeEventTarget implements
  ...
  private generate_(initData: BufferSource|null, offlineSessionId: string|null):
    ...
    try {
      ...
      asserts.assert(mangledInitData, 'init data not set!');
    } catch (exception) {
      return Promise.reject(exception);
    }
    asserts.assert(
