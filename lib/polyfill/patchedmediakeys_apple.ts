...
export class PatchedMediaKeysApple {
  ...
  private static onWebkitNeedKey_(event: MediaKeyEvent) {
    ...
    const str = StringUtils.fromUTF16(
        uint8.subarray(4),
        /* littleEndian= */
        true);
...
export class MediaKeySystemAccess implements MediaKeySystemAccess {
   ...
   */
  private checkConfig_(cfg: MediaKeySystemConfiguration):
...
export class MediaKeys implements MediaKeys {
  ...
  protected setMedia(media: HTMLMediaElement): Promise {
    ...
    this.eventManager_.listen(
        media, 'webkitneedkey',
        (PatchedMediaKeysApple.onWebkitNeedKey_ as
    ...
    try {
      ...
      MediaReadyState.waitForReadyState(
          media, HTMLMediaElement.HAVE_METADATA, this.eventManager_, () => {
...
export class MediaKeySession extends FakeEventTarget implements
    ...
    try {
      ...
      this.eventManager_.listen(
          ...
          ((event) => this.onWebkitKeyMessage_(event) as
               EventManagerExports.ListenerType));
      this.eventManager_.listen(
          session, 'webkitkeyadded',
          ((event) => this.onWebkitKeyAdded_(event) as
               EventManagerExports.ListenerType));
      this.eventManager_.listen(
          session, 'webkitkeyerror',
  ...
  private onWebkitKeyMessage_(event: MediaKeyEvent) {
    ...
    const data =
        (new Map())
            ...
            .set('message', BufferUtils.toArrayBuffer(event.message));
  ...
  private onWebkitKeyAdded_(event: MediaKeyEvent) {
    ...
    asserts.assert(
        this.updatePromise_, 'updatePromise_ should be set before now!');
