...
export class Deprecate {
  ...
  static init(versionString: string) {
    asserts.assert(
        enforcer_ == null, 'Deprecate.init should only be called once.');
    enforcer_ = new Enforcer(
        Version.parse(versionString), Deprecate.onPending_,
  ...
  static deprecateFeature(major: number, name: string, description: string) {
    ...
    asserts.assert(
        enforcer, 'Missing deprecation enforcer. Was |init| called?');
    const expiresAt = new Version(major, 0);
    enforcer.enforce(expiresAt, name, description);
  }

  private static onPending_(
      libraryVersion: Version, featureVersion: Version, name: string,
      description: string) {
    ...
    log.alwaysWarn([
      name, 'has been deprecated and will be removed in', featureVersion,
      '. We are currently at version', libraryVersion,
  ...
  private static onExpired_(
    ...
    // the reader, create one string for the message.
    const errorMessage = [
      name, 'has been deprecated and has been removed in', featureVersion,
      '. We are now at version', libraryVersion,
      '. Additional information:', description
    ].join('');
