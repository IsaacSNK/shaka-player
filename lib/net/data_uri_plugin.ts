...
export class DataUriPlugin {
  ...
  /**
     ...
     * @export
     */ 
  static parse(uri: string, request: shaka.extern.Request, requestType: NetworkingEngineExports.RequestType, progressUpdated: shaka.extern.ProgressUpdated): shaka.extern.IAbortableOperation<shaka.extern.Response> {
    try {
      const parsed = DataUriPlugin.parseRaw(uri);
      const response: shaka.extern.Response = {uri:uri, originalUri:uri, data:parsed.data, headers:{'content-type':parsed.contentType}};
  ...
  static parseRaw(uri: string): {data:BufferSource, contentType:string} {
    ...
    if (parts.length < 2 || parts[0] != 'data') {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.NETWORK, ErrorExports.Code.MALFORMED_DATA_URI, uri);
    }
