...
export class NetworkingEngine extends FakeEventTarget implements IDestroyable {
  ...
  /**
     ...
     */ 
  constructor(onProgressUpdated?: (p1: number, p2: number) => any, onHeadersReceived?: OnHeadersReceived, onDownloadFailed?: OnDownloadFailed) {
    super();
  ...
  /**
     ...
     * @export
     */ 
  static registerScheme(scheme: string, plugin: shaka.extern.SchemePlugin, priority?: number, progressSupport: boolean = false) {
    ...
    if (!existing || priority >= existing.priority) {
      schemes_[scheme] = {priority:priority, plugin:plugin, progressSupport:progressSupport};
    }
  }
  ...
  /**
     ...
     * @export
     */ 
  static makeRequest(uris: string[], retryParams: shaka.extern.RetryParameters, streamDataCallback: ((p1: BufferSource) => Promise) | null = null): shaka.extern.Request {
    return {uris:uris, method:'GET', body:null, headers:{}, allowCrossSiteCredentials:false, retryParameters:retryParams, licenseRequestType:null, sessionId:null, drmInfo:null, initData:null, initDataType:null, streamDataCallback:streamDataCallback};
  }
  ...
  request(type: RequestType, request: shaka.extern.Request): PendingRequest {
    ...
    if (this.destroyed_) {
      const p = Promise.reject(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.PLAYER, ErrorExports.Code.OPERATION_ABORTED));
      ...
      return new PendingRequest(p,  
      () => Promise.resolve(), numBytesRemainingObj);
    }
    ...
    request.retryParameters = request.retryParameters ? ObjectUtils.cloneObject(request.retryParameters) : NetworkingEngine.defaultRetryParameters();
    ...
    (responseAndGotProgress) => this.filterResponse_(type, responseAndGotProgress));
    ...
    (responseAndGotProgress) => {
      ...
      if (!responseAndGotProgress.gotProgress && this.onProgressUpdated_ && !response.fromCache && type == RequestType.SEGMENT) {
    ...
    const pendingRequest = new PendingRequest(op.promise,  
    () => op.abort(), numBytesRemainingObj);
    this.operationManager_.manage(pendingRequest);
    return pendingRequest;
  }
   
  private filterRequest_(type: RequestType, request: shaka.extern.Request): AbortableOperation<undefined> {
    ...
    return filterOperation.chain(undefined,  
    (e) => {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.NETWORK, ErrorExports.Code.REQUEST_FILTER_ERROR, e);
    });
  }
  ...
  /**
     ...
     */ 
  private makeRequestWithRetry_(type: RequestType, request: shaka.extern.Request, numBytesRemainingObj: NumBytesRemainingClass): shaka.extern.IAbortableOperation<ResponseAndGotProgress> {
    const backoff = new Backoff(request.retryParameters,  
    /* autoReset= */ 
    false);
    const index = 0;
    return this.send_(type, request, backoff, index,  
    /* lastError= */ 
  ...
  /**
     ...
     *               shaka.net.NetworkingEngine.ResponseAndGotProgress>}
     */ 
  private send_(type: RequestType, request: shaka.extern.Request, backoff: Backoff, index: number, lastError: Error | null, numBytesRemainingObj: NumBytesRemainingClass): shaka.extern.IAbortableOperation<ResponseAndGotProgress> {
    if (this.forceHTTPS_) {
    ...
    if (!plugin) {
      return AbortableOperation.failed(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.NETWORK, ErrorExports.Code.UNSUPPORTED_SCHEME, uri));
    }
    ...
    (response) => {
      ...
      const responseAndGotProgress = {response:response, gotProgress:gotProgress};
    ...
    (error) => {
      ...
      if (aborted) {
        ...
        error = new Error(ErrorExports.Severity.RECOVERABLE, ErrorExports.Category.NETWORK, ErrorExports.Code.TIMEOUT, request.uris[index], type);
      }
      if (error instanceof Error) {
        ...
        if (error.severity == ErrorExports.Severity.RECOVERABLE) {
          ...
          return this.send_(type, request, backoff, index, error, numBytesRemainingObj);
        }
      }
  ...
  /**
     ...
     *               shaka.net.NetworkingEngine.ResponseAndGotProgress>}
     */ 
  private filterResponse_(type: RequestType, responseAndGotProgress: ResponseAndGotProgress): shaka.extern.IAbortableOperation<ResponseAndGotProgress> {
    let filterOperation = AbortableOperation.completed(undefined);
    ...
    (e) => {
      ...
      throw new Error(severity, ErrorExports.Category.NETWORK, ErrorExports.Code.RESPONSE_FILTER_ERROR, e);
    });
  }
 ...
 * @export
 */ 
export class PendingRequest extends AbortableOperation implements shaka.extern.IAbortableOperation<shaka.extern.Response> {
  ...
  /**
     ...
     *   numBytesRemainingObj
     */ 
  constructor(promise: Promise, onAbort: () => Promise, numBytesRemainingObj: NumBytesRemainingClass) {
...
type SchemeObject = {plugin:shaka.extern.SchemePlugin, priority:number, progressSupport:boolean};
 
export{SchemeObject};
type ResponseAndGotProgress = {response:shaka.extern.Response, gotProgress:boolean};
 
export{ResponseAndGotProgress};
type OnHeadersReceived = (p1: {[key:string]:string}, p2: shaka.extern.Request, p3: RequestType) => any;
 
export{OnHeadersReceived};
type OnDownloadFailed = (p1: shaka.extern.Request, p2: Error | null, p3: number, p4: boolean) => any;
