...
export class HttpXHRPlugin {
  ...
  static parse(uri: string, request: shaka.extern.Request, requestType: NetworkingEngineExports.RequestType, progressUpdated: shaka.extern.ProgressUpdated, headersReceived: shaka.extern.HeadersReceived): shaka.extern.IAbortableOperation<shaka.extern.Response> {
    ...
    (resolve, reject) => {
      ...
      xhr.onabort =  
      () => {
        reject(new Error(ErrorExports.Severity.RECOVERABLE, ErrorExports.Category.NETWORK, ErrorExports.Code.OPERATION_ABORTED, uri, requestType));
      };
      ...
      (event) => {
        const headers = HttpXHRPlugin.headersToGenericObject_(xhr);
        asserts.assert(xhr.response instanceof ArrayBuffer, 'XHR should have a response by now!');
        const xhrResponse = xhr.response;
        try {
          const response = HttpPluginUtils.makeResponse(headers, xhrResponse, xhr.status, uri, xhr.responseURL, requestType);
          resolve(response);
        } catch (error) {
      ...
      xhr.onerror =  
      (event) => {
        reject(new Error(ErrorExports.Severity.RECOVERABLE, ErrorExports.Category.NETWORK, ErrorExports.Code.HTTP_ERROR, uri, event, requestType));
      };
      xhr.ontimeout =  
      (event) => {
        reject(new Error(ErrorExports.Severity.RECOVERABLE, ErrorExports.Category.NETWORK, ErrorExports.Code.TIMEOUT, uri, requestType));
      };
      xhr.onprogress =  
      (event) => {
  ...
  private static headersToGenericObject_(xhr: XMLHttpRequest): {[key:string]:string} {
...
NetworkingEngine.registerScheme('http', HttpXHRPlugin.parse, NetworkingEngineExports.PluginPriority.FALLBACK,  
/* progressSupport= */ 
true);
NetworkingEngine.registerScheme('https', HttpXHRPlugin.parse, NetworkingEngineExports.PluginPriority.FALLBACK,  
/* progressSupport= */ 
true);
NetworkingEngine.registerScheme('blob', HttpXHRPlugin.parse, NetworkingEngineExports.PluginPriority.FALLBACK,  
/* progressSupport= */ 
true);
