  ...
  export class Backoff {
    ...
    async attempt(): Promise {
      ...
      if (currentAttempt == 0) {
        goog.asserts.assert(
          !this.autoReset_,
          "Failed to delay with auto-reset!"
        );
        return;
      }
    ...
    private reset_() {
      goog.asserts.assert(
        this.autoReset_,
        "Should only be used for auto-reset!"
      );
      this.numAttempts_ = 1;
      this.nextUnfuzzedDelay_ = this.baseDelay_;
    }
