...
export class Backoff {
  ...
  /**
     ...
     */ 
  constructor(parameters: shaka.extern.RetryParameters, autoReset: boolean = false) {
  ...
  async attempt(): Promise {
    if (this.numAttempts_ >= this.maxAttempts_) {
      if (this.autoReset_) {
        ...
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.PLAYER, ErrorExports.Code.ATTEMPTS_EXHAUSTED);
      }
    }
  ...
  static defaultRetryParameters(): shaka.extern.RetryParameters {
    ...
    return {maxAttempts:2, baseDelay:1000, backoffFactor:2, fuzzFactor:0.5, timeout:30000, stallTimeout:5000, connectionTimeout:10000};
  }
