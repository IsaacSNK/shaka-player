 ...
 *
 */ 
export class EmeSessionStorageCell implements shaka.extern.EmeSessionStorageCell {
  ...
  /** @override */ 
  async getAll() {
    const op: DBOperation = this.connection_.startReadOnlyOperation(this.store_);
  ...
  async remove(sessionIds) {
    const op: DBOperation = this.connection_.startReadWriteOperation(this.store_);
    await op.forEachEntry( 
    (key, value, cursor) => {
