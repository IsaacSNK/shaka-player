 ...
 *
 */ 
export class V1StorageCell extends BaseStorageCell implements shaka.extern.StorageCell {
  ...
  async updateManifestExpiration(key, newExpiration) {
    ...
    (event) => {
      ...
      if (manifest) {
        ...
        asserts.assert(manifest.key == key, 'With in-line keys, the keys should match');
        manifest.expiration = newExpiration;
        store.put(manifest);
        p.resolve();
      } else {
        p.reject(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.KEY_NOT_FOUND, 'Could not find values for ' + key));
      }
    };
  ...
  /**
     * @override
     */ 
  async convertManifest(old: shaka.extern.ManifestDBV1): Promise<shaka.extern.ManifestDB> {
    ...
    return {creationTime:0, originalManifestUri:old.originalManifestUri, duration:old.duration, size:old.size, expiration:old.expiration == null ? Infinity : old.expiration, streams, sessionIds:old.sessionIds, drmInfo:old.drmInfo, appMetadata:old.appMetadata, sequenceMode:false};
  }
   
  private static convertPeriod_(old: shaka.extern.PeriodDBV1, periodDuration: number): shaka.extern.StreamDB[] {
    const V1StorageCell = V1StorageCell;
    ...
    (stream) => V1StorageCell.convertStream_(stream, old.startTime, periodDuration));
  }
   
  private static convertStream_(old: shaka.extern.StreamDBV1, periodStart: number, periodDuration: number): shaka.extern.StreamDB {
    const V1StorageCell = V1StorageCell;
    ...
    return {id:old.id, originalId:null, primary:old.primary, type:old.contentType, mimeType:old.mimeType, codecs:old.codecs, frameRate:old.frameRate, pixelAspectRatio:undefined, hdr:undefined, kind:old.kind, language:old.language, label:old.label, width:old.width, height:old.height, initSegmentKey:initSegmentKey, encrypted:old.encrypted, keyIds:new Set([old.keyId]), segments:old.segments.map( 
    (segment) => V1StorageCell.convertSegment_(segment, initSegmentKey, appendWindowStart, appendWindowEnd, timestampOffset)), variantIds:old.variantIds, roles:[], forced:false, audioSamplingRate:null, channelsCount:null, spatialAudio:false, closedCaptions:null, tilesLayout:undefined};
  }
   
  private static convertSegment_(old: shaka.extern.SegmentDBV1, initSegmentKey: number | null, appendWindowStart: number, appendWindowEnd: number, timestampOffset: number): shaka.extern.SegmentDB {
    const V1StorageCell = V1StorageCell;
    ...
    return {startTime:appendWindowStart + old.startTime, endTime:appendWindowStart + old.endTime, dataKey, initSegmentKey, appendWindowStart, appendWindowEnd, timestampOffset, tilesLayout:''};
  }
   
  /**
     * @override
     */ 
  convertSegmentData(old: shaka.extern.SegmentDataDBV1): shaka.extern.SegmentDataDB {
  ...
  private static getKeyFromSegmentUri_(uri: string): number {
    ...
    throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.MALFORMED_OFFLINE_URI, 'Could not parse uri ' + uri);
  }
  ...
  private static fillMissingVariants_(period: shaka.extern.PeriodDBV1) {
    ...
    const audio = period.streams.filter( 
    (s) => s.contentType == AUDIO);
    const video = period.streams.filter( 
    (s) => s.contentType == VIDEO);
     
    // Case 2 - There is nothing we need to do, so let's just get out of here. 
    if (audio.every( 
    (s) => s.variantIds) && video.every( 
    (s) => s.variantIds)) {
      return;
    }
     
    // Case 3... We don't want to be in case three. 
    asserts.assert(audio.every( 
    (s) => !s.variantIds), 'Some audio streams have variant ids and some do not.');
    asserts.assert(video.every( 
    (s) => !s.variantIds), 'Some video streams have variant ids and some do not.');
