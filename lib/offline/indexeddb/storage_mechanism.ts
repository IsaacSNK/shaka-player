...
export class StorageMechanism implements shaka.extern.StorageMechanism {
  ...
  init() {
    ...
    open.onerror =  
    (event) => {
      p.reject(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.INDEXED_DB_ERROR, open.error));
  ...
  private static createEmeSessionCell_(db: IDBDatabase): shaka.extern.EmeSessionStorageCell {
    const StorageMechanism = StorageMechanism;
  ...
  private static deleteAll_(): Promise {
    ...
    del.onerror =  
    (event) => {
      p.reject(new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.INDEXED_DB_ERROR, del.error));
