...
type StorageCellPath = {mechanism:string, cell:string};
 
export{StorageCellPath};
type StorageCellHandle = {path:StorageCellPath, cell:shaka.extern.StorageCell};
...
export class StorageMuxer implements IDestroyable {
  ...
  init(): Promise {
    ...
    (factory, name) => {
      ...
      if (mech) {
        ...
        log.info('Skipping ' + name + ' as it is not supported on this platform');
      }
    });
  ...
  getActive(): StorageCellHandle {
    ...
    this.mechanisms_.forEach( 
    (mechanism, mechanismName) => {
      mechanism.getCells().forEach( 
      (cell, cellName) => {
    ...
    throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.MISSING_STORAGE_CELL, 'Could not find a cell that supports add-operations');
  }
  ...
  /**
     ...
     */ 
  forEachCell(callback: (p1: StorageCellPath, p2: shaka.extern.StorageCell) => any) {
    this.mechanisms_.forEach( 
    (mechanism, mechanismName) => {
      mechanism.getCells().forEach( 
      (cell, cellName) => {
  ...
  getCell(mechanismName: string, cellName: string): shaka.extern.StorageCell {
    ...
    if (!mechanism) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.MISSING_STORAGE_CELL, 'Could not find mechanism with name ' + mechanismName);
    }
    const cell = mechanism.getCells().get(cellName);
    if (!cell) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.MISSING_STORAGE_CELL, 'Could not find cell with name ' + cellName);
    }
    return cell;
  }
   
  forEachEmeSessionCell(callback: (p1: shaka.extern.EmeSessionStorageCell) => any) {
    this.mechanisms_.forEach( 
    (mechanism, name) => {
  ...
  getEmeSessionCell(): shaka.extern.EmeSessionStorageCell {
    ...
    if (!mechanisms.length) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.STORAGE_NOT_SUPPORTED, 'No supported storage mechanisms found');
    }
  ...
  async erase(): Promise {
    ...
    const mechanisms: shaka.extern.StorageMechanism[] = Array.from(this.mechanisms_.values());
    ...
    await Promise.all(mechanisms.map( 
    (m) => m.erase()));
  ...
  /**
     ...
     * @export
     */ 
  static register(name: string, factory: () => shaka.extern.StorageMechanism) {
    registry_.set(name, factory);
  }
  ...
  /**
     ...
     *
     */ 
  static overrideSupport(map: Map<string, () => shaka.extern.StorageMechanism>) {
    override_ = map;
  }
  ...
  /**
     ...
     *
     */ 
  private static getRegistry_(): Map<string, () => shaka.extern.StorageMechanism> {
    const override = override_;
...
export const override_: Map<string, () => shaka.extern.StorageMechanism> = null;
 
export const registry_: Map<string, () => shaka.extern.StorageMechanism> = new Map();
