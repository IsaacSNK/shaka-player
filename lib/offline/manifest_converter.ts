...
export class ManifestConverter {
  ...
  fromManifestDB(manifestDB: shaka.extern.ManifestDB): shaka.extern.Manifest {
    ...
    const audioStreams: shaka.extern.StreamDB[] = manifestDB.streams.filter( 
    (streamDB) => this.isAudio_(streamDB));
    const videoStreams: shaka.extern.StreamDB[] = manifestDB.streams.filter( 
    (streamDB) => this.isVideo_(streamDB));
    const variants: Map<number, shaka.extern.Variant> = this.createVariants(audioStreams, videoStreams, timeline);
    const textStreams: shaka.extern.Stream[] = manifestDB.streams.filter( 
    (streamDB) => this.isText_(streamDB)).map( 
    (streamDB) => this.fromStreamDB_(streamDB, timeline));
    const imageStreams: shaka.extern.Stream[] = manifestDB.streams.filter( 
    (streamDB) => this.isImage_(streamDB)).map( 
    (streamDB) => this.fromStreamDB_(streamDB, timeline));
    ...
    return {presentationTimeline:timeline, minBufferTime:2, offlineSessionIds:manifestDB.sessionIds, variants:Array.from(variants.values()), textStreams:textStreams, imageStreams:imageStreams, sequenceMode:manifestDB.sequenceMode || false};
  }
  ...
  /**
     ...
     *
     */ 
  createVariants(audios: shaka.extern.StreamDB[], videos: shaka.extern.StreamDB[], timeline: PresentationTimeline): Map<number, shaka.extern.Variant> {
    ...
    for (const audio of audios) {
      ...
      for (const variantId of audio.variantIds) {
        ...
        asserts.assert(!variant.audio, 'A variant should only have one audio stream');
    ...
    for (const video of videos) {
      ...
      for (const variantId of video.variantIds) {
        const variant = variantMap.get(variantId);
        asserts.assert(!variant.video, 'A variant should only have one video stream');
        variant.primary = variant.primary || stream.primary;
  ...
  private fromStreamDB_(streamDB: shaka.extern.StreamDB, timeline: PresentationTimeline): shaka.extern.Stream {
    ...
    const stream: shaka.extern.Stream = {id:streamDB.id, originalId:streamDB.originalId, createSegmentIndex: 
    () => Promise.resolve(), segmentIndex, mimeType:streamDB.mimeType, codecs:streamDB.codecs, width:streamDB.width || undefined, height:streamDB.height || undefined, frameRate:streamDB.frameRate, pixelAspectRatio:streamDB.pixelAspectRatio, hdr:streamDB.hdr, kind:streamDB.kind, encrypted:streamDB.encrypted, drmInfos:[], keyIds:streamDB.keyIds, language:streamDB.language, label:streamDB.label, type:streamDB.type, primary:streamDB.primary, trickModeVideo:null, emsgSchemeIdUris:null, roles:streamDB.roles, 
    forced:streamDB.forced, channelsCount:streamDB.channelsCount, audioSamplingRate:streamDB.audioSamplingRate, spatialAudio:streamDB.spatialAudio, closedCaptions:streamDB.closedCaptions, tilesLayout:streamDB.tilesLayout};
    return stream;
  }
   
  private fromSegmentDB_(index: number, segmentDB: shaka.extern.SegmentDB): SegmentReference {
    const uri: OfflineUri = OfflineUri.segment(this.mechanism_, this.cell_, segmentDB.dataKey);
    const initSegmentReference = segmentDB.initSegmentKey != null ? this.fromInitSegmentDB_(segmentDB.initSegmentKey) : null;
    return new SegmentReference(segmentDB.startTime, segmentDB.endTime,  
    () => [uri.toString()],  
    /* startByte= */ 
    0,  
    /* endByte= */ 
    null, initSegmentReference, segmentDB.timestampOffset, segmentDB.appendWindowStart, segmentDB.appendWindowEnd,  
    /* partialReferences= */ 
    [], segmentDB.tilesLayout || '');
  }
   
  private fromInitSegmentDB_(key: number): InitSegmentReference {
    const uri: OfflineUri = OfflineUri.segment(this.mechanism_, this.cell_, key);
  ...
  private createEmptyVariant_(id: number): shaka.extern.Variant {
    return {id:id, language:'', disabledUntilTime:0, primary:false, audio:null, video:null, bandwidth:0, allowedByApplication:true, allowedByKeySystem:true, decodingInfos:[]};
  }
}
