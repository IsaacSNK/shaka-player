...
namespace shaka.offline {
  ...
  export class ManifestConverter {
    ...
    fromManifestDB(manifestDB: shaka.extern.ManifestDB): shaka.extern.Manifest {
      ...
      const variants: Map<number, shaka.extern.Variant> = this.createVariants(
        audioStreams,
        videoStreams,
        timeline
      );
      const textStreams: shaka.extern.Stream[] = manifestDB.streams
        .filter((streamDB) => this.isText_(streamDB))
        .map((streamDB) => this.fromStreamDB_(streamDB, timeline));
      const imageStreams: shaka.extern.Stream[] = manifestDB.streams
        .filter((streamDB) => this.isImage_(streamDB))
        .map((streamDB) => this.fromStreamDB_(streamDB, timeline));
     ...
     */
    createVariants(
      audios: shaka.extern.StreamDB[],
      videos: shaka.extern.StreamDB[],
    ...
    private fromSegmentDB_(
      index: number,
      segmentDB: shaka.extern.SegmentDB
    ): SegmentReference {
      const uri: OfflineUri = shaka.offline.OfflineUri.segment(
        this.mechanism_,
        this.cell_,
        segmentDB.dataKey
      );
      const initSegmentReference =
        segmentDB.initSegmentKey != null
