...
namespace shaka.offline {
  ...
  export class SessionDeleter {
    ...
    async delete(
      ...
      sessions: shaka.extern.EmeSessionDB[]
    ): Promise<string[]> {
    ...
    private async doDelete_(
      ...
      try {
        ...
        await drmEngine.initForRemoval(
          bucket.info.keySystem,
          bucket.info.licenseUri,
          bucket.info.serverCertificate,
          bucket.info.audioCapabilities,
      ...
      await Promise.all(
        bucket.sessionIds.map(async (sessionId) => {
    ...
    private static isCompatible_(
      a: shaka.extern.EmeSessionDB,
      b: shaka.extern.EmeSessionDB
    ): boolean {
