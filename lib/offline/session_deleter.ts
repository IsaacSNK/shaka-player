...
export class SessionDeleter {
  ...
  /**
     ...
     */ 
  async delete(config: shaka.extern.DrmConfiguration, netEngine: NetworkingEngine, sessions: shaka.extern.EmeSessionDB[]): Promise<string[]> {
  ...
  /**
     ...
     */ 
  private async doDelete_(config: shaka.extern.DrmConfiguration, netEngine: NetworkingEngine, bucket: Bucket_): Promise<string[]> {
    const drmEngine: DrmEngine = new DrmEngine({netEngine:netEngine, onError: 
    () => {
    }, onKeyStatus: 
    () => {
    }, onExpirationUpdated: 
    ...
    try {
      ...
      await drmEngine.initForRemoval(bucket.info.keySystem, bucket.info.licenseUri, bucket.info.serverCertificate, bucket.info.audioCapabilities, bucket.info.videoCapabilities);
    } catch (e) {
    ...
    await Promise.all(bucket.sessionIds.map( 
    async(sessionId) => {
  ...
  /**
     ...
     *
     */ 
  private static createBuckets_(sessions: shaka.extern.EmeSessionDB[]): Bucket_[] {
    const SessionDeleter = SessionDeleter;
    const ret: Bucket_[] = [];
  ...
  /**
     ...
     */ 
  private static isCompatible_(a: shaka.extern.EmeSessionDB, b: shaka.extern.EmeSessionDB): boolean {
    ...
    const comp =  
    (x, y) => x.robustness == y.robustness && x.contentType == y.contentType;
    return a.keySystem == b.keySystem && a.licenseUri == b.licenseUri && ArrayUtils.hasSameElements(a.audioCapabilities, b.audioCapabilities, comp) && ArrayUtils.hasSameElements(a.videoCapabilities, b.videoCapabilities, comp);
  }
}
type Bucket_ = {info:shaka.extern.EmeSessionDB, sessionIds:string[]};
