...
import*as SessionDeleterExports from './session_deleter';
...
import*as StreamBandwidthEstimatorExports from './stream_bandwidth_estimator';
...
import{PlayerConfiguration}from './player_configuration';
...
import{StorageCellHandle}from './storage_muxer';
...
export class Storage implements IDestroyable {
  ...
  constructor(player?: shaka.Player) {
    ...
    if (player && player.constructor != shaka.Player) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.LOCAL_PLAYER_INSTANCE_REQUIRED);
    }
    ...
    if (player) {
      ...
      asserts.assert(this.networkingEngine_, 'Storage should not be initialized with a player that had ' + '|destroy| called on it.');
    } else {
    ...
    async() => {
      ...
      await Promise.all(this.openDownloadManagers_.map( 
      (dl) => dl.abortAll()));
  ...
  configure(config: string | Object, value?: any): boolean {
    ...
    asserts.assert(this.config_, 'Cannot reconfigure storage after calling destroy.');
  ...
  getConfiguration(): shaka.extern.PlayerConfiguration {
    ...
    PlayerConfiguration.mergeConfigObjects(ret, this.config_, PlayerConfiguration.createDefault());
    return ret;
  }
  ...
  /**
     ...
     * @export
     */ 
  store(uri: string, appMetadata?: Object, mimeType?: string): shaka.extern.IAbortableOperation<shaka.extern.StoredContent> {
    asserts.assert(this.networkingEngine_, 'Cannot call |store| after calling |destroy|.');
     
    // Get a copy of the current config. 
    const config = this.getConfiguration();
    const getParser =  
    async() => {
      asserts.assert(this.networkingEngine_, 'Should not call |store| after |destroy|');
      const factory = await ManifestParser.getFactory(uri, this.networkingEngine_, config.manifest.retryParameters, mimeType || null);
      return factory();
    };
    const downloader: DownloadManager = new DownloadManager(this.networkingEngine_);
    this.openDownloadManagers_.push(downloader);
    const storeOp = this.store_(uri, appMetadata || {}, getParser, config, downloader);
  ...
  /**
     ...
     *
     */ 
  private async store_(uri: string, appMetadata: Object, getParser: () => Promise<shaka.extern.ManifestParser>, config: shaka.extern.PlayerConfiguration, downloader: DownloadManager): Promise<shaka.extern.StoredContent> {
    ...
    try {
      ...
      const canDownload = !manifest.presentationTimeline.isLive() && !manifest.presentationTimeline.isInProgress();
      if (!canDownload) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.CANNOT_STORE_LIVE_OFFLINE, uri);
      }
       
      // Create the DRM engine, and load the keys in the manifest. 
      drmEngine = await this.createDrmEngine(manifest,  
      (e) => {
        drmError = drmError || e;
      }, config);
      ...
      const {manifestDB, toDownload} = this.makeManifestDB_(drmEngine, manifest, uri, appMetadata, config, downloader);
      ...
      await this.downloadSegments_(toDownload, manifestId, manifestDB, downloader, config, activeHandle.cell, manifest, drmEngine);
      this.ensureNotDestroyed_();
      const offlineUri = OfflineUri.manifest(activeHandle.path.mechanism, activeHandle.path.cell, manifestId);
  ...
  /**
     ...
     *
     */ 
  private async downloadSegments_(toDownload: DownloadInfo[], manifestId: number, manifestDB: shaka.extern.ManifestDB, downloader: DownloadManager, config: shaka.extern.PlayerConfiguration, storage: shaka.extern.StorageCell, manifest: shaka.extern.Manifest, drmEngine: DrmEngine): Promise {
    let pendingManifestUpdates = {};
    let pendingDataSize = 0;
    const download =  
    async(toDownload: DownloadInfo[], updateDRM: boolean) => {
      for (const download of toDownload) {
        ...
        downloader.queue(download.groupId, request, estimateId, isInitSegment, onDownloaded);
      }
    ...
    try {
      if (this.getManifestIsEncrypted_(manifest) && usingBgFetch && !this.getManifestIncludesInitData_(manifest)) {
        ...
        // anything else. 
        await download(toDownload.filter( 
        (info) => info.isInitSegment), true);
        this.ensureNotDestroyed_();
        toDownload = toDownload.filter( 
        (info) => !info.isInitSegment);
        ...
        pendingDataSize = 0;
        await Storage.assignSegmentsToManifest(storage, manifestId, manifestDB, manifestUpdates, dataSize,  
        () => this.ensureNotDestroyed_());
        this.ensureNotDestroyed_();
      }
      if (!usingBgFetch) {
        ...
        () => this.ensureNotDestroyed_());
        this.ensureNotDestroyed_();
        asserts.assert(!manifestDB.isIncomplete, 'The manifest should be complete by now');
      } else {
      }
    } catch (error) {
      ...
      await storage.removeSegments(dataKeys,  
      (key) => {
      });
      throw error;
    }
  ...
  static async cleanStoredManifest(manifestId: number): Promise {
    ...
    const uri = OfflineUri.manifest(activeHandle.path.mechanism, activeHandle.path.cell, manifestId);
    await muxer.destroy();
  ...
  /**
     ...
     */ 
  static async assignSegmentsToManifest(storage: shaka.extern.StorageCell, manifestId: number, manifestDB: shaka.extern.ManifestDB, manifestUpdates: {[key:string]:number}, dataSizeUpdate: number, throwIfAbortedFn: () => any): Promise {
    ...
    try {
      ...
      for (const stream of manifestDB.streams) {
        for (const segment of stream.segments) {
          let dataKey = segment.pendingSegmentRefId ? manifestUpdates[segment.pendingSegmentRefId] : null;
          if (dataKey != null) {
          ...
          dataKey = segment.pendingInitSegmentRefId ? manifestUpdates[segment.pendingInitSegmentRefId] : null;
          if (dataKey != null) {
  ...
  /**
     ...
     *
     */ 
  private async filterManifest_(manifest: shaka.extern.Manifest, drmEngine: DrmEngine, config: shaka.extern.PlayerConfiguration): Promise {
    ...
    await StreamUtils.filterManifestByMediaCapabilities(manifest, config.offline.usePersistentLicense);
    ...
    StreamUtils.chooseCodecsAndFilterManifest(manifest, preferredVideoCodecs, preferredAudioCodecs, preferredAudioChannelCount, preferredDecodingAttributes);
    for (const variant of manifest.variants) {
      asserts.assert(StreamUtils.isPlayable(variant), 'We should have already filtered by "is playable"');
    ...
    try {
      const allowedDownload = await config.offline.downloadSizeCallback(sizeEstimate);
      if (!allowedDownload) {
        throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.STORAGE_LIMIT_REACHED);
      }
    } catch (e) {
      ...
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.DOWNLOAD_SIZE_CALLBACK_ERROR);
    }
    ...
    (variant) => variantIds.has(variant.id));
    manifest.textStreams = manifest.textStreams.filter( 
    (stream) => textIds.has(stream.id));
    manifest.imageStreams = manifest.imageStreams.filter( 
    (stream) => imageIds.has(stream.id));
  ...
  /**
     ...
     * }}
     */ 
  private makeManifestDB_(drmEngine: DrmEngine, manifest: shaka.extern.Manifest, uri: string, metadata: Object, config: shaka.extern.PlayerConfiguration, downloader: DownloadManager): {manifestDB:shaka.extern.ManifestDB, toDownload:DownloadInfo[]} {
    const pendingContent = StoredContentUtils.fromManifest(uri, manifest,  
    /* size= */ 
    0, metadata);
    ...
    const onInitData =  
    (initData, systemId) => {
      if (needsInitData && config.offline.usePersistentLicense && currentSystemId == systemId) {
        drmEngine.newInitData('cenc', initData);
      }
    };
    downloader.setCallbacks(onProgress, onInitData);
    const needsInitData = this.getManifestIsEncrypted_(manifest) && !this.getManifestIncludesInitData_(manifest);
    ...
    const {streams, toDownload} = this.createStreams_(downloader, estimator, drmEngine, manifest, config);
    const drmInfo = drmEngine.getDrmInfo();
  ...
  private getManifestIncludesInitData_(manifest: shaka.extern.Manifest): boolean {
    return manifest.variants.some( 
    (variant) => {
  ...
  private setManifestDrmFields_(manifest: shaka.extern.Manifest, manifestDB: shaka.extern.ManifestDB, drmEngine: DrmEngine, config: shaka.extern.PlayerConfiguration) {
    ...
    if (this.getManifestIsEncrypted_(manifest) && config.offline.usePersistentLicense && !sessions.length) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.NO_INIT_DATA_FOR_OFFLINE);
    }
  }
  ...
  private async remove_(contentUri: string): Promise {
    ...
    if (nullableUri == null || !nullableUri.isManifest()) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.MALFORMED_OFFLINE_URI, contentUri);
    }
    ...
    try {
      ...
      await Promise.all([this.removeFromDRM_(uri, manifest, muxer), this.removeFromStorage_(cell, uri, manifest)]);
    } finally {
      await muxer.destroy();
    }
  }
   
  private static getCapabilities_(manifestDb: shaka.extern.ManifestDB, isVideo: boolean): MediaKeySystemMediaCapability[] {
    const MimeUtils = MimeUtils;
    const ret = [];
    for (const stream of manifestDb.streams) {
      if (isVideo && stream.type == 'video') {
        ret.push({contentType:MimeUtils.getFullType(stream.mimeType, stream.codecs), robustness:manifestDb.drmInfo.videoRobustness});
      } else {
        if (!isVideo && stream.type == 'audio') {
          ret.push({contentType:MimeUtils.getFullType(stream.mimeType, stream.codecs), robustness:manifestDb.drmInfo.audioRobustness});
        }
      }
    }
    return ret;
  ...
  private async removeFromDRM_(uri: OfflineUri, manifestDb: shaka.extern.ManifestDB, muxer: StorageMuxer): Promise {
    asserts.assert(this.networkingEngine_, 'Cannot be destroyed');
    await Storage.deleteLicenseFor_(this.networkingEngine_, this.config_.drm, muxer, manifestDb);
  }
   
  private removeFromStorage_(storage: shaka.extern.StorageCell, uri: OfflineUri, manifest: shaka.extern.ManifestDB): Promise {
    ...
    return Promise.all([storage.removeSegments(segmentIds, onRemove), storage.removeManifests([uri.key()], onRemove)]);
  }
  ...
  /**
     ...
     *
     */ 
  async parseManifest(uri: string, parser: shaka.extern.ManifestParser, config: shaka.extern.PlayerConfiguration): Promise<shaka.extern.Manifest> {
    let error = null;
    const networkingEngine = this.networkingEngine_;
    asserts.assert(networkingEngine, 'Should be initialized!');
    const playerInterface: shaka.extern.ManifestParser.PlayerInterface = {networkingEngine:networkingEngine,  
    ...
    () => false, isAutoLowLatencyMode: 
    () => false, enableLowLatencyMode: 
    () => {
    }};
    ...
    await Promise.all(Iterables.map(streams,  
    (stream) => {
  ...
  /**
     ...
     *
     */ 
  async createDrmEngine(manifest: shaka.extern.Manifest, onError: (p1: Error) => any, config: shaka.extern.PlayerConfiguration): Promise<DrmEngine> {
    asserts.assert(this.networkingEngine_, 'Cannot call |createDrmEngine| after |destroy|');
    const drmEngine: DrmEngine = new DrmEngine({netEngine:this.networkingEngine_, onError:onError, onKeyStatus: 
    () => {
    }, onExpirationUpdated: 
    () => {
    }, onEvent: 
    ...
    await drmEngine.initForStorage(manifest.variants, config.offline.usePersistentLicense);
  ...
  /**
     ...
     * }}
     */ 
  private createStreams_(downloader: DownloadManager, estimator: StreamBandwidthEstimator, drmEngine: DrmEngine, manifest: shaka.extern.Manifest, config: shaka.extern.PlayerConfiguration): {streams:shaka.extern.StreamDB[], toDownload:DownloadInfo[]} {
    ...
    for (const stream of streamSet) {
      const streamDB = this.createStream_(downloader, estimator, manifest, stream, config, toDownload);
    ...
    return {streams:Array.from(streamDBs.values()), toDownload:Array.from(toDownload.values())};
  }
  ...
  /**
     ...
     *
     */ 
  private createStream_(downloader: DownloadManager, estimator: StreamBandwidthEstimator, manifest: shaka.extern.Manifest, stream: shaka.extern.Stream, config: shaka.extern.PlayerConfiguration, toDownload: Map<string, DownloadInfo>): shaka.extern.StreamDB {
    const streamDb: shaka.extern.StreamDB = {id:stream.id, originalId:stream.originalId, primary:stream.primary, type:stream.type, mimeType:stream.mimeType, codecs:stream.codecs, frameRate:stream.frameRate, pixelAspectRatio:stream.pixelAspectRatio, hdr:stream.hdr, kind:stream.kind, language:stream.language, label:stream.label, width:stream.width || null, height:stream.height || null, encrypted:stream.encrypted, keyIds:stream.keyIds, segments:[], variantIds:[], roles:stream.roles, forced:stream.forced, 
    channelsCount:stream.channelsCount, audioSamplingRate:stream.audioSamplingRate, spatialAudio:stream.spatialAudio, closedCaptions:stream.closedCaptions, tilesLayout:stream.tilesLayout};
    const startTime = manifest.presentationTimeline.getSegmentAvailabilityStart();
    const numberOfParallelDownloads = config.offline.numberOfParallelDownloads;
    let groupId = 0;
    Storage.forEachSegment_(stream, startTime,  
    (segment) => {
      ...
      if (!toDownload.has(pendingSegmentRefId)) {
        const estimateId = downloader.addDownloadEstimate(estimator.getSegmentEstimate(stream.id, segment));
        const segmentDownload = new DownloadInfo(segment, estimateId, groupId,  
        /* isInitSegment= */ 
        false);
      ...
      if (segment.initSegmentReference) {
        pendingInitSegmentRefId = DownloadInfo.idForSegmentRef(segment.initSegmentReference);
        if (!toDownload.has(pendingInitSegmentRefId)) {
          const estimateId = downloader.addDownloadEstimate(estimator.getInitSegmentEstimate(stream.id));
          const initDownload = new DownloadInfo(segment.initSegmentReference, estimateId, groupId,  
          /* isInitSegment= */ 
          true);
          toDownload.set(pendingInitSegmentRefId, initDownload);
        }
      }
      const segmentDB: shaka.extern.SegmentDB = {pendingInitSegmentRefId, initSegmentKey:pendingInitSegmentRefId ? 0 : null, startTime:segment.startTime, endTime:segment.endTime, appendWindowStart:segment.appendWindowStart, appendWindowEnd:segment.appendWindowEnd, timestampOffset:segment.timestampOffset, tilesLayout:segment.tilesLayout, pendingSegmentRefId, dataKey:0};
  ...
  private static forEachSegment_(stream: shaka.extern.Stream, startTime: number, callback: (p1: SegmentReference) => any) {
  ...
  private ensureNotDestroyed_() {
    if (this.destroyer_.destroyed()) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.OPERATION_ABORTED);
    }
  }
  ...
  private requireSupport_() {
    if (!Storage.support()) {
      throw new Error(ErrorExports.Severity.CRITICAL, ErrorExports.Category.STORAGE, ErrorExports.Code.STORAGE_NOT_SUPPORTED);
    }
  }
  ...
  /**
     ...
     * @template T
     */ 
  private startAbortableOperation_(action: shaka.extern.IAbortableOperation<T>): shaka.extern.IAbortableOperation<T> {
  ...
  private static getAllSegmentIds_(manifest: shaka.extern.ManifestDB): number[] {
  ...
  private static async deleteLicenseFor_(net: NetworkingEngine, drmConfig: shaka.extern.DrmConfiguration, muxer: StorageMuxer, manifestDb: shaka.extern.ManifestDB): Promise {
    if (!manifestDb.drmInfo) {
      return;
    }
    const sessionIdCell = muxer.getEmeSessionCell();
    const sessions: shaka.extern.EmeSessionDB[] = manifestDb.sessionIds.map( 
    (sessionId) => {
      return {sessionId:sessionId, keySystem:manifestDb.drmInfo.keySystem, licenseUri:manifestDb.drmInfo.licenseServerUri, serverCertificate:manifestDb.drmInfo.serverCertificate, audioCapabilities:Storage.getCapabilities_(manifestDb,  
      /* isVideo= */ 
      false), videoCapabilities:Storage.getCapabilities_(manifestDb,  
      /* isVideo= */ 
      true)};
    });
  ...
  /**
     ...
     *
     */ 
  private static getAllStreamsFromManifest_(manifest: shaka.extern.Manifest): Set<shaka.extern.Stream> {
  ...
  private static validateManifest_(manifest: shaka.extern.Manifest) {
    const videos = new Set(manifest.variants.map( 
    (v) => v.video));
    const audios = new Set(manifest.variants.map( 
    (v) => v.audio));
    ...
    for (const audio1 of audios) {
      for (const audio2 of audios) {
        if (audio1 != audio2 && audio1.language == audio2.language) {
          log.warning('Similar audio tracks were selected to be stored', audio1.id, audio2.id);
        }
      }
    }
    for (const text1 of texts) {
      for (const text2 of texts) {
        if (text1 != text2 && text1.language == text2.language) {
          log.warning('Similar text tracks were selected to be stored', text1.id, text2.id);
        }
      }
    }
  }
...
export const defaultSystemIds_ = (new Map()).set('org.w3.clearkey', '1077efecc0b24d02ace33c1e52e2fb4b').set('com.widevine.alpha', 'edef8ba979d64acea3c827dcd51d21ed').set('com.microsoft.playready', '9a04f07998404286ab92e65be0885f95').set('com.microsoft.playready.recommendation', '9a04f07998404286ab92e65be0885f95').set('com.microsoft.playready.software', '9a04f07998404286ab92e65be0885f95').set('com.microsoft.playready.hardware', '9a04f07998404286ab92e65be0885f95').set('com.adobe.primetime', 'f239e769efa348509c16a903c6932efb');
