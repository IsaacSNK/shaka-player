  ...
  export class Storage implements IDestroyable {
    ...
    constructor(player?: Player) {
      ...
      this.destroyer_ = new shaka.util.Destroyer(async () => {
        ...
        await Promise.all(
          this.openDownloadManagers_.map((dl) => dl.abortAll())
        );
    ...
    configure(config: string | Object, value?: any): boolean {
      ...
      goog.asserts.assert(
        this.config_,
        "Cannot reconfigure storage after calling destroy."
      );
    ...
    store(
      ...
      const downloader: DownloadManager = new shaka.offline.DownloadManager(
        this.networkingEngine_
      );
      ...
      const abortableStoreOp = new shaka.util.AbortableOperation(
        storeOp,
        () => {
    ...
    private async downloadSegments_(
      ...
      try {
        if (
          this.getManifestIsEncrypted_(manifest) &&
          usingBgFetch &&
          ...
          // anything else.
          await download(
            toDownload.filter((info) => info.isInitSegment),
            true
          );
          this.ensureNotDestroyed_();
        ...
        if (!usingBgFetch) {
          ...
          this.ensureNotDestroyed_();
          goog.asserts.assert(
            !manifestDB.isIncomplete,
            "The manifest should be complete by now"
          );
        } else {
        }
      } catch (error) {
    ...
    private async filterManifest_(
      ...
      const chosenTracks = await config.offline.trackSelectionCallback(
        allTracks
      );
      ...
      try {
        const allowedDownload = await config.offline.downloadSizeCallback(
          sizeEstimate
        );
        if (!allowedDownload) {
          throw new shaka.util.Error(
    ...
    private makeManifestDB_(
      ...
      if (needsInitData) {
        ...
        currentSystemId = shaka.offline.Storage.defaultSystemIds_.get(
          drmInfo.keySystem
        );
      }
    ...
    private getManifestIncludesInitData_(
      manifest: shaka.extern.Manifest
    ): boolean {
      return manifest.variants.some((variant) => {
      ...
      for (const stream of manifestDb.streams) {
        if (isVideo && stream.type == "video") {
          ...
          if (!isVideo && stream.type == "audio") {
            ret.push({
              contentType: MimeUtils.getFullType(
                stream.mimeType,
                stream.codecs
              ),
    ...
    private removeFromStorage_(
      ...
      const onRemove = (key) => {
        removed += 1;
        this.config_.offline.progressCallback(
          pendingContent,
          removed / toRemove
        );
      };
    ...
    private async removeEmeSessions_(): Promise<boolean> {
      ...
      const muxer: StorageMuxer = new shaka.offline.StorageMuxer();
      const deleter: SessionDeleter = new shaka.offline.SessionDeleter();
    ...
    async parseManifest(
      ...
      await Promise.all(
        shaka.util.Iterables.map(streams, (stream) => {
    ...
    async createDrmEngine(
      ...
      const drmEngine: DrmEngine = new shaka.media.DrmEngine({
    ...
    private createStreams_(
      downloader: DownloadManager,
      estimator: StreamBandwidthEstimator,
      drmEngine: DrmEngine,
      manifest: shaka.extern.Manifest,
      config: shaka.extern.PlayerConfiguration
    ...
    private createStream_(
      ...
      config: shaka.extern.PlayerConfiguration,
      toDownload: Map<string, DownloadInfo>
      ...
      const numberOfParallelDownloads =
        config.offline.numberOfParallelDownloads;
      let groupId = 0;
     ...
     * @template T
     */
    private startAbortableOperation_(
    ...
    private static getAllSegmentIds_(
      manifest: shaka.extern.ManifestDB
    ): number[] {
     ...
     *
     */
    private static getAllStreamsFromManifest_(
      manifest: shaka.extern.Manifest
...
namespace shaka.offline.Storage {
  export const defaultSystemIds_ = new Map()
