...
export class DownloadManager implements IDestroyable {
  ...
  setCallbacks(onProgress: (p1: number, p2: number) => any, onInitData: (p1: Uint8Array, p2: string) => any) {
  ...
  /**
     ...
     */ 
  abortAll(): Promise {
    const promises = this.abortCallbacks_.map( 
    (callback) => callback());
  ...
  /**
     ...
     */ 
  queue(groupId: number, request: shaka.extern.Request, estimateId: number, isInitSegment: boolean, onDownloaded: (p1: BufferSource) => Promise): Promise {
    ...
    async() => {
      ...
      this.onProgress_(this.estimator_.getEstimatedProgress(), this.estimator_.getTotalDownloaded());
  ...
  /**
     ...
     *
     */ 
  private async fetchSegment_(request: shaka.extern.Request): Promise<BufferSource> {
    const type = NetworkingEngineExports.RequestType.SEGMENT;
    const action: NetworkingEngineExports.PendingRequest = this.networkingEngine_.request(type, request);
    const abortCallback =  
    () => {
      return action.abort();
    };
