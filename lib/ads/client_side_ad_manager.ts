...
export class ClientSideAdManager implements IReleasable {
  ...
  constructor(adContainer: HTMLElement, video: HTMLMediaElement, locale: string, onEvent: (p1: FakeEvent) => any) {
    this.adContainer_ = adContainer;
    this.video_ = video;
    this.onEvent_ = onEvent;
    this.eventManager_ = new shaka.util.EventManager();
    google.ima.settings.setLocale(locale);
    const adDisplayContainer = new google.ima.AdDisplayContainer(this.adContainer_, this.video_);
    ...
    this.eventManager_.listenOnce(this.adsLoader_, google.ima.AdsManagerLoadedEvent.Type.ADS_MANAGER_LOADED,  
    (e) => {
      this.onAdsManagerLoaded_((e as google.ima.AdsManagerLoadedEvent));
    });
    this.eventManager_.listen(this.adsLoader_, google.ima.AdErrorEvent.Type.AD_ERROR,  
    (e) => {
  ...
  requestAds(imaRequest: google.ima.AdsRequest) {
    asserts.assert(imaRequest.adTagUrl || imaRequest.adsResponse, 'The ad tag needs to be set up before requesting ads, ' + 'or adsResponse must be filled.');
  ...
  private onAdError_(e: google.ima.AdErrorEvent) {
    shaka.log.warning('There was an ad error from the IMA SDK: ' + e.getError());
    ...
    this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.CUEPOINTS_CHANGED, (new Map()).set('cuepoints', [])));
  }
  ...
  private onAdsManagerLoaded_(e: google.ima.AdsManagerLoadedEvent) {
    ...
    this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.ADS_LOADED, (new Map()).set('loadTime', loadTime)));
    this.imaAdsManager_ = e.getAdsManager(this.video_);
    this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.IMA_AD_MANAGER_LOADED, (new Map()).set('imaAdManager', this.imaAdsManager_)));
    ...
    if (cuePointStarts.length) {
      ...
      for (const start of cuePointStarts) {
        const shakaCuePoint: shaka.extern.AdCuePoint = {start:start, end:null};
        cuePoints.push(shakaCuePoint);
      }
      this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.CUEPOINTS_CHANGED, (new Map()).set('cuepoints', cuePoints)));
    }
    this.addImaEventListeners_();
    try {
      const viewMode = this.isFullScreenEnabled_() ? google.ima.ViewMode.FULLSCREEN : google.ima.ViewMode.NORMAL;
      this.imaAdsManager_.init(this.video_.offsetWidth, this.video_.offsetHeight, viewMode);
      ...
      () => {
        const viewMode = this.isFullScreenEnabled_() ? google.ima.ViewMode.FULLSCREEN : google.ima.ViewMode.NORMAL;
        this.imaAdsManager_.resize(this.video_.offsetWidth, this.video_.offsetHeight, viewMode);
      });
  ...
  private addImaEventListeners_() {
    const convertEventAndSend =  
    (e: Event, type: string) => {
      const data = (new Map()).set('originalEvent', e);
      this.onEvent_(new shaka.util.FakeEvent(type, data));
    };
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdErrorEvent.Type.AD_ERROR,  
    (error) => {
    ...
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.STARTED,  
    (e) => {
      this.onAdStart_((e as google.ima.AdEvent));
    });
    ...
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.MIDPOINT,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_MIDPOINT);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.THIRD_QUARTILE,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_THIRD_QUARTILE);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.COMPLETE,  
    (e) => {
    ...
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.SKIPPED,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_SKIPPED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.VOLUME_CHANGED,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_VOLUME_CHANGED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.VOLUME_MUTED,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_MUTED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.PAUSED,  
    (e) => {
    ...
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.RESUMED,  
    (e) => {
      asserts.assert(this.ad_ != null, 'Ad should not be null!');
      this.ad_.setPaused(false);
      convertEventAndSend(e, shaka.ads.AdManager.AD_RESUMED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.SKIPPABLE_STATE_CHANGED,  
    (e) => {
      asserts.assert(this.ad_ != null, 'Ad should not be null!');
      convertEventAndSend(e, shaka.ads.AdManager.AD_SKIP_STATE_CHANGED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.CLICK,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_CLICKED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.AD_PROGRESS,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_PROGRESS);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.AD_BUFFERING,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_BUFFERING);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.IMPRESSION,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_IMPRESSION);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.DURATION_CHANGE,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_DURATION_CHANGED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.USER_CLOSE,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_CLOSED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.LOADED,  
    (e) => {
    ...
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.LINEAR_CHANGED,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_LINEAR_CHANGED);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.AD_METADATA,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_METADATA);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.LOG,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_RECOVERABLE_ERROR);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.AD_BREAK_READY,  
    (e) => {
      convertEventAndSend(e, shaka.ads.AdManager.AD_BREAK_READY);
    });
    this.eventManager_.listen(this.imaAdsManager_, google.ima.AdEvent.Type.INTERACTION,  
    (e) => {
  ...
  private onAdStart_(e: google.ima.AdEvent) {
    asserts.assert(this.imaAdsManager_, 'Should have an ads manager at this point!');
    const imaAd = e.getAd();
    this.ad_ = new ClientSideAd(imaAd, this.imaAdsManager_, this.video_);
    const data = (new Map()).set('ad', this.ad_).set('sdkAdObject', imaAd).set('originalEvent', e);
    this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_STARTED, data));
  ...
  private onAdComplete_(e: google.ima.AdEvent | null) {
    this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_STOPPED, (new Map()).set('originalEvent', e)));
