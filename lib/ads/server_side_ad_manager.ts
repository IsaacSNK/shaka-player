...
export class ServerSideAdManager implements IReleasable {
  ...
  constructor(adContainer: HTMLElement, video: HTMLMediaElement, locale: string, onEvent: (p1: FakeEvent) => any) {
    ...
    const uiSettings: google.ima.dai.api.UiSettings = new google.ima.dai.api.UiSettings();
    uiSettings.setLocale(locale);
    this.streamManager_ = new google.ima.dai.api.StreamManager(this.video_, this.adContainer_, uiSettings);
    this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.IMA_STREAM_MANAGER_LOADED, (new Map()).set('imaStreamManager', this.streamManager_)));
    ...
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.ERROR,  
    () => {
      log.info('Ad SS Error');
      this.onError_();
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.AD_BREAK_STARTED,  
    () => {
      log.info('Ad Break Started');
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.STARTED,  
    (e) => {
      log.info('Ad Started');
      this.onAdStart_((e as google.ima.dai.api.StreamEvent));
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.AD_BREAK_ENDED,  
    () => {
      log.info('Ad Break Ended');
      this.onAdBreakEnded_();
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.AD_PROGRESS,  
    (e) => {
      this.onAdProgress_((e as google.ima.dai.api.StreamEvent));
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.FIRST_QUARTILE,  
    () => {
      log.info('Ad event: First Quartile');
      this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_FIRST_QUARTILE));
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.MIDPOINT,  
    () => {
      log.info('Ad event: Midpoint');
      this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_MIDPOINT));
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.THIRD_QUARTILE,  
    () => {
      log.info('Ad event: Third Quartile');
      this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_THIRD_QUARTILE));
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.COMPLETE,  
    () => {
      log.info('Ad event: Complete');
      this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_COMPLETE));
      this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_STOPPED));
      this.adContainer_.removeAttribute('ad-active');
      this.ad_ = null;
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.SKIPPED,  
    () => {
      log.info('Ad event: Skipped');
      this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_SKIPPED));
      this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_STOPPED));
    });
    this.eventManager_.listen(this.streamManager_, google.ima.dai.api.StreamEvent.Type.CUEPOINTS_CHANGED,  
    (e) => {
  ...
  streamRequest(streamRequest: google.ima.dai.api.StreamRequest, backupUrl?: string): Promise<string> {
  ...
  private checkForSnapback_() {
    ...
    const previousCuePoint = this.streamManager_.previousCuePointForStreamTime(currentTime);
    ...
    if (previousCuePoint && !previousCuePoint.played) {
      log.info('Seeking back to the start of the ad break at ' + previousCuePoint.start + ' and will return to ' + currentTime);
  ...
  private onAdStart_(e: google.ima.dai.api.StreamEvent) {
    asserts.assert(this.streamManager_, 'Should have a stream manager at this point!');
    const imaAd = e.getAd();
    ...
    this.onEvent_(new shaka.util.FakeEvent(shaka.ads.AdManager.AD_STARTED, (new Map()).set('ad', this.ad_)));
  ...
  private onError_() {
    if (!this.backupUrl_.length) {
      this.streamPromise_.reject('IMA Stream request returned an error ' + 'and there was no backup asset uri provided.');
      this.streamPromise_ = null;
      return;
    }
    log.warning('IMA stream request returned an error. ' + 'Falling back to the backup asset uri.');
  ...
  private onCuePointsChanged_(e: google.ima.dai.api.StreamEvent) {
    ...
    for (const point of streamData.cuepoints) {
      const shakaCuePoint: shaka.extern.AdCuePoint = {start:point.start, end:point.end};
